(() => {
  ...
  function matchesAllowed(normalized) {
    ...
    if (normalized === "/verkoper/dashboard" || normalized.startsWith("/verkoper/dashboard/"))
      return true;
  ...
  function isSafeRestorablePath(path) {
  ...
  function toTrailingSlashPath(path) {
    const n = normalizePath(path);
    if (n === "/") return "/";
    return `${n}/`;
  }
  function parseInternalPathFromUnknownInput(raw) {
    const t = raw.trim();
    if (!t) return null;
  ...
  // lib/native/nativeShellKeys.ts
  var NATIVE_PENDING_ROUTE_KEY = "hc_cap_pending_route";

  // lib/native/pushDeepLink.ts
  function readDataRecord(data) {
    if (!data || typeof data !== "object") return {};
    return data;
  }
  function pathFromPushNotificationData(data) {
    const d = readDataRecord(data);
    const direct = typeof d.path === "string" ? d.path : typeof d.route === "string" ? d.route : typeof d.link === "string" ? d.link : typeof d.actionUrl === "string" ? d.actionUrl : null;
    if (direct) {
      const parsed = parseInternalPathFromUnknownInput(direct);
      if (parsed) return parsed;
    }
    const conversationId = typeof d.conversationId === "string" ? d.conversationId : typeof d.conversation_id === "string" ? d.conversation_id : null;
    ...
    const orderId = typeof d.orderId === "string" ? d.orderId : typeof d.order_id === "string" ? d.order_id : null;
  ...
  function storePendingNativeRoute(path) {
  ...
  // scripts/native-shell-cap-entry.ts
  function init() {
    if (typeof window === "undefined") return;
