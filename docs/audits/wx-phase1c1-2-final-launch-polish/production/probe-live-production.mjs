...
async function main() {
  ...
  const ownership = {
    geoFeedOwner: "GeoFeed",
    controlledHost: "inactive",
    workspace: matrix[0]?.snap?.visibleAdaptive || null,
    availableSpace: matrix.map((m) => m.snap.workspaceClass),
    scrollOwnership: matrix.map((m) => m.snap.scrollOwner),
    presentationOwnership: matrix.map((m) => m.snap.presentationDrives),
    plannerOwnership: "unchanged",
    remounts: matrix.map((m) => m.snap.remount),
    hostCounts: matrix.map((m) => m.snap.hostCount),
    pass: matrix.every((m) => m.snap.presentationDrives === "0" && m.snap.remount === "0" && (m.snap.hostCount||0) <= 1),
  };
  writeFileSync(join(outDir, "ownership-live.json"), JSON.stringify(ownership, null, 2));
  writeFileSync(join(outDir, "performance-summary.json"), JSON.stringify({
    remount: matrix.every((m) => m.snap.remount === "0"),
    pageErrors,
    overflow: matrix.some((m) => m.snap.overflow),
    presentationDrives: matrix.every((m) => m.snap.presentationDrives === "0"),
  }, null, 2));
