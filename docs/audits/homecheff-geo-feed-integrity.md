...
- National mainland filter server-side (sales + inspiration; Caribbean place labels excluded even when coords are null); nearby strict_local; nearby without location returns **empty** (explicit client empty state — never unrelated inspiration).
