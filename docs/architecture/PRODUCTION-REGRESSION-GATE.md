...
The build exits 0. There is no `npm test` script. GitHub Actions deploy is manual
and only runs `npm run build`. `scripts/smoke-check.mjs` checks that route files
...
Authenticated steps run only when `SMOKE_EMAIL` and `SMOKE_PASSWORD` are set.
This suite is not part of the Vercel build: the build image has no browser, and
