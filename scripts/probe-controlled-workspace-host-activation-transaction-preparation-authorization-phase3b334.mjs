#!/usr/bin/env node
/**
 * Phase 3B.3.34 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3055";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b334");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_34") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 35;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionPreparationAuthorization = await api.readControlledWorkspaceHostActivationTransactionPreparationAuthorization();
      ...
      return {
        ...
        hostContract,
        hostRegistry,
        transactionPreparationAuthorization,
        commitBoundary,
        activationAttempt,
        hostPlan,
      };
    ...
    const reg = probe.transactionPreparationAuthorization;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionPreparationAuthorizationCount === 1,
      noDuplicateEntry: reg.duplicateTransactionPreparationAuthorizationCount === 0,
    ...
    const transactionPreparationAuthorizationMetaOk =
      probe.version === 35 &&
      reg.phase === "3B.3.34" &&
      ...
      reg.transactionPreparationAuthorizationState === "TRANSACTION_PREPARATION_AUTHORIZED_NOT_PREPARED" &&
      reg.transactionPreparationAuthorizationResult === "controlled-workspace-host-activation-transaction-preparation-authorized-not-prepared" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionPreparationAuthorizationCount === 1 &&
      reg.duplicateTransactionPreparationAuthorizationCount === 0 &&
      ...
      reg.nextEligibleStep === "3B.3.35" &&
      reg.activationBlocker === "PHASE_3B3_34_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_ONLY" &&
    ...
    const anyFail =
      ...
      !transactionPreparationAuthorizationMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionPreparationAuthorization: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_35" : "READY_FOR_PHASE_3B_3_35",
    };

    const proofPath = join(outDir, "phase3b3-34-controlled-workspace-host-activation-transaction-preparation-authorization-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.34",
      status: "controlled-workspace-host-activation-transaction-preparation-authorization-prepared",
      transactionPreparationAuthorizationContract: "valid",
    ...
    writeFileSync(
      join(outDir, "phase3b3-34-controlled-workspace-host-activation-transaction-preparation-authorization-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.34 Controlled Workspace Host Activation Transaction Preparation Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionPreparationAuthorizationResult} state=${reg.transactionPreparationAuthorizationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- transactionPreparationAuthorizationMetaOk: ${transactionPreparationAuthorizationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-34-controlled-workspace-host-activation-transaction-preparation-authorization-summary.md"),
      summary,
    );
