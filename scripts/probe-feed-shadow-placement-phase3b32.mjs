#!/usr/bin/env node
/**
 * Phase 3B.3.2 — Chromium proof: Controlled Host Shadow Placement.
 * New proof run (not a reuse of 3B.3.1). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3023";
  let commit = "unknown";
  let branch = "workspace/phase3b32-controlled-host-shadow-placement";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b32");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 3;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; shadow placement; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const placementMetaOk =
      shadowPlacement.placementState === "shadow-registered" &&
      shadowPlacement.placementMode === "sibling-after-legacy-mount" &&
      shadowPlacement.hostActivation === false &&
      shadowPlacement.renderActivation === false &&
      shadowPlacement.registrationVisibleInMetadata === true &&
      shadowIdentity.expectedMountCount === 1 &&
      shadowIdentity.identityTransitionAllowed === false &&
      hostPlan.placementState === "shadow-registered";
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "SHADOW_PLACEMENT_REGISTERED",
        "SHADOW_WITH_PLACED_HOST",
      ...
      overallVerdict:
        anyFail || !placementMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_3"
          : "READY_FOR_PHASE_3B_3_3",
    };

    const outPath = join(outDir, "phase3b3-2-feed-shadow-placement-proof.json");
    ...
    const readiness = {
      ...
      browserProof: anyFail || !placementMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-2-feed-shadow-placement-readiness.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-2-feed-shadow-placement-summary.md"),
      [
        `# Phase 3B.3.2 Shadow Placement Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Shadow placement: ${shadowPlacement.placementState} / ${shadowPlacement.placementMode}`,
        `- Host activation: false (blocked by PHASE_3B3_2_SHADOW_PLACEMENT_ONLY)`,
    ...
    await browser.close();
    process.exit(anyFail || !placementMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
