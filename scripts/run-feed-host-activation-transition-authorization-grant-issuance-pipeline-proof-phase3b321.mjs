#!/usr/bin/env node
/**
 * Phase 3B.3.21 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.21 host activation transition authorization grant issuance
...
const port = process.env.PHASE3B321_PORT || "3042";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b319 = join(root, "docs/audits/artifacts/phase3b320");
const out3b320 = join(root, "docs/audits/artifacts/phase3b321");
...
async function main() {
  ...
  if (priorIssuanceDecisionProof.overallVerdict !== "READY_FOR_PHASE_3B_3_21") {
    throw new Error(
      `Phase 3B.3.20 issuance decision proof required: ${priorIssuanceDecisionProof.overallVerdict}`,
    );
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b321] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b321] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log(
      "[phase3b321] Phase 3B.3.21 host activation transition authorization grant issuance plan proof",
    );
    await run("node", [
    ...
    if (p319.overallVerdict !== "READY_FOR_PHASE_3B_3_22") {
      throw new Error(`Phase 3B.3.21 proof failed: ${p319.overallVerdict}`);
    }
    ...
    console.log("[phase3b321] READY_FOR_PHASE_3B_3_22");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b320, "phase3b3-21-server-log-tail.txt"),
