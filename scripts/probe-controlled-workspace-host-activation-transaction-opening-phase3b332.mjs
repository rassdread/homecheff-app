#!/usr/bin/env node
/**
 * Phase 3B.3.32 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3053";
  let commit = "unknown";
  let branch = "workspace/phase3b332-controlled-workspace-host-activation-transaction-opening";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b332");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_32") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 33;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionOpening = await api.readControlledWorkspaceHostActivationTransactionOpening();
    ...
    const reg = probe.transactionOpening;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionOpeningCount === 1,
      noDuplicateEntry: reg.duplicateTransactionOpeningCount === 0,
    ...
    const activationTransactionOpeningMetaOk =
      probe.version === 33 &&
      reg.phase === "3B.3.32" &&
      ...
      reg.transactionOpeningState === "TRANSACTION_OPENED_NOT_PREPARED" &&
      reg.transactionOpeningResult === "controlled-workspace-host-activation-transaction-opened-not-prepared" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionOpeningCount === 1 &&
      reg.duplicateTransactionOpeningCount === 0 &&
      ...
      reg.nextEligibleStep === "3B.3.33" &&
      reg.activationBlocker === "PHASE_3B3_32_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_ONLY" &&
      (reg.predecessorActivationTransactionOpeningAuthorizationState === "TRANSACTION_OPENING_AUTHORIZED_NOT_OPENED" ||
        reg.predecessorActivationCommitBoundaryEntryState === "COMMIT_BOUNDARY_ENTERED") &&
    ...
    const anyFail =
      ...
      !activationTransactionOpeningMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionOpening: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_33" : "READY_FOR_PHASE_3B_3_33",
    };

    const proofPath = join(outDir, "phase3b3-32-controlled-workspace-host-activation-transaction-opening-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-32-controlled-workspace-host-activation-transaction-opening-prepared.json"),
    ...
    const summary = [
      ...
      `- activationTransactionOpeningMetaOk: ${activationTransactionOpeningMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
