#!/usr/bin/env node
/**
 * Phase 3B.3.31 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.31 activation commit-boundary entry Chromium proof
 */
...
const port = process.env.PHASE3B331_PORT || "3052";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b330 = join(root, "docs/audits/artifacts/phase3b331");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_31") {
    throw new Error(`Phase 3B.3.30 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b331] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b331] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b331] Phase 3B.3.31 transaction-opening authorization proof");
    await run("node", [
    ...
    if (p331.overallVerdict !== "READY_FOR_PHASE_3B_3_32") {
      throw new Error(`Phase 3B.3.31 proof failed: ${p331.overallVerdict}`);
    }
    if (p331.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p331.commit} HEAD=${commit}`);
    }
    console.log("[phase3b331] READY_FOR_PHASE_3B_3_32");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
