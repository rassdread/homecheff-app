#!/usr/bin/env node
/**
 * WX Phase 1B.5.5 — Tool & Action Surface Presentation browser proof.
 * Asserts tool-action diagnostics present, zero visible tool chrome delta, no remount.
 */
...
function parseArgs(argv) {
  ...
  let outDir = join(process.cwd(), "docs/audits/wx-phase1b5-5-tool-action-presentation");
  let journey = false;
  for (const a of argv) {
...
async function snap(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      toolActionVersion: ws?.getAttribute("data-wx-tool-action-version"),
      toolActionToken: ws?.getAttribute("data-wx-tool-action-token"),
      toolActionStatus: ws?.getAttribute("data-wx-tool-action-status"),
      ...
      toolIds: ws?.getAttribute("data-wx-tool-ids"),
      toolPersistent: ws?.getAttribute("data-wx-tool-persistent"),
      toolReachable: ws?.getAttribute("data-wx-tool-reachable"),
      toolAbsent: ws?.getAttribute("data-wx-tool-absent"),
      toolSuppressed: ws?.getAttribute("data-wx-tool-suppressed"),
      toolReserved: ws?.getAttribute("data-wx-tool-reserved"),
      toolFuture: ws?.getAttribute("data-wx-tool-future"),
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    const checks = {
      phase: s.phase === "1b.5.5",
      toolAction: s.toolAction === "wx-tool-action-presentation-v1",
      toolActionVersion: s.toolActionVersion === "1.0.0",
      toolRendersOff: s.toolRenders === "0",
      toolDrivesChromeOff: s.toolDrivesChrome === "0",
      toolChromeActivationOff: s.toolChromeActivation === "0",
      toolStaticChromeOn: s.toolStaticChrome === "1",
      ...
      hasToken: Boolean(s.toolActionToken),
      hasOrderedToolIds:
        typeof s.toolIds === "string" && s.toolIds.includes("tool") && s.toolIds.includes("action-create"),
      toolStatusOk: s.toolActionStatus === "ok",
      ...
      pass:
        ...
        steps.every(
          (s) =>
            ...
            s.snap.noNewToolUi &&
            s.snap.toolDrivesChrome === "0" &&
            s.snap.toolRenders === "0" &&
            s.snap.toolChromeActivation === "0" &&
            s.snap.toolStaticChrome === "1" &&
            s.snap.phase === "1b.5.5",
        ),
    };
    await page.close();
  ...
  const report = {
    ...
    verdict: allPass
      ? "WX_PHASE_1B5_5_BROWSER_PROOF_PASS"
      : "WX_PHASE_1B5_5_BROWSER_PROOF_FAIL",
