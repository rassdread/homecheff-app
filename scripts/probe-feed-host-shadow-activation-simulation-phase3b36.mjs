#!/usr/bin/env node
/**
 * Phase 3B.3.6 — Chromium proof: Controlled Host Shadow Activation Simulation.
 * New proof run (not a reuse of 3B.3.3). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3027";
  let commit = "unknown";
  let branch = "workspace/phase3b36-controlled-host-eligibility";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b36");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 7;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host shadow activation simulation; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const simulationMetaOk =
      ...
      hostRegistration.registrationState === "registered" &&
      hostEligibility.eligibilityState === "eligible" &&
      hostEligibility.activationBlocker ===
        "PHASE_3B3_4_HOST_ELIGIBILITY_ONLY" &&
      hostActivationReadiness.readinessState === "ready" &&
      hostActivationReadiness.activationBlocker ===
        "PHASE_3B3_5_HOST_ACTIVATION_READINESS_ONLY" &&
      hostShadowActivationSimulation.simulationState === "completed" &&
      hostShadowActivationSimulation.wouldActivate === true &&
      hostShadowActivationSimulation.canStartActivation === false &&
      hostShadowActivationSimulation.hostActivation === false &&
      hostShadowActivationSimulation.renderActivation === false &&
      hostShadowActivationSimulation.activationBlocker ===
        "PHASE_3B3_6_HOST_SHADOW_ACTIVATION_SIMULATION_ONLY" &&
      hostShadowActivationSimulation.nextEligibleStep === "3B.3.7" &&
      hostShadowActivationSimulation.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostShadowActivationSimulation.diagnostics?.simulationCompleted ===
        true &&
      hostShadowActivationSimulation.diagnostics?.wouldActivate === true &&
      hostShadowActivationSimulation.diagnostics?.activationBlocked === true &&
      hostShadowActivationSimulation.diagnostics?.canStartActivation ===
        false &&
      Array.isArray(hostShadowActivationSimulation.simulationReasons) &&
      hostShadowActivationSimulation.simulationReasons.length > 0 &&
      Array.isArray(
        hostShadowActivationSimulation.diagnostics?.activeBlockers,
      ) &&
      shadowPlacement.placementState === "shadow-registered" &&
      shadowIdentity.expectedMountCount === 1 &&
      hostPlan.registrationState === "registered" &&
      hostPlan.eligibilityState === "eligible" &&
      hostPlan.readinessState === "ready" &&
      hostPlan.simulationState === "completed" &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_SHADOW_ACTIVATION_SIMULATED",
        "SIMULATION_METADATA_VISIBLE",
        "SIMULATION_DIAGNOSTICS_VISIBLE",
      ...
      overallVerdict:
        anyFail || !simulationMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_7"
          : "READY_FOR_PHASE_3B_3_7",
    };

    const outPath = join(
      outDir,
      "phase3b3-6-feed-host-shadow-activation-simulation-proof.json",
    );
    ...
    const readiness = {
      ...
      browserProof: anyFail || !simulationMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(
        outDir,
        "phase3b3-6-feed-host-shadow-activation-simulation-prepared.json",
      ),
    ...
    writeFileSync(
      join(
        outDir,
        "phase3b3-6-feed-host-shadow-activation-simulation-summary.md",
      ),
      [
        `# Phase 3B.3.6 Host Shadow Activation Simulation Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Simulation: state=${hostShadowActivationSimulation.simulationState} wouldActivate=${hostShadowActivationSimulation.wouldActivate}`,
        `- Diagnostics: phase=${hostShadowActivationSimulation.diagnostics.currentPhase} why=${hostShadowActivationSimulation.diagnostics.whyWouldActivate}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId} state=${hostRegistry.registrationState}`,
        `- Host activation: false (blocked by PHASE_3B3_6_HOST_SHADOW_ACTIVATION_SIMULATION_ONLY); canStartActivation=false`,
    ...
    await browser.close();
    process.exit(anyFail || !simulationMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
