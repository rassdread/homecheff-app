...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3056";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b335");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 36;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionPreparation = await api.readControlledWorkspaceHostActivationTransactionPreparation();
    ...
    const reg = probe.transactionPreparation;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionPreparationCount === 1,
      noDuplicateEntry: reg.duplicateTransactionPreparationCount === 0,
    ...
    const transactionPreparedMetaOk =
      probe.version === 36 &&
      reg.phase === "3B.3.35" &&
      ...
      reg.transactionPreparationState === "TRANSACTION_PREPARED_NOT_COMMITTED" &&
      reg.transactionPreparationResult === "controlled-workspace-host-activation-transaction-prepared-not-committed" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionPreparationCount === 1 &&
      reg.duplicateTransactionPreparationCount === 0 &&
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionPreparation: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_36" : "READY_FOR_PHASE_3B_3_36",
    };

    const proofPath = join(outDir, "phase3b3-35-controlled-workspace-host-activation-transaction-preparation-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-35-controlled-workspace-host-activation-transaction-preparation-prepared.json"),
    ...
    const summary = [
      ...
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionPreparationResult} state=${reg.transactionPreparationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- transactionPreparedMetaOk: ${transactionPreparedMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-35-controlled-workspace-host-activation-transaction-preparation-summary.md"),
      summary,
    );
