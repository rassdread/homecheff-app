#!/usr/bin/env node
/**
 * Phase 3B.3.26 — Chromium proof: Controlled Workspace Host Activation Readiness.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3047";
  let commit = "unknown";
  let branch = "workspace/phase3b326-controlled-workspace-host-activation-readiness";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b326");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_26") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 27;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const readiness = await api.readControlledWorkspaceHostActivationReadiness();
    ...
    const forcedNegativeProofs = {
      ...
      candidateNotAuthorized: reg.candidateAuthorized === false,
      ...
      futureActivationNotAuthorized: reg.futureActivationAuthorized === false,
    ...
    const activationReadinessMetaOk =
      probe.version === 27 &&
      reg.phase === "3B.3.26" &&
      ...
      reg.activationReadinessContractId === "feed.discovery.adaptive-workspace.host-activation-readiness.contract.v1" &&
      reg.activationReadinessState === "READY_NOT_AUTHORIZED" &&
      reg.activationReadinessResult === "controlled-workspace-host-activation-ready-not-authorized" &&
      reg.activationReadinessCompleted === true &&
      reg.activationReadinessReady === true &&
      reg.activationReadinessBlocked === true &&
      reg.activationReadinessExecutable === false &&
      reg.candidateSelected === true &&
      reg.candidateReady === true &&
      reg.candidateAuthorized === false &&
      ...
      reg.futureActivationAuthorized === false &&
      ...
      reg.predecessorCandidateSelectionState === "SELECTED_NOT_ACTIVATED" &&
      ...
      reg.nextEligibleStep === "3B.3.27" &&
      reg.activationBlocker === "PHASE_3B3_26_CONTROLLED_WORKSPACE_HOST_ACTIVATION_READINESS_ONLY" &&
    ...
    const anyFail = invariants.some((i) => i.status !== "PASS") || !activationReadinessMetaOk;
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationReadiness: reg,
      hostActivationTransitionAuthorizationGrantIssuanceCommitBoundary: cb,
      activationReadinessMetaOk,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_27" : "READY_FOR_PHASE_3B_3_27",
    };

    const proofPath = join(outDir, "phase3b3-26-controlled-workspace-host-activation-readiness-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-26-controlled-workspace-host-activation-readiness-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.26 Controlled Workspace Host Activation Readiness Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Readiness: result=${reg.activationReadinessResult} ready=${reg.candidateReady} authorized=${reg.candidateAuthorized}`,
      `- Predecessor selection: state=${reg.predecessorCandidateSelectionState}`,
      ...
      `- activationReadinessMetaOk: ${activationReadinessMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-26-controlled-workspace-host-activation-readiness-summary.md"),
      summary,
    );
