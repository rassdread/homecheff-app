/**
 * Phase 3B.3.38 static validator — controlled workspace host activation transaction-commit.
 */
...
const PREDECESSOR_PHASE = "3B.3.37";
const PREDECESSOR_HEAD =
  "714efe6b815d336a30fae35f7c8fd8583f1fe533";
const PREDECESSOR_PROOF_TARGET =
  "8d7f26e628b9073eb02e930347fbef94ad12498f";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-transaction-commit-authorized-not-committed";
const PREDECESSOR_LIFECYCLE = "TRANSACTION_COMMIT_AUTHORIZED_NOT_COMMITTED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_38";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-commit.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-commit-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-commit-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-commit-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-transaction-commit-phase3b338.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-transaction-commit-proof-phase3b338.mjs",
);
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b338/phase3b3-38-controlled-workspace-host-activation-transaction-commit-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b338-controlled-workspace-host-activation-transaction-commit.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B338_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.38 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B338_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostActivationTransactionCommitDescriptor();
assert.equal(descriptor.phase, "3B.3.38");
assert.equal(
  descriptor.transactionCommitResult,
  "controlled-workspace-host-activation-transaction-committed-not-executed",
);
assert.equal(
  descriptor.transactionCommitState,
...
assert.equal(
  descriptor.predecessorActivationTransactionCommitAuthorizationState,
  PREDECESSOR_LIFECYCLE,
);
assert.equal(
  descriptor.predecessorActivationTransactionCommitAuthorizationResult,
  PREDECESSOR_RESULT,
);
...
assert.equal(descriptor.nextEligibleStep, "3B.3.39");
assert.equal(
...
rejectUnresolved("descriptor.transactionCommitResult", descriptor.transactionCommitResult);
...
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.38");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.39");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "issuance transaction committed early / duplicate record",
    input: { entryRecords: [{}, {}], issuanceTransactionCommitted: true },
  },
  {
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_39");
  assert.equal(proof.transactionCommittedMetaOk, true);
  ...
  validateFeedWorkspaceHostActivationTransactionCommitPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-activation-transaction-commit-phase3b338: PASS",
);
