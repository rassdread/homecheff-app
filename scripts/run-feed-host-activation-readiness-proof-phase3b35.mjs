#!/usr/bin/env node
/**
 * Phase 3B.3.5 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.5 host activation readiness Chromium proof (new)
 */
...
const port = process.env.PHASE3B35_PORT || "3026";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b32 = join(root, "docs/audits/artifacts/phase3b35");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b35] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b35] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b35] Phase 3B.3.5 host activation readiness proof");
    await run("node", [
      "scripts/probe-feed-host-activation-readiness-phase3b35.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b32}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b32, "phase3b3-5-feed-host-activation-readiness-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_6") {
      throw new Error(`Phase 3B.3.5 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b35] READY_FOR_PHASE_3B_3_6");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b32, "phase3b3-5-server-log-tail.txt"),
