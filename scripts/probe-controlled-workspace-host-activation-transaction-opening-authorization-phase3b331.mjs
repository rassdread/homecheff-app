#!/usr/bin/env node
/**
 * Phase 3B.3.31 — Chromium proof: Controlled Workspace Host Activation Transaction Opening Readiness.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3052";
  let commit = "unknown";
  let branch = "workspace/phase3b331-controlled-workspace-host-activation-transaction-opening-authorization";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b331");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_31") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 32;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionOpeningAuthorization = await api.readControlledWorkspaceHostActivationTransactionOpeningAuthorization();
      ...
      return {
        ...
        hostContract,
        hostRegistry,
        transactionOpeningAuthorization,
        commitBoundary,
        activationAttempt,
        hostPlan,
      };
    ...
    const reg = probe.transactionOpeningAuthorization;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionOpeningAuthorizationCount === 1,
      noDuplicateEntry: reg.duplicateTransactionOpeningAuthorizationCount === 0,
    ...
    const activationTransactionOpeningAuthorizationMetaOk =
      probe.version === 32 &&
      reg.phase === "3B.3.31" &&
      ...
      reg.transactionOpeningAuthorizationState === "TRANSACTION_OPENING_AUTHORIZED_NOT_OPENED" &&
      reg.transactionOpeningAuthorizationResult === "controlled-workspace-host-activation-transaction-opening-authorized-not-opened" &&
      reg.transactionOpeningAuthorizationCompleted === true &&
      ...
      reg.grantCount === 1 &&
      reg.transactionOpeningAuthorizationCount === 1 &&
      reg.duplicateTransactionOpeningAuthorizationCount === 0 &&
    ...
    const anyFail =
      ...
      !activationTransactionOpeningAuthorizationMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionOpeningAuthorization: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_32" : "READY_FOR_PHASE_3B_3_32",
    };

    const proofPath = join(outDir, "phase3b3-31-controlled-workspace-host-activation-transaction-opening-authorization-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-31-controlled-workspace-host-activation-transaction-opening-authorization-prepared.json"),
    ...
    const summary = [
      ...
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Commit-boundary entry: result=${reg.transactionOpeningAuthorizationResult} entered=${reg.activationCommitBoundaryEntered} state=${reg.activationCommitBoundaryState}`,
      ...
      `- activationTransactionOpeningAuthorizationMetaOk: ${activationTransactionOpeningAuthorizationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
