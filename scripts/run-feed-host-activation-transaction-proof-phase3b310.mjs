#!/usr/bin/env node
/**
 * Phase 3B.3.10 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.10 host activation transaction Chromium proof (new)
 */
...
const port = process.env.PHASE3B310_PORT || "3031";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b310 = join(root, "docs/audits/artifacts/phase3b310");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b310, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b310] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b310] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b310] Phase 3B.3.10 host activation transaction proof");
    await run("node", [
      "scripts/probe-feed-host-activation-transaction-phase3b310.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b310}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b310, "phase3b3-10-feed-host-activation-transaction-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_11") {
      throw new Error(`Phase 3B.3.10 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b310] READY_FOR_PHASE_3B_3_11");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b310, "phase3b3-10-server-log-tail.txt"),
