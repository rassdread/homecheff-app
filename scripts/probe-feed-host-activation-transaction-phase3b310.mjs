#!/usr/bin/env node
/**
 * Phase 3B.3.10 — Chromium proof: Controlled Host Activation Transaction.
 * New proof run (not a reuse of 3B.3.9). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3031";
  let commit = "unknown";
  let branch = "workspace/phase3b310-controlled-host-activation-transaction";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b310");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    pass(
      ...
      "legacy writer; host activation transaction; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const transactionMetaOk =
      ...
      hostActivationTransaction.beginState === "legacy-dormant-single-mount" &&
      hostActivationTransaction.intendedEndState ===
        "controlled-host-active-same-instance-no-remount" &&
      Array.isArray(hostActivationTransaction.commitConditions) &&
      hostActivationTransaction.commitConditions.length > 0 &&
      Array.isArray(hostActivationTransaction.rollbackConditions) &&
      hostActivationTransaction.rollbackConditions.length > 0 &&
      Array.isArray(hostActivationTransaction.validationCheckpoints) &&
      hostActivationTransaction.validationCheckpoints.length > 0 &&
      Array.isArray(hostActivationTransaction.transactionCheckpoints) &&
      hostActivationTransaction.transactionCheckpoints.length > 0 &&
      Array.isArray(hostActivationTransaction.compensatingActions) &&
      hostActivationTransaction.compensatingActions.length > 0 &&
      Array.isArray(hostActivationTransaction.abortConditions) &&
      hostActivationTransaction.abortConditions.length > 0 &&
      Array.isArray(hostActivationTransaction.invariants) &&
      hostActivationTransaction.invariants.length === 20 &&
      hostActivationTransaction.decisionResult === "ALLOW" &&
      hostActivationTransaction.planResult === "plan-complete-not-executable" &&
      hostActivationTransaction.pipelineResult ===
        "pipeline-complete-not-executable" &&
      hostActivationTransaction.wouldActivate === true &&
      hostActivationTransaction.canStartActivation === false &&
      hostActivationTransaction.hostActivation === false &&
      hostActivationTransaction.renderActivation === false &&
      hostActivationTransaction.activationBlocker ===
        "PHASE_3B3_10_HOST_ACTIVATION_TRANSACTION_ONLY" &&
      hostActivationTransaction.nextEligibleStep === "3B.3.11" &&
      hostActivationTransaction.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationTransaction.diagnostics?.transactionCompleted === true &&
      hostActivationTransaction.diagnostics?.transactionResult ===
        "transaction-complete-not-committed" &&
      hostActivationTransaction.diagnostics?.wouldCommit === true &&
      hostActivationTransaction.diagnostics?.transactionCommitted === false &&
      hostActivationTransaction.diagnostics?.activationBlocked === true &&
      hostActivationTransaction.diagnostics?.canStartActivation === false &&
      hostActivationTransaction.diagnostics?.currentPhase === "3B.3.10" &&
      hostActivationTransaction.diagnostics?.nextEligibleStep === "3B.3.11" &&
      shadowPlacement.placementState === "shadow-registered" &&
      shadowIdentity.expectedMountCount === 1 &&
      hostPlan.planState === "completed" &&
      hostPlan.transactionState === "completed" &&
      hostPlan.transactionResult === "transaction-complete-not-committed" &&
      hostPlan.wouldCommit === true &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !transactionMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_11"
          : "READY_FOR_PHASE_3B_3_11",
    };

    const outPath = join(
      outDir,
      "phase3b3-10-feed-host-activation-transaction-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldActivate: true,
      wouldCommit: hostActivationTransaction.wouldCommit,
      transactionCommitted: hostActivationTransaction.transactionCommitted,
      hostActivation: false,
      renderActivation: false,
      canStartActivation: false,
      writer: "legacy",
      ...
      browserProof: anyFail || !transactionMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-10-feed-host-activation-transaction-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-10-feed-host-activation-transaction-summary.md"),
      [
        `# Phase 3B.3.10 Host Activation Transaction Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Transaction: result=${hostActivationTransaction.transactionResult} wouldCommit=${hostActivationTransaction.wouldCommit} committed=${hostActivationTransaction.transactionCommitted}`,
        `- Diagnostics: phase=${hostActivationTransaction.diagnostics.currentPhase} next=${hostActivationTransaction.diagnostics.nextEligibleStep}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_10_HOST_ACTIVATION_TRANSACTION_ONLY); canStartActivation=false; transactionCommitted=false`,
    ...
    await browser.close();
    process.exit(anyFail || !transactionMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
