#!/usr/bin/env node
/**
 * Phase 3B.3.29 — Chromium proof: Controlled Workspace Host Activation Commit Boundary Entry.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3050";
  let commit = "unknown";
  let branch = "workspace/phase3b329-controlled-workspace-host-activation-commit-boundary-entry";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b329");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_29") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 30;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const commitBoundaryEntry = await api.readControlledWorkspaceHostActivationCommitBoundaryEntry();
    ...
    const reg = probe.commitBoundaryEntry;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.commitBoundaryEntryCount === 1,
      noDuplicateEntry: reg.duplicateCommitBoundaryEntryCount === 0,
    ...
    const activationCommitBoundaryEntryMetaOk =
      probe.version === 30 &&
      reg.phase === "3B.3.29" &&
      ...
      reg.activationCommitBoundaryEntryId === "feed.discovery.adaptive-workspace.host-activation-commit-boundary-entry.v1" &&
      reg.activationCommitBoundaryEntryContractId === "feed.discovery.adaptive-workspace.host-activation-commit-boundary-entry.contract.v1" &&
      reg.commitBoundaryEntryState === "COMMIT_BOUNDARY_ENTERED" &&
      reg.commitBoundaryEntryResult === "controlled-workspace-host-activation-commit-boundary-entered" &&
      reg.commitBoundaryEntryCompleted === true &&
      ...
      reg.grantCount === 1 &&
      reg.commitBoundaryEntryCount === 1 &&
      reg.duplicateCommitBoundaryEntryCount === 0 &&
      reg.candidateIdentityUnique === true &&
      reg.activationGrantIdentityUnique === true &&
      reg.activationCommitBoundaryIdentityUnique === true &&
      reg.activationCommitBoundaryEntryIdentityUnique === true &&
      ...
      reg.predecessorActivationGrantIssuanceState === "GRANTED_NOT_ACTIVATED" &&
      ...
      reg.nextEligibleStep === "3B.3.30" &&
      reg.activationBlocker === "PHASE_3B3_29_CONTROLLED_WORKSPACE_HOST_ACTIVATION_COMMIT_BOUNDARY_ENTRY_ONLY" &&
    ...
    const anyFail =
      ...
      !activationCommitBoundaryEntryMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationCommitBoundaryEntry: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_30" : "READY_FOR_PHASE_3B_3_30",
    };

    const proofPath = join(outDir, "phase3b3-29-controlled-workspace-host-activation-commit-boundary-entry-proof.json");
    ...
    const prepared = {
      ...
      activationCommitBoundaryEntryId: "feed.discovery.adaptive-workspace.host-activation-commit-boundary-entry.v1",
    ...
    writeFileSync(
      join(outDir, "phase3b3-29-controlled-workspace-host-activation-commit-boundary-entry-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.29 Controlled Workspace Host Activation Commit Boundary Entry Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Commit-boundary entry: result=${reg.commitBoundaryEntryResult} entered=${reg.activationCommitBoundaryEntered} state=${reg.activationCommitBoundaryState}`,
      `- Predecessor grant issuance: state=${reg.predecessorActivationGrantIssuanceState}`,
      ...
      `- activationCommitBoundaryEntryMetaOk: ${activationCommitBoundaryEntryMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-29-controlled-workspace-host-activation-commit-boundary-entry-summary.md"),
      summary,
    );
