/**
 * Phase 3B.3.42 static validator — controlled workspace host candidate activation readiness.
 */
...
const PREDECESSOR_PHASE = "3B.3.41";
const PREDECESSOR_BRANCH =
  "workspace/phase3b341-controlled-workspace-host-activation-issuance-pipeline-execution";
const PREDECESSOR_HEAD =
  "d599ec35d27e9a754c435ca879e2049f022ad21c";
const PREDECESSOR_PROOF_TARGET =
  "96cd5fe85b77862f540b2414d97946ee2273795a";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-issuance-pipeline-executed-not-activated";
const PREDECESSOR_LIFECYCLE = "PIPELINE_EXECUTED_NOT_ACTIVATED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_42";
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b342/phase3b3-42-controlled-workspace-host-candidate-activation-readiness-prepared.json",
);
const auditPath =
...
if (!artifactsPresent && process.env.REQUIRE_PHASE3B342_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.42 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B342_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostCandidateActivationReadinessDescriptor();
assert.equal(descriptor.phase, "3B.3.42");
assert.equal(
  descriptor.candidateActivationReadinessResult,
  "controlled-workspace-host-candidate-activation-ready-not-activated",
);
assert.equal(
  descriptor.candidateActivationReadinessState,
...
assert.equal(
  descriptor.activationCandidateActivationReadinessId,
  CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_ID,
);
assert.equal(
  descriptor.activationCandidateActivationReadinessContractId,
  CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_CONTRACT_ID,
);
...
assert.equal(descriptor.nextEligibleStep, "3B.3.43");
assert.equal(
...
rejectUnresolved("descriptor.candidateActivationReadinessResult", descriptor.candidateActivationReadinessResult);
...
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.42");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.43");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "pre-advanced candidateActivationReady",
    input: { candidateActivationReady: true },
  },
  {
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_43");
  assert.equal(proof.candidateActivationReadyMetaOk, true);
  ...
  validateFeedWorkspaceHostCandidateActivationReadinessPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-candidate-activation-readiness-phase3b342: PASS",
);
