#!/usr/bin/env node
/**
 * AW-R2 — Chromium proof: Controlled Workspace LIVE Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3070";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/aw-r2");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_AW_R2") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 50;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const controlledLiveAuthorization = await api.readControlledWorkspaceLiveAuthorization();
    ...
    const reg = probe.controlledLiveAuthorization;
    ...
    const forcedNegativeProofs = {
      ...
      activationExecutionAuthorizedButBlocked: reg.activationExecutionAllowed === true && probe.activationAttempt.allowed === false,
      candidateActivated: reg.candidateActivated === true,
      candidateActiveDeclared: reg.candidateActive === true,
    ...
    const controlledLiveAuthorizationMetaOk =
      probe.version >= 50 &&
      reg.phase === "AW-R2" &&
      ...
      reg.activationIssuancePipelineExecutionAuthorizationId === "feed.discovery.adaptive-workspace.host-activation-issuance-pipeline-execution-authorization.v1" &&
      reg.activationIssuancePipelineExecutionAuthorizationContractId === "feed.discovery.adaptive-workspace.host-activation-issuance-pipeline-execution-authorization.contract.v1" &&
      ...
      reg.activationCandidateActivationReadinessId === "feed.discovery.adaptive-workspace.host-candidate-activation-readiness.v1" &&
      reg.activationCandidateActivationReadinessContractId === "feed.discovery.adaptive-workspace.host-candidate-activation-readiness.contract.v1" &&
      reg.activationCandidateActivationAuthorizationId === "feed.discovery.adaptive-workspace.host-candidate-activation-authorization.v1" &&
      reg.activationCandidateActivationAuthorizationContractId === "feed.discovery.adaptive-workspace.host-candidate-activation-authorization.contract.v1" &&
      reg.activationCandidateActivationId === "feed.discovery.adaptive-workspace.host-candidate-activation.v1" &&
      reg.activationCandidateActivationContractId === "feed.discovery.adaptive-workspace.host-candidate-activation.contract.v1" &&
      reg.activationCandidateActiveId === "feed.discovery.adaptive-workspace.host-candidate-active.v1" &&
      reg.activationCandidateActiveContractId === "feed.discovery.adaptive-workspace.host-candidate-active.contract.v1" &&
      reg.activationCandidateExecutableId === "feed.discovery.adaptive-workspace.host-candidate-executable.v1" &&
      ...
      reg.activationLiveAuthorizationId === "feed.discovery.adaptive-workspace.host-live-authorization.v1" &&
      reg.activationLiveAuthorizationContractId === "feed.discovery.adaptive-workspace.host-live-authorization.contract.v1" &&
      reg.issuancePipelineExecutionReady === true &&
      reg.issuancePipelineExecutionAuthorized === true &&
      reg.issuancePipelineExecuted === true &&
      reg.candidateActivationReady === true &&
      reg.candidateActivationAuthorized === true &&
      ...
      reg.candidateActivationState === "LIVE_AUTHORIZED_NOT_EXECUTABLE" &&
      reg.candidateActivationResult === "controlled-workspace-live-authorized-not-executable" &&
      ...
      reg.activationExecutionAllowed === true &&
      reg.candidateActivated === true &&
      ...
      reg.nextEligibleStep === "AW-R3" &&
      reg.activationBlocker === "PHASE_AW_R2_CONTROLLED_LIVE_AUTHORIZATION_ONLY" &&
    ...
    const anyFail =
      ...
      !controlledLiveAuthorizationMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceLiveAuthorization: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_AW_R3" : "READY_FOR_AW_R3",
    };

    const proofPath = join(outDir, "aw-r2-controlled-workspace-live-authorization-proof.json");
    ...
    const prepared = {
      ...
      controlledLiveAuthorizationContract: "valid",
    ...
    writeFileSync(
      join(outDir, "aw-r2-controlled-workspace-live-authorization-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# AW-R2 Controlled Workspace LIVE Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.candidateActivationResult} state=${reg.candidateActivationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- controlledLiveAuthorizationMetaOk: ${controlledLiveAuthorizationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "aw-r2-controlled-workspace-live-authorization-summary.md"),
      summary,
    );
