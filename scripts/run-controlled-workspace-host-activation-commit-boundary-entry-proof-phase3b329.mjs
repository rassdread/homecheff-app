#!/usr/bin/env node
/**
 * Phase 3B.3.29 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.29 activation commit-boundary entry Chromium proof
 */
...
const port = process.env.PHASE3B329_PORT || "3050";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b329 = join(root, "docs/audits/artifacts/phase3b329");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b329, { recursive: true });
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_29") {
    throw new Error(`Phase 3B.3.28 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b329] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b329] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b329] Phase 3B.3.29 activation commit-boundary entry proof");
    await run("node", [
    ...
    const p329 = JSON.parse(
      readFileSync(
        join(
          out3b329,
          "phase3b3-29-controlled-workspace-host-activation-commit-boundary-entry-proof.json",
        ),
        "utf8",
      ),
    );
    if (p329.overallVerdict !== "READY_FOR_PHASE_3B_3_30") {
      throw new Error(`Phase 3B.3.29 proof failed: ${p329.overallVerdict}`);
    }
    if (p329.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p329.commit} HEAD=${commit}`);
    }
    console.log("[phase3b329] READY_FOR_PHASE_3B_3_30");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b329, "phase3b3-29-server-log-tail.txt"),
