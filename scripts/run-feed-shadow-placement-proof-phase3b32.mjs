...
const port = process.env.PHASE3B32_PORT || "3023";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b32 = join(root, "docs/audits/artifacts/phase3b32");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b32] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b32] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b32] Phase 3B.3.2 shadow placement proof");
    await run("node", [
      "scripts/probe-feed-shadow-placement-phase3b32.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b32}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b32, "phase3b3-2-feed-shadow-placement-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_3") {
      throw new Error(`Phase 3B.3.2 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b32] READY_FOR_PHASE_3B_3_3");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b32, "phase3b3-2-server-log-tail.txt"),
