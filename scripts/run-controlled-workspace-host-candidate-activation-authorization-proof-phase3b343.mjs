...
const port = process.env.PHASE3B343_PORT || "3064";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b333 = join(root, "docs/audits/artifacts/phase3b343");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b343] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b343] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_44") {
      throw new Error(`Phase 3B.3.43 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b343] READY_FOR_PHASE_3B_3_44");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
