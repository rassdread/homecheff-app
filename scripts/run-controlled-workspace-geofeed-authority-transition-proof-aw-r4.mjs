...
const port = process.env.AW_R4_PORT || "3072";
const outDir = join(root, "docs/audits/artifacts/aw-r4");
...
try {
  ...
  await run("node", [
    "scripts/probe-controlled-workspace-geofeed-authority-transition-aw-r4.mjs",
    `--base-url=${baseUrl}`,
    `--commit=${commit}`,
    "--branch=workspace/aw-r4-geofeed-authority-transition",
    `--out-dir=${outDir}`,
  ]);
  console.log("[aw-r4] READY_FOR_AW_R5");
} finally {
  server.kill("SIGTERM");
}
