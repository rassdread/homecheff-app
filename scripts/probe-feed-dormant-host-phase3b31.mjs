#!/usr/bin/env node
/**
 * Phase 3B.3.1 — Chromium proof that dormant controlled host has zero Feed impact.
 *
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3022";
  let commit = "unknown";
  let branch = "identity/phase2-auth-foundation";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b3");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 2;
      },
      { timeout: 120000 },
    );
    // Let late legacy IntersectionObservers settle before baseline.
    {
    ...
    const reqAtBaseline = baseline.counters.requestStartCount;
    const shadowReqDelta =
      afterShadow.counters.requestStartCount - reqAtBaseline;
    const forceReqDelta =
    ...
    pass(
      ...
      afterShadow.counters.intersectionObserverCreateCount ===
        baseline.counters.intersectionObserverCreateCount,
    ...
    pass(
      ...
      hostContract.activeWriter === "legacy" &&
        ...
        activationAttempt.blockers.includes("PHASE_3B3_1_DORMANT_HOST_ONLY"),
      "legacy writer; host activation blocked",
      JSON.stringify(activationAttempt),
    );
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "DORMANT_HOST_REGISTERED",
        "SHADOW_WITH_DORMANT_HOST",
      ...
      overallVerdict: anyFail
        ? "NOT_READY_FOR_PHASE_3B_3_2"
        : "READY_FOR_PHASE_3B_3_2",
    };

    const outPath = join(outDir, "phase3b3-1-feed-dormant-host-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-1-feed-dormant-host-readiness.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-1-feed-dormant-host-summary.md"),
      [
        `# Phase 3B.3.1 Dormant Host Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Host activation: false (blocked)`,
