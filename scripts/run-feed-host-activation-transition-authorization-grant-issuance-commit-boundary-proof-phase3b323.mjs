#!/usr/bin/env node
/**
 * Phase 3B.3.23 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.23 host activation transition authorization grant issuance
...
const port = process.env.PHASE3B323_PORT || "3044";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b321 = join(root, "docs/audits/artifacts/phase3b322");
const out3b322 = join(root, "docs/audits/artifacts/phase3b323");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b322, { recursive: true });
...
async function main() {
  ...
  if (priorIssuanceDecisionProof.overallVerdict !== "READY_FOR_PHASE_3B_3_23") {
    throw new Error(
      `Phase 3B.3.22 issuance decision proof required: ${priorIssuanceDecisionProof.overallVerdict}`,
    );
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b323] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b323] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log(
      "[phase3b323] Phase 3B.3.23 host activation transition authorization grant issuance plan proof",
    );
    await run("node", [
    ...
    if (p319.overallVerdict !== "READY_FOR_PHASE_3B_3_24") {
      throw new Error(`Phase 3B.3.23 proof failed: ${p319.overallVerdict}`);
    }
    ...
    console.log("[phase3b323] READY_FOR_PHASE_3B_3_24");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b322, "phase3b3-23-server-log-tail.txt"),
