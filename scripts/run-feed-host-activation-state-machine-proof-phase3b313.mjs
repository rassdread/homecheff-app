#!/usr/bin/env node
/**
 * Phase 3B.3.13 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.13 host activation state machine Chromium proof (new)
 */
...
const port = process.env.PHASE3B313_PORT || "3034";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b313 = join(root, "docs/audits/artifacts/phase3b313");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b313] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b313] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b313] Phase 3B.3.13 host activation state machine proof");
    await run("node", [
      "scripts/probe-feed-host-activation-state-machine-phase3b313.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b313}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b313, "phase3b3-13-feed-host-activation-state-machine-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_14") {
      throw new Error(`Phase 3B.3.13 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b313] READY_FOR_PHASE_3B_3_14");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
