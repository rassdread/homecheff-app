#!/usr/bin/env node
/**
 * Phase 3B.3.9 — Chromium proof: Controlled Host Activation Pipeline.
 * New proof run (not a reuse of 3B.3.8). Requires production server with
...
function parseArgs(argv) {
  ...
  let commit = "unknown";
  let branch = "workspace/phase3b39-controlled-host-activation-pipeline";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b39");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    pass(
      ...
      "legacy writer; host activation pipeline; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const pipelineMetaOk =
      ...
      hostActivationPipeline.decisionResult === "ALLOW" &&
      hostActivationPipeline.planResult === "plan-complete-not-executable" &&
      hostActivationPipeline.wouldActivate === true &&
      Array.isArray(hostActivationPipeline.pipelineStages) &&
      hostActivationPipeline.pipelineStages.length > 0 &&
      Array.isArray(hostActivationPipeline.stageOrder) &&
      hostActivationPipeline.stageOrder.length ===
        hostActivationPipeline.pipelineStages.length &&
      Array.isArray(hostActivationPipeline.stageDependencies) &&
      hostActivationPipeline.stageDependencies.length > 0 &&
      Array.isArray(hostActivationPipeline.validationPoints) &&
      Array.isArray(hostActivationPipeline.rollbackCheckpoints) &&
      Array.isArray(hostActivationPipeline.abortConditions) &&
      Array.isArray(hostActivationPipeline.invariants) &&
      hostActivationPipeline.invariants.length === 20 &&
      hostActivationPipeline.canStartActivation === false &&
      hostActivationPipeline.hostActivation === false &&
      hostActivationPipeline.renderActivation === false &&
      hostActivationPipeline.activationBlocker ===
        "PHASE_3B3_9_HOST_ACTIVATION_PIPELINE_ONLY" &&
      hostActivationPipeline.nextEligibleStep === "3B.3.10" &&
      hostActivationPipeline.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationPipeline.diagnostics?.pipelineCompleted === true &&
      hostActivationPipeline.diagnostics?.pipelineResult ===
        "pipeline-complete-not-executable" &&
      hostActivationPipeline.diagnostics?.activationBlocked === true &&
      hostActivationPipeline.diagnostics?.canStartActivation === false &&
      hostActivationPipeline.diagnostics?.currentPhase === "3B.3.9" &&
      hostActivationPipeline.diagnostics?.nextEligibleStep === "3B.3.10" &&
      shadowPlacement.placementState === "shadow-registered" &&
      shadowIdentity.expectedMountCount === 1 &&
      hostPlan.planState === "completed" &&
      hostPlan.pipelineState === "completed" &&
      hostPlan.pipelineResult === "pipeline-complete-not-executable" &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !pipelineMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_10"
          : "READY_FOR_PHASE_3B_3_10",
    };

    const outPath = join(
      outDir,
      "phase3b3-9-feed-host-activation-pipeline-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldActivate: true,
      stageCount: hostActivationPipeline.pipelineStages.length,
      hostActivation: false,
      renderActivation: false,
      canStartActivation: false,
      writer: "legacy",
      ...
      browserProof: anyFail || !pipelineMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-9-feed-host-activation-pipeline-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-9-feed-host-activation-pipeline-summary.md"),
      [
        `# Phase 3B.3.9 Host Activation Pipeline Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Pipeline: result=${hostActivationPipeline.pipelineResult} stages=${hostActivationPipeline.pipelineStages.length} wouldActivate=${hostActivationPipeline.wouldActivate}`,
        `- Diagnostics: phase=${hostActivationPipeline.diagnostics.currentPhase} stageCount=${hostActivationPipeline.diagnostics.stageCount}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_9_HOST_ACTIVATION_PIPELINE_ONLY); canStartActivation=false`,
    ...
    await browser.close();
    process.exit(anyFail || !pipelineMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
