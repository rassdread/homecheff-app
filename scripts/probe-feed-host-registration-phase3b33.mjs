#!/usr/bin/env node
/**
 * Phase 3B.3.3 — Chromium proof: Controlled Host Registration.
 * New proof run (not a reuse of 3B.3.2). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3024";
  let commit = "unknown";
  let branch = "workspace/phase3b33-controlled-host-registration";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b33");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 4;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host registration; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const registrationMetaOk =
      ...
      hostRegistration.activationRestriction ===
        "PHASE_3B3_3_HOST_REGISTRATION_ONLY" &&
      hostRegistration.nextEligibleStep === "3B.3.4" &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      scenarios: [
        "HOST_REGISTERED",
        "REGISTRY_METADATA_VISIBLE",
        "SHADOW_WITH_REGISTERED_HOST",
      ...
      overallVerdict:
        anyFail || !registrationMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_4"
          : "READY_FOR_PHASE_3B_3_4",
    };

    const outPath = join(outDir, "phase3b3-3-feed-host-registration-proof.json");
    ...
    const readiness = {
      ...
      browserProof: anyFail || !registrationMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-3-feed-host-registration-readiness.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-3-feed-host-registration-summary.md"),
      [
        `# Phase 3B.3.3 Host Registration Proof Summary`,
        ``,
        ...
        `- Host activation: false (blocked by PHASE_3B3_3_HOST_REGISTRATION_ONLY)`,
    ...
    await browser.close();
    process.exit(anyFail || !registrationMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
