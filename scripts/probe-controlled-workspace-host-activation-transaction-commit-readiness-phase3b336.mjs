#!/usr/bin/env node
/**
 * Phase 3B.3.35 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3057";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b336");
  for (const a of argv) {
...
async function main() {
  ...
  const priorPath = join(process.cwd(),
    "docs/audits/artifacts/phase3b335/phase3b3-34-controlled-workspace-host-activation-transaction-preparation-authorization-proof.json");
  const prior = JSON.parse(readFileSync(priorPath, "utf8"));
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_36") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionCommitReadiness = await api.readControlledWorkspaceHostActivationTransactionCommitReadiness();
    ...
    const reg = probe.transactionCommitReadiness;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionCommitReadinessCount === 1,
      noDuplicateEntry: reg.duplicateTransactionCommitReadinessCount === 0,
      ...
      transactionNotPrepared: reg.issuanceTransactionPrepared === true,
    ...
    const transactionCommitReadinessMetaOk =
      probe.version === 37 &&
      reg.phase === "3B.3.36" &&
      ...
      reg.transactionCommitReadinessState === "TRANSACTION_COMMIT_READY_NOT_COMMITTED" &&
      reg.transactionCommitReadinessResult === "controlled-workspace-host-activation-transaction-commit-ready-not-committed" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionCommitReadinessCount === 1 &&
      reg.duplicateTransactionCommitReadinessCount === 0 &&
      ...
      reg.nextEligibleStep === "3B.3.37" &&
      reg.activationBlocker === "PHASE_3B3_36_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_ONLY" &&
      reg.predecessorActivationTransactionPreparationState === "TRANSACTION_PREPARED_NOT_COMMITTED" &&
      reg.predecessorActivationTransactionPreparationResult === "controlled-workspace-host-activation-transaction-prepared-not-committed" &&
    ...
    const anyFail =
      ...
      !transactionCommitReadinessMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionCommitReadiness: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_37" : "READY_FOR_PHASE_3B_3_37",
    };

    const proofPath = join(outDir, "phase3b3-36-controlled-workspace-host-activation-transaction-commit-readiness-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.36",
      status: "controlled-workspace-host-activation-transaction-commit-readiness-prepared",
      transactionCommitReadinessContract: "valid",
      ...
      activationTransactionPreparationId: "feed.discovery.adaptive-workspace.host-activation-transaction-commit-readiness.v1",
      ...
      predecessorActivationTransactionPreparationReadinessResult: "controlled-workspace-host-activation-transaction-commit-readiness-ready-not-prepared",
    ...
    writeFileSync(
      join(outDir, "phase3b3-36-controlled-workspace-host-activation-transaction-commit-readiness-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.35 Controlled Workspace Host Activation Transaction Preparation Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionCommitReadinessResult} state=${reg.transactionCommitReadinessState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- transactionCommitReadinessMetaOk: ${transactionCommitReadinessMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-36-controlled-workspace-host-activation-transaction-commit-readiness-summary.md"),
      summary,
    );
