...
assert.equal(TERMS_EFFECTIVE_DATE, '2026-08-16');
...
const FakeDate = class extends RealDate {
  constructor(...args: never[]) {
    if (args.length === 0) {
      super('2026-08-17T12:00:00.000Z');
      return;
    }
    super(...(args as []));
  }
  static now() {
    return Date.parse('2026-08-17T12:00:00.000Z');
  }
} as unknown as DateConstructor;
// @ts-expect-error test double
globalThis.Date = FakeDate;
assert.equal(LEGAL_DOCUMENTS.terms.effectiveDate, '2026-08-16');
assert.equal(LEGAL_DOCUMENTS.privacy.effectiveDate, '2026-08-14');
assert.equal(formatLegalEffectiveDate('2026-08-16', 'nl'), '16 augustus 2026');
assert.equal(formatLegalEffectiveDate('2026-08-16', 'en'), '16 August 2026');
globalThis.Date = RealDate;

assert.equal(formatLegalEffectiveDate(TERMS_EFFECTIVE_DATE, 'nl'), '16 augustus 2026');
assert.equal(formatLegalEffectiveDate(TERMS_EFFECTIVE_DATE, 'en'), '16 August 2026');
