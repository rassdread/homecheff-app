#!/usr/bin/env node
/**
 * Phase 3B.3.41 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.41 issuance-pipeline-execution Chromium proof
 */
...
const port = process.env.PHASE3B341_PORT || "3062";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b333 = join(root, "docs/audits/artifacts/phase3b341");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_41") {
    throw new Error(`Phase 3B.3.40 predecessor proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b341] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b341] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b341] Phase 3B.3.41 issuance-pipeline-execution proof");
    await run("node", [
    ...
    const p333 = JSON.parse(
      readFileSync(
        join(
          out3b333,
          "phase3b3-41-controlled-workspace-host-activation-issuance-pipeline-execution-proof.json",
        ),
        "utf8",
      ),
    );
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_42") {
      throw new Error(`Phase 3B.3.41 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b341] READY_FOR_PHASE_3B_3_42");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
