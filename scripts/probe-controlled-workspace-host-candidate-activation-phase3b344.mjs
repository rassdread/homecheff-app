#!/usr/bin/env node
/**
 * Phase 3B.3.44 — Chromium proof: Controlled Workspace Host Candidate Activation Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3065";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b344");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const candidateActivationReadiness = await api.readControlledWorkspaceHostCandidateActivation();
    ...
    const candidateActivatedMetaOk =
      probe.version === 45 &&
      reg.phase === "3B.3.44" &&
      ...
      reg.candidateActivationState === "CANDIDATE_ACTIVATED_NOT_ACTIVE" &&
      reg.candidateActivationResult === "controlled-workspace-host-candidate-activated-not-active" &&
      ...
      reg.nextEligibleStep === "3B.3.45" &&
      reg.activationBlocker === "PHASE_3B3_44_CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_ONLY" &&
    ...
    const artifact = {
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_45" : "READY_FOR_PHASE_3B_3_45",
    };

    const proofPath = join(outDir, "phase3b3-44-controlled-workspace-host-candidate-activation-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.44",
      status: "controlled-workspace-host-candidate-activation-prepared",
    ...
    writeFileSync(
      join(outDir, "phase3b3-44-controlled-workspace-host-candidate-activation-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.44 Controlled Workspace Host Candidate Activation Authorization Proof Summary",
      "",
      ...
      `- candidateActivatedMetaOk: ${candidateActivatedMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-44-controlled-workspace-host-candidate-activation-summary.md"),
      summary,
    );
