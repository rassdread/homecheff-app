...
const port = process.env.PHASE3B345_PORT || "3066";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b345 = join(root, "docs/audits/artifacts/phase3b345");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b345, { recursive: true });
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_45") {
    throw new Error(`Phase 3B.3.44 predecessor proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b345] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b345] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b345] Phase 3B.3.45 candidate-active proof");
    await run("node", [
    ...
    const p333 = JSON.parse(
      readFileSync(
        join(
          out3b345,
          "phase3b3-45-controlled-workspace-host-candidate-active-proof.json",
        ),
        "utf8",
      ),
    );
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_46") {
      throw new Error(`Phase 3B.3.45 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b345] READY_FOR_PHASE_3B_3_46");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b345, "phase3b3-45-server-log-tail.txt"),
