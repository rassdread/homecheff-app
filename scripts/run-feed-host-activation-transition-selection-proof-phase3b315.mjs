#!/usr/bin/env node
/**
 * Phase 3B.3.15 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.15 host activation transition selection Chromium proof (new)
 */
...
const port = process.env.PHASE3B315_PORT || "3036";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b315 = join(root, "docs/audits/artifacts/phase3b315");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b315, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b315] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b315] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log(
      "[phase3b315] Phase 3B.3.15 host activation transition selection proof",
    );
    await run("node", [
      "scripts/probe-feed-host-activation-transition-selection-phase3b315.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b315}`,
    ...
    if (p315.overallVerdict !== "READY_FOR_PHASE_3B_3_16") {
      throw new Error(`Phase 3B.3.15 proof failed: ${p315.overallVerdict}`);
    }
    if (p315.commit !== commit) {
      throw new Error(
        `Proof commit mismatch: artifact=${p315.commit} HEAD=${commit}`,
      );
    }

    console.log("[phase3b315] READY_FOR_PHASE_3B_3_16");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b315, "phase3b3-15-server-log-tail.txt"),
