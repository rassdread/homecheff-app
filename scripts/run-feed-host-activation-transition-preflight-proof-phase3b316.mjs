#!/usr/bin/env node
/**
 * Phase 3B.3.16 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.16 host activation transition preflight Chromium proof (new)
 */
...
const port = process.env.PHASE3B316_PORT || "3037";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b316 = join(root, "docs/audits/artifacts/phase3b316");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b316, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b316] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b316] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    if (selectionProof.overallVerdict !== "READY_FOR_PHASE_3B_3_16") {
      throw new Error(
        `Phase 3B.3.15 selection proof required: ${selectionProof.overallVerdict}`,
      );
    }
    ...
    console.log(
      "[phase3b316] Phase 3B.3.16 host activation transition preflight proof",
    );
    await run("node", [
      "scripts/probe-feed-host-activation-transition-preflight-phase3b316.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b316}`,
    ...
    if (p316.overallVerdict !== "READY_FOR_PHASE_3B_3_17") {
      throw new Error(`Phase 3B.3.16 proof failed: ${p316.overallVerdict}`);
    }
    if (p316.commit !== commit) {
      throw new Error(
        `Proof commit mismatch: artifact=${p316.commit} HEAD=${commit}`,
      );
    }

    console.log("[phase3b316] READY_FOR_PHASE_3B_3_17");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b316, "phase3b3-16-server-log-tail.txt"),
