...
assert.equal(host.nextEligibleStep, "3B.3.8");
assert.ok(host.activationBlockers.includes(PHASE_3B3_3_HOST_REGISTRATION_ONLY));
assert.ok(host.activationBlockers.includes(PHASE_3B3_7_HOST_ACTIVATION_DECISION_ONLY));
...
assert.equal(
  ...
  "3B.3.8-controlled-host-activation-candidate",
);
...
assert.match(probeBridge, /version:\s*8/);
