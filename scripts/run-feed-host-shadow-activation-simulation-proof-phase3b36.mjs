#!/usr/bin/env node
/**
 * Phase 3B.3.6 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.6 host shadow activation simulation Chromium proof (new)
 */
...
const port = process.env.PHASE3B36_PORT || "3027";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b32 = join(root, "docs/audits/artifacts/phase3b36");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b36] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b36] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b36] Phase 3B.3.6 host shadow activation simulation proof");
    await run("node", [
      "scripts/probe-feed-host-shadow-activation-simulation-phase3b36.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b32}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b32, "phase3b3-6-feed-host-shadow-activation-simulation-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_7") {
      throw new Error(`Phase 3B.3.6 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b36] READY_FOR_PHASE_3B_3_7");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b32, "phase3b3-6-server-log-tail.txt"),
