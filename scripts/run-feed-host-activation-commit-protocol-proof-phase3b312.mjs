#!/usr/bin/env node
/**
 * Phase 3B.3.12 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.12 host activation commit protocol Chromium proof (new)
 */
...
const port = process.env.PHASE3B312_PORT || "3033";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b312 = join(root, "docs/audits/artifacts/phase3b312");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b312, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b312] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b312] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b312] Phase 3B.3.12 host activation commit protocol proof");
    await run("node", [
      "scripts/probe-feed-host-activation-commit-protocol-phase3b312.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b312}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b312, "phase3b3-12-feed-host-activation-commit-protocol-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_13") {
      throw new Error(`Phase 3B.3.12 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b312] READY_FOR_PHASE_3B_3_13");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b312, "phase3b3-12-server-log-tail.txt"),
