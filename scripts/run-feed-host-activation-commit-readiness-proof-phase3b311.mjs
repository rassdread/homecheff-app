#!/usr/bin/env node
/**
 * Phase 3B.3.11 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.11 host activation commit readiness Chromium proof (new)
 */
...
const port = process.env.PHASE3B311_PORT || "3032";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b311 = join(root, "docs/audits/artifacts/phase3b311");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b311, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b311] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b311] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b311] Phase 3B.3.11 host activation commit readiness proof");
    await run("node", [
      "scripts/probe-feed-host-activation-commit-readiness-phase3b311.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b311}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b311, "phase3b3-11-feed-host-activation-commit-readiness-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_12") {
      throw new Error(`Phase 3B.3.11 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b311] READY_FOR_PHASE_3B_3_12");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b311, "phase3b3-11-server-log-tail.txt"),
