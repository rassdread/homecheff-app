#!/usr/bin/env node
/**
 * Phase 3B.3.39 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  ...
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b340");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_40") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const pipelineExecutionAuthorization = await api.readControlledWorkspaceHostActivationIssuancePipelineExecutionAuthorization();
    ...
    const reg = probe.pipelineExecutionAuthorization;
    ...
    const forcedNegativeProofs = {
      ...
      pipelineExecutionAuthorizationted: reg.issuanceTransactionCommitted === true,
    ...
    const issuancePipelineExecutionAuthorizedMetaOk =
      probe.version === 41 &&
      reg.phase === "3B.3.40" &&
      ...
      reg.pipelineExecutionAuthorizationState === "PIPELINE_EXECUTION_AUTHORIZED_NOT_EXECUTED" &&
      reg.pipelineExecutionAuthorizationResult === "controlled-workspace-host-activation-issuance-pipeline-execution-authorized-not-executed" &&
    ...
    const anyFail =
      ...
      !issuancePipelineExecutionAuthorizedMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationIssuancePipelineExecutionAuthorization: reg,
    ...
    const proofPath = join(outDir, "phase3b3-40-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.40",
      status: "controlled-workspace-host-activation-issuance-pipeline-execution-authorization-prepared",
      pipelineExecutionAuthorizationContract: "valid",
      ...
      activationIssuancePipelineExecutionAuthorizationId: "feed.discovery.adaptive-workspace.host-activation-issuance-pipeline-execution-authorization.v1",
    ...
    writeFileSync(
      join(outDir, "phase3b3-40-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.40 Controlled Workspace Host Activation Transaction Commit Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.pipelineExecutionAuthorizationResult} state=${reg.pipelineExecutionAuthorizationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- issuancePipelineExecutionAuthorizedMetaOk: ${issuancePipelineExecutionAuthorizedMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-40-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-summary.md"),
      summary,
    );
