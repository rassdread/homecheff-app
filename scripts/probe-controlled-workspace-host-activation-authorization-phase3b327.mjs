#!/usr/bin/env node
/**
 * Phase 3B.3.27 — Chromium proof: Controlled Workspace Host Activation Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3048";
  let commit = "unknown";
  let branch = "workspace/phase3b327-controlled-workspace-host-activation-authorization";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b327");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_27") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 28;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const authorization = await api.readControlledWorkspaceHostActivationAuthorization();
    ...
    const forcedNegativeProofs = {
      ...
      candidateNotGranted: reg.candidateGranted === false,
      noGrantPresent: reg.grantPresent === false,
      futureGrantNotIssued: reg.futureGrantIssued === false,
    ...
    const activationAuthorizationMetaOk =
      probe.version === 28 &&
      reg.phase === "3B.3.27" &&
      ...
      reg.activationAuthorizationContractId === "feed.discovery.adaptive-workspace.host-activation-authorization.contract.v1" &&
      reg.activationAuthorizationState === "AUTHORIZED_NOT_GRANTED" &&
      reg.activationAuthorizationResult === "controlled-workspace-host-activation-authorized-not-granted" &&
      reg.activationAuthorizationCompleted === true &&
      reg.activationAuthorizationAuthorized === true &&
      reg.activationAuthorizationBlocked === true &&
      reg.activationAuthorizationExecutable === false &&
      reg.candidateSelected === true &&
      reg.candidateReady === true &&
      reg.candidateAuthorized === true &&
      reg.candidateGranted === false &&
      reg.grantPresent === false &&
      reg.futureGrantPossible === true &&
      reg.futureGrantIssued === false &&
      ...
      reg.predecessorActivationReadinessState === "READY_NOT_AUTHORIZED" &&
      ...
      reg.nextEligibleStep === "3B.3.28" &&
      reg.activationBlocker === "PHASE_3B3_27_CONTROLLED_WORKSPACE_HOST_ACTIVATION_AUTHORIZATION_ONLY" &&
    ...
    const anyFail = invariants.some((i) => i.status !== "PASS") || !activationAuthorizationMetaOk;
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationAuthorization: reg,
      hostActivationTransitionAuthorizationGrantIssuanceCommitBoundary: cb,
      activationAuthorizationMetaOk,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_28" : "READY_FOR_PHASE_3B_3_28",
    };

    const proofPath = join(outDir, "phase3b3-27-controlled-workspace-host-activation-authorization-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-27-controlled-workspace-host-activation-authorization-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.27 Controlled Workspace Host Activation Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Authorization: result=${reg.activationAuthorizationResult} authorized=${reg.candidateAuthorized} granted=${reg.candidateGranted}`,
      `- Predecessor readiness: state=${reg.predecessorActivationReadinessState}`,
      ...
      `- activationAuthorizationMetaOk: ${activationAuthorizationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-27-controlled-workspace-host-activation-authorization-summary.md"),
      summary,
    );
