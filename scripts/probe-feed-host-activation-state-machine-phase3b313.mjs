#!/usr/bin/env node
/**
 * Phase 3B.3.13 — Chromium proof: Controlled Host Activation State Machine.
 * New proof run (not a reuse of 3B.3.12). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3034";
  let commit = "unknown";
  let branch = "workspace/phase3b313-controlled-host-activation-state-machine";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b313");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    pass(
      ...
      "legacy writer; host activation state machine; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const stateMachineMetaOk =
      ...
      hostActivationStateMachine.initialState === "LEGACY_DORMANT" &&
      hostActivationStateMachine.transitionExecuted === false &&
      hostActivationStateMachine.protocolExecuted === false &&
      hostActivationStateMachine.transactionCommitted === false &&
      Array.isArray(hostActivationStateMachine.allowedTransitions) &&
      hostActivationStateMachine.allowedTransitions.length > 0 &&
      Array.isArray(hostActivationStateMachine.blockedTransitions) &&
      hostActivationStateMachine.blockedTransitions.includes(
        "COMMIT_READY->ACTIVE",
      ) &&
      Array.isArray(hostActivationStateMachine.transitionGuards) &&
      hostActivationStateMachine.transitionGuards.length > 0 &&
      Array.isArray(hostActivationStateMachine.transitionBlockers) &&
      Array.isArray(hostActivationStateMachine.transitionPreconditions) &&
      Array.isArray(hostActivationStateMachine.transitionValidationPoints) &&
      Array.isArray(hostActivationStateMachine.terminalStates) &&
      hostActivationStateMachine.terminalStates.includes("ACTIVE") &&
      hostActivationStateMachine.canStartActivation === false &&
      hostActivationStateMachine.hostActivation === false &&
      hostActivationStateMachine.activationBlocker ===
        "PHASE_3B3_13_HOST_ACTIVATION_STATE_MACHINE_ONLY" &&
      hostActivationStateMachine.nextEligibleStep === "3B.3.14" &&
      hostActivationStateMachine.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationStateMachine.diagnostics?.machineCompleted === true &&
      hostActivationStateMachine.diagnostics?.currentState === "COMMIT_READY" &&
      hostActivationStateMachine.diagnostics?.transitionExecuted === false &&
      hostActivationStateMachine.diagnostics?.activeUnreachable === true &&
      hostActivationStateMachine.diagnostics?.currentPhase === "3B.3.13" &&
      hostActivationStateMachine.diagnostics?.nextEligibleStep === "3B.3.14" &&
      shadowPlacement.placementState === "shadow-registered" &&
      shadowIdentity.expectedMountCount === 1 &&
      hostPlan.planState === "completed" &&
      hostPlan.stateMachineState === "completed" &&
      hostPlan.currentActivationLifecycleState === "COMMIT_READY" &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !stateMachineMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_14"
          : "READY_FOR_PHASE_3B_3_14",
    };

    const outPath = join(
      outDir,
      "phase3b3-13-feed-host-activation-state-machine-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldCommit: true,
      commitReady: true,
      allowedTransitionCount: hostActivationStateMachine.allowedTransitions.length,
      blockedTransitionCount: hostActivationStateMachine.blockedTransitions.length,
      ...
      browserProof: anyFail || !stateMachineMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-13-feed-host-activation-state-machine-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-13-feed-host-activation-state-machine-summary.md"),
      [
        `# Phase 3B.3.13 Host Activation State Machine Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- State machine: result=${hostActivationStateMachine.machineResult} current=${hostActivationStateMachine.currentState} transitionExecuted=${hostActivationStateMachine.transitionExecuted}`,
        `- Diagnostics: phase=${hostActivationStateMachine.diagnostics.currentPhase} next=${hostActivationStateMachine.diagnostics.nextEligibleStep}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_13_HOST_ACTIVATION_STATE_MACHINE_ONLY); canStartActivation=false; transitionExecuted=false`,
    ...
    await browser.close();
    process.exit(anyFail || !stateMachineMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
