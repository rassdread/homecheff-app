#!/usr/bin/env node
/**
 * Phase 3B.3.4 — Chromium proof: Controlled Host Eligibility.
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3025";
  let commit = "unknown";
  let branch = "workspace/phase3b34-controlled-host-eligibility";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b34");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 5;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host eligibility; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const eligibilityMetaOk =
      ...
      hostRegistration.activationRestriction ===
        "PHASE_3B3_3_HOST_REGISTRATION_ONLY" &&
      hostEligibility.eligibilityState === "eligible" &&
      hostEligibility.canStartActivation === false &&
      hostEligibility.hostActivation === false &&
      hostEligibility.renderActivation === false &&
      hostEligibility.activationBlocker ===
        "PHASE_3B3_4_HOST_ELIGIBILITY_ONLY" &&
      hostEligibility.nextEligibleStep === "3B.3.5" &&
      hostEligibility.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostEligibility.diagnostics?.activationBlocked === true &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !eligibilityMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_5"
          : "READY_FOR_PHASE_3B_3_5",
    };

    const outPath = join(outDir, "phase3b3-4-feed-host-eligibility-proof.json");
    ...
    const readiness = {
      ...
      browserProof: anyFail || !eligibilityMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-4-feed-host-eligibility-readiness.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-4-feed-host-eligibility-summary.md"),
      [
        `# Phase 3B.3.4 Host Eligibility Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Eligibility: state=${hostEligibility.eligibilityState} reason=${hostEligibility.eligibilityReason}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId} state=${hostRegistry.registrationState}`,
        `- Host activation: false (blocked by PHASE_3B3_4_HOST_ELIGIBILITY_ONLY)`,
    ...
    await browser.close();
    process.exit(anyFail || !eligibilityMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
