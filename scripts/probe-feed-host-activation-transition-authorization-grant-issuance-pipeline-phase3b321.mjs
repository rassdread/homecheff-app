#!/usr/bin/env node
/**
 * Phase 3B.3.21 — Chromium proof: Controlled Host Activation Transition
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3042";
  let commit = "unknown";
  let branch = "workspace/phase3b321-controlled-host-activation-transition-authorization-grant-issuance-pipeline";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b321");
  for (const a of argv) {
...
async function main() {
  ...
  if (!existsSync(priorIssuanceDecisionProofPath)) {
    throw new Error("Phase 3B.3.20 issuance decision proof required");
  }
  const priorIssuanceDecisionProof = JSON.parse(
    readFileSync(priorIssuanceDecisionProofPath, "utf8"),
  );
  if (priorIssuanceDecisionProof.overallVerdict !== "READY_FOR_PHASE_3B_3_21") {
    throw new Error(
      `Phase 3B.3.20 proof not ready: ${priorIssuanceDecisionProof.overallVerdict}`,
    );
  }
  ...
  try {
    ...
    const hostActivationTransitionAuthorizationGrantIssuancePipeline =
      await page.evaluate(async () => {
        return window.__HC_FEED_SEALED_PROBE__.readHostActivationTransitionAuthorizationGrantIssuancePipeline();
      });
    ...
    const issuancePipeline =
      hostActivationTransitionAuthorizationGrantIssuancePipeline;
    const diag = issuancePipeline.diagnostics ?? {};

    // Forced negative proofs: every execution/grant/materialization/authority
    // surface that the issuance-pipeline evaluation could theoretically
    ...
    // lib/adaptive-workspace/tests/run-host-activation-transition-authorization-grant-issuance-pipeline-tests.ts.
    const forcedNegativeProofs = {
      grantCreationBlocked:
        issuancePipeline.grantCreated === false &&
        issuancePipeline.grantCreationAllowed === false,
      grantIssuanceBlocked:
        issuancePipeline.grantIssued === false &&
        issuancePipeline.grantIssuanceAllowed === false,
      grantMaterializationBlocked:
        issuancePipeline.grantMaterialized === false &&
        issuancePipeline.grantMaterializationAllowed === false,
      grantPersistenceBlocked:
        issuancePipeline.grantPersisted === false &&
        issuancePipeline.grantPersistenceAllowed === false,
      grantApplicationBlocked:
        issuancePipeline.grantApplied === false &&
        issuancePipeline.grantApplicationAllowed === false,
      grantActivationBlocked:
        issuancePipeline.grantActivated === false &&
        issuancePipeline.grantActivationAllowed === false,
      grantConsumptionBlocked:
        issuancePipeline.grantConsumed === false &&
        issuancePipeline.grantConsumptionAllowed === false,
      grantRevocationBlocked:
        issuancePipeline.grantRevoked === false &&
        issuancePipeline.grantRevocationAllowed === false,
      grantAuthorityBlocked:
        issuancePipeline.grantAuthorityAvailable === false &&
        issuancePipeline.grantAuthorityEnabled === false,
      authorityDelegationBlocked:
        issuancePipeline.grantAuthorityDelegated === false &&
        issuancePipeline.grantAuthorityTransferred === false,
      issuancePipelineExecutionBlocked:
        issuancePipeline.issuancePipelineExecuted === false,
      authorizationGrantBlocked:
        issuancePipeline.authorizationGranted === false &&
        issuancePipeline.authorizationGrantAllowed === false,
      authorizationApplicationBlocked:
        issuancePipeline.authorizationApplied === false &&
        issuancePipeline.authorizationApplicationAllowed === false,
      transitionAuthorizationBlocked:
        issuancePipeline.transitionAuthorized === false &&
        issuancePipeline.transitionAuthorizationAllowed === false,
      transitionExecutionBlocked:
        issuancePipeline.transitionExecuted === false &&
        issuancePipeline.transitionExecutionAllowed === false,
      preflightExecutionBlocked:
        issuancePipeline.preflightExecuted === false &&
        issuancePipeline.preflightExecutionAllowed === false,
      selectionExecutionBlocked:
        issuancePipeline.selectionExecuted === false &&
        issuancePipeline.selectionExecutionAllowed === false,
      graphTraversalBlocked:
          issuancePipeline.graphTraversalAllowed === false,
      activationBlocked:
        issuancePipeline.hostActivation === false &&
        issuancePipeline.renderActivation === false &&
        issuancePipeline.canStartActivation === false,
      commitBlocked:
        issuancePipeline.transactionCommitted === false &&
        issuancePipeline.protocolExecuted === false,
      ...
      currentStateUnmutated: issuancePipeline.currentState === "COMMIT_READY",
      currentNodeUnmutated: issuancePipeline.currentNode === "COMMIT_READY",
      selectedTransitionUnmutated:
        issuancePipeline.selectedTransition === "COMMIT_READY->ACTIVE",
      tokenAbsent: issuancePipeline.tokenPresent === false,
      secretAbsent: issuancePipeline.secretPresent === false,
      signatureAbsent: issuancePipeline.signaturePresent === false,
      nonceAbsent: issuancePipeline.noncePresent === false,
      credentialAbsent: issuancePipeline.credentialPresent === false,
      certificateAbsent: issuancePipeline.certificatePresent === false,
      permitAbsent: issuancePipeline.permitPresent === false,
      callbackAbsent: issuancePipeline.callbackPresent === false,
      executableHandleAbsent:
        issuancePipeline.executableHandlePresent === false,
      runtimeCapabilityAbsent:
        issuancePipeline.runtimeCapabilityPresent === false,
    };
    const anyForcedNegativeFail = Object.values(forcedNegativeProofs).some(
      (v) => v !== true,
    );

    const issuancePipelineMetaOk =
      ...
      // Issuance decision (new for 3B.3.21)
      issuancePipeline.issuancePipelineState === "completed" &&
      issuancePipeline.issuancePipelineResult ===
        "authorization-grant-issuance-pipeline-ready-not-executable" &&
      issuancePipeline.issuancePipelineCompleted === true &&
      issuancePipeline.issuancePipelineExecuted === false &&
      issuancePipeline.issuancePipelineReady === true &&
      issuancePipeline.issuancePipelineBlocked === true &&
      issuancePipeline.issuancePipelineExecutable === false &&
      issuancePipeline.planCoverageExact === true &&
      issuancePipeline.sourcePlanStepCount === 30 &&
      issuancePipeline.coveredPlanStepCount === 30 &&
      issuancePipeline.pipelineDependencyGraphAcyclic === true &&
      issuancePipeline.wouldExecuteIssuancePipeline === true &&
      issuancePipeline.pipelineStageCount === 30 &&
      issuancePipeline.blockedPipelineStageCount === 30 &&
      issuancePipeline.executablePipelineStageCount === 0 &&
      issuancePipeline.issuanceEligible === true &&
      issuancePipeline.issuanceBlocked === true &&
      issuancePipeline.wouldIssueGrant === true &&
      issuancePipeline.grantIssued === false &&
      issuancePipeline.grantCreated === false &&
      issuancePipeline.grantMaterialized === false &&
      issuancePipeline.grantPersisted === false &&
      ...
      issuancePipeline.grantAuthorityDelegated === false &&
      issuancePipeline.grantAuthorityTransferred === false &&
      issuancePipeline.tokenPresent === false &&
      issuancePipeline.secretPresent === false &&
      issuancePipeline.signaturePresent === false &&
      issuancePipeline.noncePresent === false &&
      issuancePipeline.credentialPresent === false &&
      issuancePipeline.certificatePresent === false &&
      issuancePipeline.permitPresent === false &&
      issuancePipeline.callbackPresent === false &&
      issuancePipeline.executableHandlePresent === false &&
      issuancePipeline.runtimeCapabilityPresent === false &&
      issuancePipeline.grantReadinessResult ===
        "authorization-grant-ready-not-issued" &&
      issuancePipeline.grantReady === true &&
      issuancePipeline.authorizationDecisionResult ===
        "authorization-eligible-not-granted" &&
      issuancePipeline.authorizationEligible === true &&
      issuancePipeline.authorizationGranted === false &&
      issuancePipeline.transitionAuthorized === false &&
      issuancePipeline.currentState === "COMMIT_READY" &&
      issuancePipeline.currentNode === "COMMIT_READY" &&
      issuancePipeline.selectedTransition === "COMMIT_READY->ACTIVE" &&
      issuancePipeline.transitionExecuted === false &&
      issuancePipeline.protocolExecuted === false &&
      issuancePipeline.transactionCommitted === false &&
      ...
      issuancePipeline.hostActivation === false &&
      issuancePipeline.renderActivation === false &&
      issuancePipeline.activationBlocker ===
        "PHASE_3B3_21_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_GRANT_ISSUANCE_PIPELINE_ONLY" &&
      issuancePipeline.nextEligibleStep === "3B.3.22" &&
      issuancePipeline.runtimeId === "feed.discovery.legacy-single-mount.v1" &&
      diag.issuancePipelineCompleted === true &&
      diag.issuancePipelineReady === true &&
      diag.issuancePipelineBlocked === true &&
      diag.issuancePipelineExecutable === false &&
      diag.wouldExecuteIssuancePipeline === true &&
      ...
      diag.currentPhase === "3B.3.21" &&
      diag.nextEligibleStep === "3B.3.22" &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_ACTIVATION_TRANSITION_AUTHORIZATION_GRANT_ISSUANCE_PIPELINE",
        "AUTHORIZATION_GRANT_ISSUANCE_ELIGIBLE_NOT_ISSUED",
        "ISSUANCE_PIPELINE_METADATA_VISIBLE",
        "ISSUANCE_PIPELINE_DIAGNOSTICS_VISIBLE",
      ...
      overallVerdict:
        anyFail || !issuancePipelineMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_22"
    ...
    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.21",
      status: "host-activation-transition-authorization-grant-issuance-pipeline-prepared",
      issuancePipelineContract: "valid",
      ...
      browserProof: anyFail || !issuancePipelineMetaOk ? "fail" : "pass",
      ...
      issuancePipelineExecutionAuthorized: false,
    ...
    writeFileSync(
      ...
      [
        ...
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Issuance decision: result=${issuancePipeline.issuancePipelineResult} eligible=${issuancePipeline.issuanceEligible} issued=${issuancePipeline.grantIssued} wouldIssueGrant=${issuancePipeline.wouldIssueGrant}`,
        `- Grant readiness linkage: result=${grantReadiness.grantReadinessResult} ready=${grantReadiness.grantReady}`,
        `- Selected transition: ${issuancePipeline.selectedTransition} (current=${issuancePipeline.currentState}/${issuancePipeline.currentNode})`,
    ...
    console.log(
      JSON.stringify(
        {
          ok: !anyFail && issuancePipelineMetaOk,
          outPath,
          verdict: artifact.overallVerdict,
    ...
    await browser.close();
    process.exit(anyFail || !issuancePipelineMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
