#!/usr/bin/env node
/**
 * Phase 3B.3.36 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.36 transaction-commit-readiness Chromium proof
 */
...
const port = process.env.PHASE3B336_PORT || "3057";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b333 = join(root, "docs/audits/artifacts/phase3b336");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_36") {
    throw new Error(`Phase 3B.3.36 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b336] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b336] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b336] Phase 3B.3.36 transaction-commit-readiness proof");
    await run("node", [
    ...
    const p333 = JSON.parse(
      readFileSync(
        join(
          out3b333,
          "phase3b3-36-controlled-workspace-host-activation-transaction-commit-readiness-proof.json",
        ),
        "utf8",
      ),
    );
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_37") {
      throw new Error(`Phase 3B.3.36 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b336] READY_FOR_PHASE_3B_3_37");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
