#!/usr/bin/env node
/**
 * WX Phase 1B.5.3 — Assist Surface Eligibility browser proof.
 * Asserts eligibility diagnostics present, zero visible assist DOM delta, no remount.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3114";
  let outDir = join(process.cwd(), "docs/audits/wx-phase1b5-3-assist-surface-eligibility");
  let journey = false;
  for (const a of argv) {
...
async function snap(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      focusableDiagnostics: [...document.querySelectorAll("[data-wx-assist-eligibility]")].some(
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    const checks = {
      phase: s.phase === "1b.5.3",
      assistEligibility: s.assistEligibility === "wx-assist-surface-eligibility-v1",
      assistEligibilityVersion: s.assistEligibilityVersion === "1.0.0",
      ...
      hasToken: Boolean(s.assistEligibilityToken),
      hasOrderedAssistIds:
        typeof s.assistIds === "string" && s.assistIds.includes("assist-primary"),
      ...
      pass:
        ...
        steps.every(
          (s) =>
            ...
            s.snap.drivesChrome === "0" &&
            s.snap.rendersAssist === "0" &&
            s.snap.phase === "1b.5.3",
        ),
    };
    await page.close();
  ...
  const report = {
    ...
    verdict: allPass
      ? "WX_PHASE_1B5_3_BROWSER_PROOF_PASS"
      : "WX_PHASE_1B5_3_BROWSER_PROOF_FAIL",
    ...
    JSON.stringify(
      {
        ...
        visibleAssistDomDeltaZero: report.visibleAssistDomDeltaZero,
