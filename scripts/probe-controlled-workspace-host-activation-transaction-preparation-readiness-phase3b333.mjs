#!/usr/bin/env node
/**
 * Phase 3B.3.33 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3054";
  let commit = "unknown";
  let branch = "workspace/phase3b333-controlled-workspace-host-activation-transaction-preparation-readiness";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b333");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_33") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 34;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionPreparationReadiness = await api.readControlledWorkspaceHostActivationTransactionPreparationReadiness();
      ...
      return {
        ...
        hostContract,
        hostRegistry,
        transactionPreparationReadiness,
        commitBoundary,
        activationAttempt,
        hostPlan,
      };
    ...
    const reg = probe.transactionPreparationReadiness;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionPreparationReadinessCount === 1,
      noDuplicateEntry: reg.duplicateTransactionPreparationReadinessCount === 0,
      ...
      transactionNotPrepared: reg.issuanceTransactionPrepared === false,
      transactionNotCommitted: reg.issuanceTransactionCommitted === false,
    ...
    const transactionPreparationReadinessMetaOk =
      probe.version === 34 &&
      reg.phase === "3B.3.33" &&
      ...
      reg.transactionPreparationReadinessState === "TRANSACTION_PREPARATION_READY_NOT_PREPARED" &&
      reg.transactionPreparationReadinessResult === "controlled-workspace-host-activation-transaction-preparation-ready-not-prepared" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionPreparationReadinessCount === 1 &&
      reg.duplicateTransactionPreparationReadinessCount === 0 &&
      ...
      reg.transactionPreparationAuthorized === false &&
      reg.issuanceTransactionState === "OPENED" &&
      reg.issuanceTransactionOpened === true &&
      reg.issuanceTransactionPrepared === false &&
      reg.nextEligibleStep === "3B.3.34" &&
      reg.activationBlocker === "PHASE_3B3_33_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_ONLY" &&
      reg.predecessorActivationTransactionOpeningState === "TRANSACTION_OPENED_NOT_PREPARED" &&
      reg.predecessorActivationTransactionOpeningResult === "controlled-workspace-host-activation-transaction-opened-not-prepared" &&
      reg.predecessorActivationCommitBoundaryEntryState === "COMMIT_BOUNDARY_ENTERED" &&
    ...
    const anyFail =
      ...
      !transactionPreparationReadinessMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionPreparationReadiness: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_34" : "READY_FOR_PHASE_3B_3_34",
    };

    const proofPath = join(outDir, "phase3b3-33-controlled-workspace-host-activation-transaction-preparation-readiness-proof.json");
    ...
    const prepared = {
      ...
      activationTransactionPreparationReadinessAuthorizationId: "feed.discovery.adaptive-workspace.host-activation-transaction-preparation-readiness-authorization.v1",
      ...
      predecessorActivationTransactionPreparationReadinessAuthorizationState: "TRANSACTION_OPENING_AUTHORIZED_NOT_OPENED",
      predecessorActivationTransactionPreparationReadinessAuthorizationResult: "controlled-workspace-host-activation-transaction-preparation-readiness-authorized-not-opened",
    ...
    writeFileSync(
      join(outDir, "phase3b3-33-controlled-workspace-host-activation-transaction-preparation-readiness-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.33 Controlled Workspace Host Activation Transaction Opening Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionPreparationReadinessResult} state=${reg.transactionPreparationReadinessState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- Predecessor authorization: state=${reg.predecessorActivationTransactionPreparationReadinessAuthorizationState}`,
      ...
      `- transactionPreparationReadinessMetaOk: ${transactionPreparationReadinessMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-33-controlled-workspace-host-activation-transaction-preparation-readiness-summary.md"),
      summary,
    );
