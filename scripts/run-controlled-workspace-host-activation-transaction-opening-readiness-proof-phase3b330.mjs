#!/usr/bin/env node
/**
 * Phase 3B.3.30 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.30 activation commit-boundary entry Chromium proof
 */
...
const port = process.env.PHASE3B330_PORT || "3051";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b330 = join(root, "docs/audits/artifacts/phase3b330");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_30") {
    throw new Error(`Phase 3B.3.29 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b330] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b330] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b330] Phase 3B.3.30 transaction-opening readiness proof");
    await run("node", [
    ...
    if (p330.overallVerdict !== "READY_FOR_PHASE_3B_3_31") {
      throw new Error(`Phase 3B.3.30 proof failed: ${p330.overallVerdict}`);
    }
    if (p330.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p330.commit} HEAD=${commit}`);
    }
    console.log("[phase3b330] READY_FOR_PHASE_3B_3_31");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
