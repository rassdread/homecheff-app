#!/usr/bin/env node
/**
 * Phase 3B.3.18 — Chromium proof: Controlled Host Activation Transition
 * Authorization Grant Readiness. New proof run (not a reuse of 3B.3.17).
 * Requires production server with NEXT_PUBLIC_FEED_SEALED_BASELINE=1.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3039";
  let commit = "unknown";
  let branch = "workspace/phase3b318-controlled-host-activation-transition-authorization-grant-readiness";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b318");
  for (const a of argv) {
...
async function main() {
  ...
  if (!existsSync(priorAuthProofPath)) {
    throw new Error("Phase 3B.3.17 authorization decision proof required");
  }
  ...
  if (priorAuthProof.overallVerdict !== "READY_FOR_PHASE_3B_3_18") {
    throw new Error(
      `Phase 3B.3.17 proof not ready: ${priorAuthProof.overallVerdict}`,
    );
  }
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 19;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host activation transition authorization grant readiness; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const diag = grantReadiness.diagnostics ?? {};

    // Forced negative proofs: every execution/grant/mutation surface that the
    // grant readiness evaluation could theoretically unlock must remain
    // false. The browser probe API is intentionally read-only (no
    // execute/grant/issue/apply functions are exposed on window), so these
    // are asserted directly from the returned descriptor/diagnostics; deeper
    // fail-closed validation (throwing on forced-true candidates) is
    // unit-test-covered in
    // lib/adaptive-workspace/tests/run-host-activation-transition-authorization-grant-readiness-tests.ts.
    const forcedNegativeProofs = {
      grantCreationBlocked:
        grantReadiness.grantCreated === false &&
        grantReadiness.grantCreationAllowed === false,
      grantIssuanceBlocked:
        grantReadiness.grantIssued === false &&
        grantReadiness.grantIssuanceAllowed === false,
      grantPersistenceBlocked:
        grantReadiness.grantPersisted === false &&
        grantReadiness.grantPersistenceAllowed === false,
      grantApplicationBlocked:
        grantReadiness.grantApplied === false &&
        grantReadiness.grantApplicationAllowed === false,
      grantAuthorityBlocked:
        grantReadiness.grantAuthorityAvailable === false &&
        grantReadiness.grantAuthorityEnabled === false,
      grantExecutionBlocked:
        grantReadiness.grantReadinessExecuted === false &&
        grantReadiness.grantExecutionAllowed === false,
      ...
      transitionAuthorizationBlocked:
        grantReadiness.transitionAuthorized === false &&
        grantReadiness.transitionAuthorizationAllowed === false,
      transitionExecutionBlocked:
        grantReadiness.transitionExecuted === false &&
        grantReadiness.transitionExecutionAllowed === false,
      preflightExecutionBlocked:
        grantReadiness.preflightExecuted === false &&
        grantReadiness.preflightExecutionAllowed === false,
      selectionExecutionBlocked:
        grantReadiness.selectionExecuted === false &&
        grantReadiness.selectionExecutionAllowed === false,
      graphTraversalBlocked:
        grantReadiness.graphTraversalExecuted === false &&
        grantReadiness.graphTraversalAllowed === false,
      activationBlocked:
        grantReadiness.hostActivation === false &&
        grantReadiness.renderActivation === false &&
        grantReadiness.canStartActivation === false,
      commitBlocked:
        grantReadiness.transactionCommitted === false &&
        grantReadiness.protocolExecuted === false,
      ...
      currentStateUnmutated: grantReadiness.currentState === "COMMIT_READY",
      currentNodeUnmutated: grantReadiness.currentNode === "COMMIT_READY",
      selectedTransitionUnmutated:
        grantReadiness.selectedTransition === "COMMIT_READY->ACTIVE",
      grantTokenAbsent: grantReadiness.grantTokenPresent === false,
      grantSecretAbsent: grantReadiness.grantSecretPresent === false,
      grantSignatureAbsent: grantReadiness.grantSignaturePresent === false,
      grantCallbackAbsent: grantReadiness.grantCallbackPresent === false,
    };
    ...
    const grantMetaOk =
      ...
      // Authorization decision linkage (3B.3.17)
      ...
      // Grant readiness (new for 3B.3.18)
      ...
      grantReadiness.grantAuthorityAvailable === false &&
      grantReadiness.grantAuthorityEnabled === false &&
      grantReadiness.grantTokenPresent === false &&
      grantReadiness.grantSecretPresent === false &&
      grantReadiness.grantSignaturePresent === false &&
      grantReadiness.grantCallbackPresent === false &&
      grantReadiness.authorizationDecisionResult ===
        "authorization-eligible-not-granted" &&
      grantReadiness.authorizationEligible === true &&
      grantReadiness.authorizationGranted === false &&
      grantReadiness.transitionAuthorized === false &&
      grantReadiness.currentState === "COMMIT_READY" &&
      grantReadiness.currentNode === "COMMIT_READY" &&
      grantReadiness.selectedTransition === "COMMIT_READY->ACTIVE" &&
      grantReadiness.selectionExecutionAllowed === false &&
      grantReadiness.preflightExecutionAllowed === false &&
      grantReadiness.authorizationGrantAllowed === false &&
      grantReadiness.authorizationApplicationAllowed === false &&
      grantReadiness.transitionAuthorizationAllowed === false &&
      grantReadiness.transitionExecuted === false &&
      grantReadiness.graphTraversalExecuted === false &&
      grantReadiness.protocolExecuted === false &&
      ...
      grantReadiness.runtimeId === "feed.discovery.legacy-single-mount.v1" &&
      diag.grantReadinessCompleted === true &&
      diag.grantReady === true &&
      diag.grantBlocked === true &&
      ...
      diag.currentPhase === "3B.3.18" &&
      diag.nextEligibleStep === "3B.3.19" &&
      ...
      hostPlan.grantReadinessState === "completed" &&
      hostPlan.grantReadinessResult ===
        "authorization-grant-ready-not-issued" &&
      hostPlan.grantReadinessCompleted === true &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_ACTIVATION_TRANSITION_AUTHORIZATION_GRANT_READINESS",
        "AUTHORIZATION_GRANT_READY_NOT_ISSUED",
        "GRANT_READINESS_METADATA_VISIBLE",
        "GRANT_READINESS_DIAGNOSTICS_VISIBLE",
        "AUTHORIZATION_DECISION_LINKAGE_VISIBLE",
    ...
    const readiness = {
      ...
      browserProof: anyFail || !grantMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(
        outDir,
        "phase3b3-18-feed-host-activation-transition-authorization-grant-readiness-prepared.json",
      ),
    ...
    writeFileSync(
      join(
        outDir,
        "phase3b3-18-feed-host-activation-transition-authorization-grant-readiness-summary.md",
      ),
      [
        ...
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Grant readiness: result=${grantReadiness.grantReadinessResult} ready=${grantReadiness.grantReady} issued=${grantReadiness.grantIssued} wouldIssueGrant=${grantReadiness.wouldIssueGrant}`,
        `- Authorization decision linkage: result=${authDecision.authorizationDecisionResult} eligible=${authDecision.authorizationEligible} granted=${authDecision.authorizationGranted}`,
        `- Selected transition: ${grantReadiness.selectedTransition} (current=${grantReadiness.currentState}/${grantReadiness.currentNode})`,
    ...
    await browser.close();
    process.exit(anyFail || !grantMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
