#!/usr/bin/env node
/**
 * Phase 3B.3.8 — Chromium proof: Controlled Host Activation Plan.
 * New proof run (not a reuse of 3B.3.7). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3029";
  let commit = "unknown";
  let branch = "workspace/phase3b38-controlled-host-activation-plan";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b38");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 9;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host activation plan; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const planMetaOk =
      ...
      hostShadowActivationSimulation.activationBlocker ===
      ...
      hostActivationDecision.wouldActivate === true &&
      hostActivationDecision.activationBlocker ===
        "PHASE_3B3_7_HOST_ACTIVATION_DECISION_ONLY" &&
      hostActivationDecision.nextEligibleStep === "3B.3.8" &&
      hostActivationPlan.planState === "completed" &&
      hostActivationPlan.planResult === "plan-complete-not-executable" &&
      hostActivationPlan.decisionResult === "ALLOW" &&
      hostActivationPlan.wouldActivate === true &&
      Array.isArray(hostActivationPlan.plannedSteps) &&
      hostActivationPlan.plannedSteps.length > 0 &&
      Array.isArray(hostActivationPlan.preconditions) &&
      hostActivationPlan.preconditions.length > 0 &&
      Array.isArray(hostActivationPlan.validationPoints) &&
      Array.isArray(hostActivationPlan.rollbackCheckpoints) &&
      Array.isArray(hostActivationPlan.abortConditions) &&
      Array.isArray(hostActivationPlan.invariants) &&
      hostActivationPlan.invariants.length === 20 &&
      hostActivationPlan.canStartActivation === false &&
      hostActivationPlan.hostActivation === false &&
      hostActivationPlan.renderActivation === false &&
      hostActivationPlan.activationBlocker ===
        "PHASE_3B3_8_HOST_ACTIVATION_PLAN_ONLY" &&
      hostActivationPlan.nextEligibleStep === "3B.3.9" &&
      hostActivationPlan.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationPlan.diagnostics?.planCompleted === true &&
      hostActivationPlan.diagnostics?.planResult ===
        "plan-complete-not-executable" &&
      hostActivationPlan.diagnostics?.decisionResult === "ALLOW" &&
      hostActivationPlan.diagnostics?.wouldActivate === true &&
      hostActivationPlan.diagnostics?.activationBlocked === true &&
      hostActivationPlan.diagnostics?.canStartActivation === false &&
      hostActivationPlan.diagnostics?.currentPhase === "3B.3.8" &&
      hostActivationPlan.diagnostics?.nextEligibleStep === "3B.3.9" &&
      shadowPlacement.placementState === "shadow-registered" &&
      shadowIdentity.expectedMountCount === 1 &&
      hostPlan.decisionState === "completed" &&
      hostPlan.decisionResult === "ALLOW" &&
      hostPlan.planState === "completed" &&
      hostPlan.planResult === "plan-complete-not-executable" &&
      hostPlan.wouldActivate === true &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !planMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_9"
          : "READY_FOR_PHASE_3B_3_9",
    };

    const outPath = join(
      outDir,
      "phase3b3-8-feed-host-activation-plan-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldActivate: true,
      plannedStepCount: hostActivationPlan.plannedSteps.length,
      hostActivation: false,
      renderActivation: false,
      canStartActivation: false,
      writer: "legacy",
      ...
      browserProof: anyFail || !planMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-8-feed-host-activation-plan-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-8-feed-host-activation-plan-summary.md"),
      [
        `# Phase 3B.3.8 Host Activation Plan Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Plan: result=${hostActivationPlan.planResult} steps=${hostActivationPlan.plannedSteps.length} wouldActivate=${hostActivationPlan.wouldActivate}`,
        `- Diagnostics: phase=${hostActivationPlan.diagnostics.currentPhase} stepCount=${hostActivationPlan.diagnostics.plannedStepCount}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_8_HOST_ACTIVATION_PLAN_ONLY); canStartActivation=false`,
    ...
    await browser.close();
    process.exit(anyFail || !planMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
