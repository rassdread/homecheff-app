#!/usr/bin/env node
/**
 * Phase 3B.3.16 — Chromium proof: Controlled Host Activation Transition Preflight.
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3037";
  let commit = "unknown";
  let branch = "workspace/phase3b316-controlled-host-activation-transition-preflight";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b316");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    pass(
      ...
      "legacy writer; host activation transition preflight; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const preflightMetaOk =
      ...
      hostActivationTransitionPreflight.selectedTransition ===
        "COMMIT_READY->ACTIVE" &&
      hostActivationTransitionPreflight.selectedFromState === "COMMIT_READY" &&
      hostActivationTransitionPreflight.selectedToState === "ACTIVE" &&
      ...
      hostActivationTransitionPreflight.transitionExecuted === false &&
      hostActivationTransitionPreflight.graphTraversalExecuted === false &&
      hostActivationTransitionPreflight.protocolExecuted === false &&
      hostActivationTransitionPreflight.transactionCommitted === false &&
      hostActivationTransitionPreflight.canStartActivation === false &&
      hostActivationTransitionPreflight.hostActivation === false &&
      hostActivationTransitionPreflight.activationBlocker ===
        "PHASE_3B3_16_HOST_ACTIVATION_TRANSITION_PREFLIGHT_ONLY" &&
      hostActivationTransitionPreflight.nextEligibleStep === "3B.3.17" &&
      hostActivationTransitionPreflight.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationTransitionPreflight.diagnostics?.preflightCompleted === true &&
      hostActivationTransitionPreflight.diagnostics?.currentState === "COMMIT_READY" &&
      hostActivationTransitionPreflight.diagnostics?.preflightExecuted === false &&
      hostActivationTransitionPreflight.diagnostics?.transitionAuthorized === false &&
      hostActivationTransitionPreflight.diagnostics?.executionImpossible === true &&
      hostActivationTransitionPreflight.diagnostics?.currentPhase === "3B.3.16" &&
      hostActivationTransitionPreflight.diagnostics?.nextEligibleStep === "3B.3.17" &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_ACTIVATION_TRANSITION_PREFLIGHT",
      ...
      overallVerdict:
        anyFail || !preflightMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_17"
          : "READY_FOR_PHASE_3B_3_17",
    };

    const outPath = join(
      outDir,
      "phase3b3-16-feed-host-activation-transition-preflight-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldCommit: true,
      commitReady: true,
      checkCount: hostActivationTransitionPreflight.preflightChecks.length,
      passedCount: hostActivationTransitionPreflight.passedChecks.length,
      failedCount: 0,
      warningCount: 0,
      ...
      browserProof: anyFail || !preflightMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-16-feed-host-activation-transition-preflight-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-16-feed-host-activation-transition-preflight-summary.md"),
      [
        `# Phase 3B.3.16 Host Activation Transition Preflight Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Transition preflight: result=${hostActivationTransitionPreflight.preflightResult} selected=${hostActivationTransitionPreflight.selectedTransition} preflightExecuted=${hostActivationTransitionPreflight.preflightExecuted}`,
        `- Diagnostics: phase=${hostActivationTransitionPreflight.diagnostics.currentPhase} next=${hostActivationTransitionPreflight.diagnostics.nextEligibleStep}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_16_HOST_ACTIVATION_TRANSITION_PREFLIGHT_ONLY); canStartActivation=false; transitionAuthorized=false`,
    ...
    await browser.close();
    process.exit(anyFail || !preflightMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
