#!/usr/bin/env node
/**
 * Phase 3B.3.5 — Chromium proof: Controlled Host Activation Readiness.
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3026";
  let commit = "unknown";
  let branch = "workspace/phase3b35-controlled-host-eligibility";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b35");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 6;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host activation readiness; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const readinessMetaOk =
      ...
      hostActivationReadiness.canStartActivation === false &&
      hostActivationReadiness.hostActivation === false &&
      hostActivationReadiness.renderActivation === false &&
      hostActivationReadiness.activationBlocker ===
        "PHASE_3B3_5_HOST_ACTIVATION_READINESS_ONLY" &&
      hostActivationReadiness.nextEligibleStep === "3B.3.6" &&
      hostActivationReadiness.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationReadiness.diagnostics?.readinessSatisfied === true &&
      hostActivationReadiness.diagnostics?.activationBlocked === true &&
      hostActivationReadiness.diagnostics?.canStartActivation === false &&
      Array.isArray(hostActivationReadiness.readinessReasons) &&
      hostActivationReadiness.readinessReasons.length > 0 &&
      Array.isArray(hostActivationReadiness.diagnostics?.activeBlockers) &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_ACTIVATION_READY",
        "READINESS_METADATA_VISIBLE",
        "READINESS_DIAGNOSTICS_VISIBLE",
      ...
      overallVerdict:
        anyFail || !readinessMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_6"
          : "READY_FOR_PHASE_3B_3_6",
    };

    const outPath = join(
      outDir,
      "phase3b3-5-feed-host-activation-readiness-proof.json",
    );
    ...
    const readiness = {
      ...
      browserProof: anyFail || !readinessMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-5-feed-host-activation-readiness-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-5-feed-host-activation-readiness-summary.md"),
      [
        ...
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Readiness: state=${hostActivationReadiness.readinessState} blockers=${hostActivationReadiness.readinessBlockers.join(",")}`,
        `- Diagnostics: phase=${hostActivationReadiness.diagnostics.currentPhase} missing=${hostActivationReadiness.diagnostics.missingConditionsForActivation.join(",")}`,
    ...
    await browser.close();
    process.exit(anyFail || !readinessMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
