...
const port = process.env.PHASE3B33_PORT || "3024";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b32 = join(root, "docs/audits/artifacts/phase3b33");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b33] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b33] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b33] Phase 3B.3.3 host registration proof");
    await run("node", [
      "scripts/probe-feed-host-registration-phase3b33.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b32}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b32, "phase3b3-3-feed-host-registration-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_4") {
      throw new Error(`Phase 3B.3.3 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b33] READY_FOR_PHASE_3B_3_4");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b32, "phase3b3-3-server-log-tail.txt"),
