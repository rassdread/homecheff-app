/**
 * Phase 3B.3.40 static validator — controlled workspace host activation issuance pipeline execution authorization.
 */
...
const PREDECESSOR_PHASE = "3B.3.39";
const PREDECESSOR_BRANCH =
  "workspace/phase3b339-controlled-workspace-host-activation-issuance-pipeline-execution-readiness";
const PREDECESSOR_HEAD =
  "ffb0037c18f119ea4cb267b100d6341f779b6da5";
const PREDECESSOR_PROOF_TARGET =
  "45ee6762e4aeaf7d6725d32340cc96ec9089a167";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-issuance-pipeline-execution-ready-not-executed";
const PREDECESSOR_LIFECYCLE = "PIPELINE_EXECUTION_READY_NOT_EXECUTED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_40";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-issuance-pipeline-execution-authorization.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-issuance-pipeline-execution-authorization-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-issuance-pipeline-execution-authorization-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-issuance-pipeline-execution-authorization-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-phase3b340.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-proof-phase3b340.mjs",
);
...
const priorProofPath = join(
  root,
  "docs/audits/artifacts/phase3b339/phase3b3-39-controlled-workspace-host-activation-issuance-pipeline-execution-readiness-proof.json",
);
mustExist(
  "docs/audits/artifacts/phase3b339/phase3b3-39-controlled-workspace-host-activation-issuance-pipeline-execution-readiness-proof.json",
);
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b340/phase3b3-40-controlled-workspace-host-activation-issuance-pipeline-execution-authorization-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b340-controlled-workspace-host-activation-issuance-pipeline-execution-authorization.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B340_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.40 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B340_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostActivationIssuancePipelineExecutionAuthorizationDescriptor();
assert.equal(descriptor.phase, "3B.3.40");
assert.equal(
  descriptor.pipelineExecutionAuthorizationResult,
  "controlled-workspace-host-activation-issuance-pipeline-execution-authorized-not-executed",
);
assert.equal(
  descriptor.pipelineExecutionAuthorizationState,
...
assert.equal(descriptor.nextEligibleStep, "3B.3.41");
assert.equal(
...
assert.ok(
  CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_BLOCKERS.includes(
    PHASE_3B3_40_CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_ONLY,
  ),
);

rejectUnresolved("descriptor.pipelineExecutionAuthorizationResult", descriptor.pipelineExecutionAuthorizationResult);
...
const evaluation = evaluateControlledWorkspaceHostActivationIssuancePipelineExecutionAuthorization();
assert.equal(evaluation.diagnostics.activationCommitBoundaryEntered, true);
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.40");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.41");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "duplicate issuance pipeline execution readiness",
    input: { issuancePipelineExecutionAuthorized: true },
  },
  {
  ...
  },
  {
    ...
    input: { entryRecords: [{}, {}], issuancePipelineExecutionAuthorized: true },
  },
  {
...
const contract = createControlledWorkspaceHostActivationIssuancePipelineExecutionAuthorizationContract();
assert.equal(contract.phase, "3B.3.40");
assert.equal(contract.nextEligibleStep, "3B.3.41");
const identity = createFeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationIdentity();
assert.equal(
...
if (artifactsPresent) {
  ...
  assert.equal(prepared.nextEligibleStep, "3B.3.41");
...
console.log(
  "validate-adaptive-workspace-host-activation-issuance-pipeline-execution-authorization-phase3b340: PASS",
);
