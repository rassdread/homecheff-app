#!/usr/bin/env node
/**
 * Phase 3B.3.36 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3058";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b337");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_37") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionCommitAuthorization = await api.readControlledWorkspaceHostActivationTransactionCommitAuthorization();
    ...
    const reg = probe.transactionCommitAuthorization;
    ...
    const forcedNegativeProofs = {
      ...
      entryCountOne: reg.transactionCommitAuthorizationCount === 1,
      noDuplicateEntry: reg.duplicateTransactionCommitAuthorizationCount === 0,
    ...
    const transactionCommitAuthorizationMetaOk =
      ...
      reg.transactionCommitAuthorizationState === "TRANSACTION_COMMIT_AUTHORIZED_NOT_COMMITTED" &&
      reg.transactionCommitAuthorizationResult === "controlled-workspace-host-activation-transaction-commit-authorized-not-committed" &&
      ...
      reg.grantCount === 1 &&
      reg.transactionCommitAuthorizationCount === 1 &&
      reg.duplicateTransactionCommitAuthorizationCount === 0 &&
      ...
      reg.nextEligibleStep === "3B.3.38" &&
      reg.activationBlocker === "PHASE_3B3_37_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_ONLY" &&
      reg.predecessorActivationTransactionPreparationState === "TRANSACTION_PREPARED_NOT_COMMITTED" &&
      reg.predecessorActivationTransactionCommitReadinessResult === "controlled-workspace-host-activation-transaction-commit-ready-not-committed" &&
      reg.predecessorActivationTransactionCommitReadinessState === "TRANSACTION_COMMIT_READY_NOT_COMMITTED" &&
    ...
    const anyFail =
      ...
      !transactionCommitAuthorizationMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionCommitAuthorization: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_38" : "READY_FOR_PHASE_3B_3_38",
    };

    const proofPath = join(outDir, "phase3b3-37-controlled-workspace-host-activation-transaction-commit-authorization-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.37",
      status: "controlled-workspace-host-activation-transaction-commit-authorization-prepared",
      transactionCommitAuthorizationContract: "valid",
    ...
    writeFileSync(
      join(outDir, "phase3b3-37-controlled-workspace-host-activation-transaction-commit-authorization-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.37 Controlled Workspace Host Activation Transaction Commit Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionCommitAuthorizationResult} state=${reg.transactionCommitAuthorizationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- transactionCommitAuthorizationMetaOk: ${transactionCommitAuthorizationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-37-controlled-workspace-host-activation-transaction-commit-authorization-summary.md"),
      summary,
    );
