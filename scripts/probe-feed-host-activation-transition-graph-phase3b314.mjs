#!/usr/bin/env node
/**
 * Phase 3B.3.14 — Chromium proof: Controlled Host Activation Transition Graph.
 * New proof run (not a reuse of 3B.3.13). Requires production server with
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3035";
  let commit = "unknown";
  let branch = "workspace/phase3b314-controlled-host-activation-transition-graph";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b314");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    pass(
      ...
      "legacy writer; host activation transition graph; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const transitionGraphMetaOk =
      ...
      hostActivationTransitionGraph.entryNode === "LEGACY_DORMANT" &&
      hostActivationTransitionGraph.graphTraversalExecuted === false &&
      hostActivationTransitionGraph.transitionExecuted === false &&
      hostActivationTransitionGraph.protocolExecuted === false &&
      hostActivationTransitionGraph.transactionCommitted === false &&
      Array.isArray(hostActivationTransitionGraph.graphNodes) &&
      hostActivationTransitionGraph.graphNodes.length > 0 &&
      Array.isArray(hostActivationTransitionGraph.graphEdges) &&
      hostActivationTransitionGraph.graphEdges.length > 0 &&
      Array.isArray(hostActivationTransitionGraph.allowedPaths) &&
      hostActivationTransitionGraph.allowedPaths.length > 0 &&
      Array.isArray(hostActivationTransitionGraph.blockedPaths) &&
      hostActivationTransitionGraph.blockedPaths.includes(
        "COMMIT_READY->ACTIVE",
      ) &&
      Array.isArray(hostActivationTransitionGraph.reachableNodes) &&
      hostActivationTransitionGraph.reachableNodes.includes("COMMIT_READY") &&
      Array.isArray(hostActivationTransitionGraph.unreachableNodes) &&
      hostActivationTransitionGraph.unreachableNodes.includes("ACTIVE") &&
      Array.isArray(hostActivationTransitionGraph.edgeGuards) &&
      hostActivationTransitionGraph.edgeGuards.length > 0 &&
      Array.isArray(hostActivationTransitionGraph.edgeBlockers) &&
      Array.isArray(hostActivationTransitionGraph.edgePreconditions) &&
      hostActivationTransitionGraph.canStartActivation === false &&
      hostActivationTransitionGraph.hostActivation === false &&
      hostActivationTransitionGraph.activationBlocker ===
        "PHASE_3B3_14_HOST_ACTIVATION_TRANSITION_GRAPH_ONLY" &&
      hostActivationTransitionGraph.nextEligibleStep === "3B.3.15" &&
      hostActivationTransitionGraph.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationTransitionGraph.diagnostics?.graphCompleted === true &&
      hostActivationTransitionGraph.diagnostics?.currentNode === "COMMIT_READY" &&
      hostActivationTransitionGraph.diagnostics?.graphTraversalExecuted === false &&
      hostActivationTransitionGraph.diagnostics?.activeUnreachable === true &&
      hostActivationTransitionGraph.diagnostics?.traversalImpossible === true &&
      hostActivationTransitionGraph.diagnostics?.currentPhase === "3B.3.14" &&
      hostActivationTransitionGraph.diagnostics?.nextEligibleStep === "3B.3.15" &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      hostContract,
      scenarios: [
        "HOST_ACTIVATION_TRANSITION_GRAPH",
      ...
      overallVerdict:
        anyFail || !transitionGraphMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_15"
          : "READY_FOR_PHASE_3B_3_15",
    };

    const outPath = join(
      outDir,
      "phase3b3-14-feed-host-activation-transition-graph-proof.json",
    );
    ...
    const readiness = {
      ...
      wouldCommit: true,
      commitReady: true,
      nodeCount: hostActivationTransitionGraph.graphNodes.length,
      edgeCount: hostActivationTransitionGraph.graphEdges.length,
      allowedPathCount: hostActivationTransitionGraph.allowedPaths.length,
      blockedPathCount: hostActivationTransitionGraph.blockedPaths.length,
      ...
      browserProof: anyFail || !transitionGraphMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-14-feed-host-activation-transition-graph-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-14-feed-host-activation-transition-graph-summary.md"),
      [
        `# Phase 3B.3.14 Host Activation Transition Graph Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Transition graph: result=${hostActivationTransitionGraph.graphResult} current=${hostActivationTransitionGraph.currentNode} graphTraversalExecuted=${hostActivationTransitionGraph.graphTraversalExecuted}`,
        `- Diagnostics: phase=${hostActivationTransitionGraph.diagnostics.currentPhase} next=${hostActivationTransitionGraph.diagnostics.nextEligibleStep}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_14_HOST_ACTIVATION_TRANSITION_GRAPH_ONLY); canStartActivation=false; graphTraversalExecuted=false`,
    ...
    console.log(
      JSON.stringify(
        {
          ok: !anyFail && transitionGraphMetaOk,
          outPath,
          verdict: artifact.overallVerdict,
    ...
    await browser.close();
    process.exit(anyFail || !transitionGraphMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
