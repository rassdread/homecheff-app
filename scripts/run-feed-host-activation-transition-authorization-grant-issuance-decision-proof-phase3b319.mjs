#!/usr/bin/env node
/**
 * Phase 3B.3.19 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.19 host activation transition authorization grant issuance
...
const port = process.env.PHASE3B319_PORT || "3040";
...
mkdirSync(out3b319, { recursive: true });
...
async function main() {
  ...
  if (priorGrantReadinessProof.overallVerdict !== "READY_FOR_PHASE_3B_3_19") {
    throw new Error(
      `Phase 3B.3.18 grant readiness proof required: ${priorGrantReadinessProof.overallVerdict}`,
    );
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b319] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b319] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log(
      "[phase3b319] Phase 3B.3.19 host activation transition authorization grant issuance decision proof",
    );
    await run("node", [
    ...
    if (p319.overallVerdict !== "READY_FOR_PHASE_3B_3_20") {
      throw new Error(`Phase 3B.3.19 proof failed: ${p319.overallVerdict}`);
    }
    ...
    console.log("[phase3b319] READY_FOR_PHASE_3B_3_20");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b319, "phase3b3-19-server-log-tail.txt"),
