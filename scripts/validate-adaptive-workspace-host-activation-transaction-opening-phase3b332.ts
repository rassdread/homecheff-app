/**
 * Phase 3B.3.32 static validator — controlled workspace host activation transaction-opening.
 */
...
const PREDECESSOR_PHASE = "3B.3.31";
const PREDECESSOR_HEAD =
  "d1e1dd801e8793d96d4931ca4e3cde9bda30d21a";
const PREDECESSOR_PROOF_TARGET =
  "6d186d70f02659faa2ac2f927551465a29f31255";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-transaction-opening-authorized-not-opened";
const PREDECESSOR_LIFECYCLE = "TRANSACTION_OPENING_AUTHORIZED_NOT_OPENED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_32";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-opening.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-opening-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-opening-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-opening-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-transaction-opening-phase3b332.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-transaction-opening-proof-phase3b332.mjs",
);
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b332/phase3b3-32-controlled-workspace-host-activation-transaction-opening-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b332-controlled-workspace-host-activation-transaction-opening.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B332_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.32 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B332_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostActivationTransactionOpeningDescriptor();
assert.equal(descriptor.phase, "3B.3.32");
assert.equal(
  descriptor.transactionOpeningResult,
  "controlled-workspace-host-activation-transaction-opened-not-prepared",
);
assert.equal(descriptor.transactionOpeningState, "TRANSACTION_OPENED_NOT_PREPARED");
...
assert.equal(descriptor.transactionOpeningCount, 1);
assert.equal(descriptor.duplicateTransactionOpeningCount, 0);
...
assert.equal(
  descriptor.predecessorActivationTransactionOpeningAuthorizationState,
  PREDECESSOR_LIFECYCLE,
);
assert.equal(
  descriptor.predecessorActivationTransactionOpeningAuthorizationResult,
  PREDECESSOR_RESULT,
);
...
assert.equal(descriptor.nextEligibleStep, "3B.3.33");
assert.equal(
...
rejectUnresolved("descriptor.transactionOpeningResult", descriptor.transactionOpeningResult);
...
const evaluation = evaluateControlledWorkspaceHostActivationTransactionOpening(registry);
assert.equal(evaluation.diagnostics.activationCommitBoundaryEntered, true);
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.32");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.33");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "transaction opening completed early",
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_33");
  assert.equal(proof.activationTransactionOpeningMetaOk, true);
  assert.equal(proof.forcedNegativeProofsOk, true);
  assert.equal(proof.bridgeVersion ?? proof.version ?? 33, 33);
  rejectUnresolved("proof.overallVerdict", proof.overallVerdict);
  const prepared = JSON.parse(readFileSync(preparedPath, "utf8"));
  validateFeedWorkspaceHostActivationTransactionOpeningPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-activation-transaction-opening-phase3b332: PASS",
);
