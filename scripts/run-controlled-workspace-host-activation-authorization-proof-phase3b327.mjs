#!/usr/bin/env node
/**
 * Phase 3B.3.27 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.27 activation authorization Chromium proof
 */
...
const port = process.env.PHASE3B327_PORT || "3048";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b327 = join(root, "docs/audits/artifacts/phase3b327");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b327, { recursive: true });
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_27") {
    throw new Error(`Phase 3B.3.26 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b327] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b327] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b327] Phase 3B.3.27 activation authorization proof");
    await run("node", [
    ...
    const p326 = JSON.parse(
      readFileSync(
        join(
          out3b327,
          "phase3b3-27-controlled-workspace-host-activation-authorization-proof.json",
        ),
        "utf8",
      ),
    );
    if (p326.overallVerdict !== "READY_FOR_PHASE_3B_3_28") {
      throw new Error(`Phase 3B.3.27 proof failed: ${p326.overallVerdict}`);
    }
    if (p326.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p326.commit} HEAD=${commit}`);
    }
    console.log("[phase3b327] READY_FOR_PHASE_3B_3_28");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b327, "phase3b3-27-server-log-tail.txt"),
