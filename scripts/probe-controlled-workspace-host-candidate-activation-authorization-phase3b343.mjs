#!/usr/bin/env node
/**
 * Phase 3B.3.43 — Chromium proof: Controlled Workspace Host Candidate Activation Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3064";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b343");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const candidateActivationReadiness = await api.readControlledWorkspaceHostCandidateActivationAuthorization();
    ...
    const candidateActivationAuthorizedMetaOk =
      probe.version === 44 &&
      reg.phase === "3B.3.43" &&
      ...
      reg.candidateActivationAuthorizationState === "CANDIDATE_ACTIVATION_AUTHORIZED_NOT_ACTIVATED" &&
      reg.candidateActivationAuthorizationResult === "controlled-workspace-host-candidate-activation-authorized-not-activated" &&
      ...
      reg.nextEligibleStep === "3B.3.44" &&
      reg.activationBlocker === "PHASE_3B3_43_CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_ONLY" &&
    ...
    const anyFail =
      ...
      !candidateActivationAuthorizedMetaOk ||
    ...
    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.43",
      status: "controlled-workspace-host-candidate-activation-authorization-prepared",
      ...
      activationCandidateActivationAuthorizationId: "feed.discovery.adaptive-workspace.host-candidate-activation-authorization.v1",
    ...
    const summary = [
      "# Phase 3B.3.43 Controlled Workspace Host Candidate Activation Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.candidateActivationAuthorizationResult} state=${reg.candidateActivationAuthorizationState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- candidateActivationAuthorizedMetaOk: ${candidateActivationAuthorizedMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
