#!/usr/bin/env node
/**
 * Phase 3B.3.47 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.47 candidate-execution-started Chromium proof
 */
...
const port = process.env.PHASE3B346_PORT || "3068";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b346 = join(root, "docs/audits/artifacts/phase3b347");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_47") {
    throw new Error(`Phase 3B.3.47 predecessor proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b347] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b347] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b347] Phase 3B.3.47 candidate-execution-started proof");
    await run("node", [
    ...
    const p333 = JSON.parse(
      readFileSync(
        join(
          out3b346,
          "phase3b3-47-controlled-workspace-host-candidate-execution-started-proof.json",
        ),
        "utf8",
      ),
    );
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_48") {
      throw new Error(`Phase 3B.3.47 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b347] READY_FOR_PHASE_3B_3_48");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b346, "phase3b3-47-server-log-tail.txt"),
