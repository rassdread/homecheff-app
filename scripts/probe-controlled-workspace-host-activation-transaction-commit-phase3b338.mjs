#!/usr/bin/env node
/**
 * Phase 3B.3.37 — Chromium proof: Controlled Workspace Host Activation Transaction Opening.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3059";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b338");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_38") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionCommit = await api.readControlledWorkspaceHostActivationTransactionCommit();
    ...
    const reg = probe.transactionCommit;
    ...
    const transactionCommittedMetaOk =
      probe.version === 39 &&
      reg.phase === "3B.3.38" &&
      ...
      reg.transactionCommitState === "TRANSACTION_COMMITTED_NOT_EXECUTED" &&
      reg.transactionCommitResult === "controlled-workspace-host-activation-transaction-committed-not-executed" &&
      ...
      reg.nextEligibleStep === "3B.3.39" &&
      reg.activationBlocker === "PHASE_3B3_38_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_ONLY" &&
      reg.predecessorActivationTransactionPreparationState === "TRANSACTION_PREPARED_NOT_COMMITTED" &&
      reg.predecessorActivationTransactionCommitAuthorizationResult === "controlled-workspace-host-activation-transaction-commit-authorized-not-committed" &&
      reg.predecessorActivationTransactionCommitAuthorizationState === "TRANSACTION_COMMIT_AUTHORIZED_NOT_COMMITTED" &&
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionCommit: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_39" : "READY_FOR_PHASE_3B_3_39",
    };

    const proofPath = join(outDir, "phase3b3-38-controlled-workspace-host-activation-transaction-commit-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-38-controlled-workspace-host-activation-transaction-commit-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.38 Controlled Workspace Host Activation Transaction Commit Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.transactionCommitResult} state=${reg.transactionCommitState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- transactionCommittedMetaOk: ${transactionCommittedMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-38-controlled-workspace-host-activation-transaction-commit-summary.md"),
      summary,
    );
