#!/usr/bin/env node
/**
 * Phase 3B.3.24 — Chromium proof: Controlled Workspace Host Candidate Registration.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3045";
  let commit = "unknown";
  let branch = "workspace/phase3b324-controlled-workspace-host-candidate-registration";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b324");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_24") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const registration = await api.readControlledWorkspaceHostCandidateRegistration();
    ...
    const forcedNegativeProofs = {
      candidateNotSelected: reg.candidateSelected === false,
      candidateNotActivated: reg.candidateActivated === false,
      candidateNotExecutable: reg.candidateRegistrationExecutable === false,
    ...
    const candidateRegistrationMetaOk =
      probe.version === 25 &&
      reg.phase === "3B.3.24" &&
      ...
      reg.candidateRegistrationState === "REGISTERED_NOT_SELECTED" &&
      reg.candidateRegistrationResult === "controlled-workspace-host-candidate-registered-not-selected" &&
      reg.candidateRegistrationCompleted === true &&
      reg.candidateRegistrationReady === true &&
      reg.candidateRegistrationBlocked === true &&
      reg.candidateRegistrationExecutable === false &&
      reg.candidateRegistered === true &&
      reg.candidateSelected === false &&
      reg.candidateActivated === false &&
      reg.wouldSelectCandidate === true &&
      reg.futureSelectionTarget === true &&
      reg.candidateCount === 1 &&
      reg.registeredCandidateCount === 1 &&
      reg.selectedCandidateCount === 0 &&
      ...
      reg.nextEligibleStep === "3B.3.25" &&
      reg.activationBlocker === "PHASE_3B3_24_CONTROLLED_WORKSPACE_HOST_CANDIDATE_REGISTRATION_ONLY" &&
    ...
    const anyFail = invariants.some((i) => i.status !== "PASS") || !candidateRegistrationMetaOk;
    ...
    const artifact = {
      ...
      controlledWorkspaceHostCandidateRegistration: reg,
      hostActivationTransitionAuthorizationGrantIssuanceCommitBoundary: cb,
      candidateRegistrationMetaOk,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_25" : "READY_FOR_PHASE_3B_3_25",
    };

    const proofPath = join(outDir, "phase3b3-24-controlled-workspace-host-candidate-registration-proof.json");
    ...
    writeFileSync(
      join(outDir, "phase3b3-24-controlled-workspace-host-candidate-registration-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.24 Controlled Workspace Host Candidate Registration Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Candidate: result=${reg.candidateRegistrationResult} selected=${reg.candidateSelected}`,
      ...
      `- candidateRegistrationMetaOk: ${candidateRegistrationMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-24-controlled-workspace-host-candidate-registration-summary.md"),
      summary,
    );
