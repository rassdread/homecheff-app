/**
 * Phase 3B.3.37 static validator — controlled workspace host activation transaction-commit-authorization.
 */
...
const PREDECESSOR_PHASE = "3B.3.36";
const PREDECESSOR_HEAD =
  "10654dc07334935712b430338bbf378c50af3d34";
const PREDECESSOR_PROOF_TARGET =
  "0c5b5f652d78fa01077b23656e8d480038480a0f";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-transaction-commit-ready-not-committed";
const PREDECESSOR_LIFECYCLE = "TRANSACTION_COMMIT_READY_NOT_COMMITTED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_37";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-commit-authorization.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-commit-authorization-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-commit-authorization-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-commit-authorization-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-transaction-commit-authorization-phase3b337.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-transaction-commit-authorization-proof-phase3b337.mjs",
);
...
const proofPath = join(
  root,
  "docs/audits/artifacts/phase3b337/phase3b3-37-controlled-workspace-host-activation-transaction-commit-authorization-proof.json",
);
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b337/phase3b3-37-controlled-workspace-host-activation-transaction-commit-authorization-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b337-controlled-workspace-host-activation-transaction-commit-authorization.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B337_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.37 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B337_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostActivationTransactionCommitAuthorizationDescriptor();
assert.equal(descriptor.phase, "3B.3.37");
assert.equal(
  descriptor.transactionCommitAuthorizationResult,
  "controlled-workspace-host-activation-transaction-commit-authorized-not-committed",
);
assert.equal(
  descriptor.transactionCommitAuthorizationState,
...
assert.equal(descriptor.transactionCommitAuthorizationCount, 1);
assert.equal(descriptor.duplicateTransactionCommitAuthorizationCount, 0);
...
assert.equal(
  descriptor.predecessorActivationTransactionCommitReadinessState,
  PREDECESSOR_LIFECYCLE,
);
assert.equal(
  descriptor.predecessorActivationTransactionCommitReadinessResult,
  PREDECESSOR_RESULT,
);
...
assert.equal(descriptor.nextEligibleStep, "3B.3.38");
assert.equal(
...
rejectUnresolved("descriptor.transactionCommitAuthorizationResult", descriptor.transactionCommitAuthorizationResult);
...
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.37");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.38");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "duplicate commit authorization",
    input: { transactionCommitAuthorized: true },
  },
  {
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_38");
  assert.equal(proof.transactionCommitAuthorizationMetaOk, true);
  assert.equal(proof.forcedNegativeProofsOk, true);
  assert.equal(proof.bridgeVersion ?? proof.version ?? 38, 38);
  rejectUnresolved("proof.overallVerdict", proof.overallVerdict);
  const prepared = JSON.parse(readFileSync(preparedPath, "utf8"));
  validateFeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-activation-transaction-commit-authorization-phase3b337: PASS",
);
