#!/usr/bin/env node
/**
 * WX Phase 1B.5.9 — Contextual Intent Resolution browser proof.
 * Asserts intent diagnostics present, zero DOM/layout/chrome delta, no remount.
 */
...
function parseArgs(argv) {
  ...
  let outDir = join(process.cwd(), "docs/audits/wx-phase1b5-9-context-intent");
  let journey = false;
  for (const a of argv) {
...
async function snap(page) {
  return page.evaluate(() => {
    ...
    const newHonestyUi = document.querySelector(
      "[data-wx-density-panel],[data-wx-compact-ui],[data-wx-honesty-ui],[data-wx-density-overlay],[data-wx-priority-panel],[data-wx-priority-ui],[data-wx-rank-bar],[data-wx-relevance-panel],[data-wx-relevance-ui],[data-wx-relevance-bar],[data-wx-intent-panel],[data-wx-intent-ui],[data-wx-intent-bar]",
    );
    ...
    return {
      ...
      contextRelevance: ws?.getAttribute("data-wx-context-relevance"),
      contextRelevanceVersion: ws?.getAttribute("data-wx-context-relevance-version"),
      contextRelevanceToken: ws?.getAttribute("data-wx-context-relevance-token"),
      contextRelevanceStatus: ws?.getAttribute("data-wx-context-relevance-status"),
      relevanceRenders: ws?.getAttribute("data-wx-relevance-renders"),
      relevanceDrivesChrome: ws?.getAttribute("data-wx-relevance-drives-chrome"),
      relevanceAppliesOrdering: ws?.getAttribute("data-wx-relevance-applies-ordering"),
      relevance: ws?.getAttribute("data-wx-relevance"),
      relevanceScore: ws?.getAttribute("data-wx-relevance-score"),
      relevanceIds: ws?.getAttribute("data-wx-relevance-ids"),
      contextIntent: ws?.getAttribute("data-wx-context-intent"),
      contextIntentVersion: ws?.getAttribute("data-wx-context-intent-version"),
      contextIntentToken: ws?.getAttribute("data-wx-context-intent-token"),
      contextIntentStatus: ws?.getAttribute("data-wx-context-intent-status"),
      intentRenders: ws?.getAttribute("data-wx-intent-renders"),
      intentDrivesChrome: ws?.getAttribute("data-wx-intent-drives-chrome"),
      intentAppliesOrdering: ws?.getAttribute("data-wx-intent-applies-ordering"),
      intent: ws?.getAttribute("data-wx-intent"),
      intentScore: ws?.getAttribute("data-wx-intent-score"),
      intentIds: ws?.getAttribute("data-wx-intent-ids"),
      ...
      noNewRelevanceUi: newHonestyUi == null,
      noNewIntentUi: newHonestyUi == null,
      ...
      focusableDiagnostics: [...document.querySelectorAll("[data-wx-context-intent],[data-wx-context-relevance],[data-wx-context-priority]")].some(
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    if (baselineIntentUi == null) baselineIntentUi = s.noNewPriorityUi;
    ...
    const checks = {
      phase: s.phase === "1b.5.9",
      contextIntent: s.contextIntent === "wx-context-intent-v1",
      contextIntentVersion: s.contextIntentVersion === "1.0.0",
      intentRendersOff: s.intentRenders === "0",
      intentDrivesChromeOff: s.intentDrivesChrome === "0",
      intentAppliesOrderingOff: s.intentAppliesOrdering === "0",
      hasIntent: typeof s.intent === "string" && s.intent.includes("stage:"),
      hasIntentScore: typeof s.intentScore === "string" && s.intentScore.includes("stage:"),
      intentStatusOk: s.contextIntentStatus === "ok",
      contextRelevanceStillPresent: s.contextRelevance === "wx-context-relevance-v1",
      relevanceRendersOff: s.relevanceRenders === "0",
      contextPriorityStillPresent: s.contextPriority === "wx-context-priority-v1",
      ...
      hasToken: Boolean(s.contextIntentToken),
      hasOrderedIds:
        typeof s.intentIds === "string" &&
        s.intentIds.includes("stage") &&
        s.intentIds.includes("tool"),
      noNewPriorityUi: s.noNewPriorityUi === true,
      noNewRelevanceUi: s.noNewRelevanceUi === true,
      noNewIntentUi: s.noNewIntentUi === true,
      noNewDisclosureUi: s.noNewDisclosureUi === true,
      noNewAssistUi: s.noNewAssistUi === true,
      noNewToolUi: s.noNewToolUi === true,
      visibleIntentDomDeltaZero: s.noNewPriorityUi === baselineIntentUi,
  ...
  if (args.journey) {
    ...
    journey = {
      ...
      noConsoleErrors: filteredConsole.length === 0,
      noPageErrors: pageErrors.length === 0,
      zeroVisibleIntentUi: steps.every((s) => s.snap.noNewPriorityUi),
      ...
      pass:
        ...
        steps.every(
          (s) =>
            ...
            s.snap.noNewToolUi &&
            s.snap.intentDrivesChrome === "0" &&
            s.snap.intentRenders === "0" &&
            s.snap.intentAppliesOrdering === "0" &&
            s.snap.relevanceDrivesChrome === "0" &&
            s.snap.phase === "1b.5.9",
        ),
    };
    await page.close();
  ...
  const report = {
    ...
    verdict: allPass
      ? "WX_PHASE_1B5_9_BROWSER_PROOF_PASS"
      : "WX_PHASE_1B5_9_BROWSER_PROOF_FAIL",
    passCount: cases.filter((c) => c.pass).length,
    caseCount: cases.length,
    visibleIntentDomDeltaZero: cases.every((c) => c.checks.visibleIntentDomDeltaZero),
    ...
    JSON.stringify(
      {
        ...
        visibleIntentDomDeltaZero: report.visibleIntentDomDeltaZero,
