#!/usr/bin/env node
/**
 * Phase 3B.3.9 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.9 host activation pipeline Chromium proof (new)
 */
...
const port = process.env.PHASE3B39_PORT || "3030";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b39 = join(root, "docs/audits/artifacts/phase3b39");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b39, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b39] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b39] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b39] Phase 3B.3.9 host activation pipeline proof");
    await run("node", [
      "scripts/probe-feed-host-activation-pipeline-phase3b39.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b39}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b39, "phase3b3-9-feed-host-activation-pipeline-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_10") {
      throw new Error(`Phase 3B.3.9 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b39] READY_FOR_PHASE_3B_3_10");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b39, "phase3b3-9-server-log-tail.txt"),
