#!/usr/bin/env node
/**
 * Phase 3B.3.8 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.8 host activation plan Chromium proof (new)
 */
...
const port = process.env.PHASE3B38_PORT || "3029";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b38 = join(root, "docs/audits/artifacts/phase3b38");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b38, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b38] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b38] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b38] Phase 3B.3.8 host activation plan proof");
    await run("node", [
      "scripts/probe-feed-host-activation-plan-phase3b38.mjs",
      `--base-url=${baseUrl}`,
      `--commit=${commit}`,
      `--branch=${branch}`,
      `--out-dir=${out3b38}`,
    ...
    const p32 = JSON.parse(
      readFileSync(
        join(out3b38, "phase3b3-8-feed-host-activation-plan-proof.json"),
        "utf8",
      ),
    );
    if (p32.overallVerdict !== "READY_FOR_PHASE_3B_3_9") {
      throw new Error(`Phase 3B.3.8 proof failed: ${p32.overallVerdict}`);
    }
    ...
    console.log("[phase3b38] READY_FOR_PHASE_3B_3_9");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b38, "phase3b3-8-server-log-tail.txt"),
