...
assert.equal(host.nextEligibleStep, "3B.3.3");
assert.ok(
...
assert.equal(
  ...
  "3B.3.3-controlled-host-activation-candidate",
);
...
assert.equal(gate.currentStep, "3B.3.2");
assert.equal(gate.eligibleStep, "3B.3.3");
...
assert.match(probeBridge, /version:\s*3/);
