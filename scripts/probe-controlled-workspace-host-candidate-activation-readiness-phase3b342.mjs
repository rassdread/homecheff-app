#!/usr/bin/env node
/**
 * Phase 3B.3.42 — Chromium proof: Controlled Workspace Host Candidate Activation Readiness.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3063";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b342");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_42") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const candidateActivationReadiness = await api.readControlledWorkspaceHostCandidateActivationReadiness();
    ...
    const candidateActivationReadyMetaOk =
      probe.version === 43 &&
      reg.phase === "3B.3.42" &&
      ...
      reg.candidateActivationReadinessState === "CANDIDATE_ACTIVATION_READY_NOT_ACTIVATED" &&
      reg.candidateActivationReadinessResult === "controlled-workspace-host-candidate-activation-ready-not-activated" &&
    ...
    const prepared = {
      ...
      activationCandidateActivationReadinessId: "feed.discovery.adaptive-workspace.host-candidate-activation-readiness.v1",
    ...
    const summary = [
      "# Phase 3B.3.42 Controlled Workspace Host Activation Transaction Commit Authorization Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Transaction opening: result=${reg.candidateActivationReadinessResult} state=${reg.candidateActivationReadinessState} started=${reg.transactionOpeningStarted} completed=${reg.transactionOpeningCompleted}`,
      ...
      `- candidateActivationReadyMetaOk: ${candidateActivationReadyMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
