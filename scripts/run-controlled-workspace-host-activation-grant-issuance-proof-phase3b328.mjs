#!/usr/bin/env node
/**
 * Phase 3B.3.28 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.28 activation grant issuance Chromium proof
 */
...
const port = process.env.PHASE3B328_PORT || "3049";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b327 = join(root, "docs/audits/artifacts/phase3b328");
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_28") {
    throw new Error(`Phase 3B.3.27 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b328] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b328] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b328] Phase 3B.3.28 activation grant issuance proof");
    await run("node", [
    ...
    const p328 = JSON.parse(
      readFileSync(
        join(
          out3b327,
          "phase3b3-28-controlled-workspace-host-activation-grant-issuance-proof.json",
        ),
        "utf8",
      ),
    );
    if (p328.overallVerdict !== "READY_FOR_PHASE_3B_3_29") {
      throw new Error(`Phase 3B.3.28 proof failed: ${p328.overallVerdict}`);
    }
    if (p328.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p328.commit} HEAD=${commit}`);
    }
    console.log("[phase3b328] READY_FOR_PHASE_3B_3_29");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b327, "phase3b3-28-server-log-tail.txt"),
