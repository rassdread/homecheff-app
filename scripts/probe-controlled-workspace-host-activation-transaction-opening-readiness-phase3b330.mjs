#!/usr/bin/env node
/**
 * Phase 3B.3.30 — Chromium proof: Controlled Workspace Host Activation Transaction Opening Readiness.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3051";
  let commit = "unknown";
  let branch = "workspace/phase3b330-controlled-workspace-host-activation-transaction-opening-readiness";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b330");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_30") {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 31;
      },
      { timeout: 120000 },
    );
    ...
    const probe = await page.evaluate(async () => {
      ...
      const transactionOpeningReadiness = await api.readControlledWorkspaceHostActivationTransactionOpeningReadiness();
    ...
    const reg = probe.transactionOpeningReadiness;
    ...
    const forcedNegativeProofs = {
      ...
      transactionOpeningNotAuthorized: reg.transactionOpeningAuthorized === false,
      transactionOpeningNotStarted: reg.transactionOpeningStarted === false,
      transactionOpeningNotCompleted: reg.transactionOpeningCompleted === false,
      ...
      entryCountOne: reg.transactionOpeningReadinessCount === 1,
      noDuplicateEntry: reg.duplicateTransactionOpeningReadinessCount === 0,
    ...
    const activationTransactionOpeningReadinessMetaOk =
      probe.version === 31 &&
      reg.phase === "3B.3.30" &&
      ...
      reg.activationTransactionOpeningReadinessContractId === "feed.discovery.adaptive-workspace.host-activation-transaction-opening-readiness.contract.v1" &&
      reg.transactionOpeningReadinessState === "TRANSACTION_OPENING_READY_NOT_OPENED" &&
      reg.transactionOpeningReadinessResult === "controlled-workspace-host-activation-transaction-opening-ready-not-opened" &&
      reg.transactionOpeningReadinessCompleted === true &&
      ...
      reg.grantCount === 1 &&
      reg.transactionOpeningReadinessCount === 1 &&
      reg.duplicateTransactionOpeningReadinessCount === 0 &&
      ...
      reg.activationBlocker === "PHASE_3B3_30_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_READINESS_ONLY" &&
    ...
    const anyFail =
      ...
      !activationTransactionOpeningReadinessMetaOk ||
    ...
    const artifact = {
      ...
      controlledWorkspaceHostActivationTransactionOpeningReadiness: reg,
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_31" : "READY_FOR_PHASE_3B_3_31",
    };

    const proofPath = join(outDir, "phase3b3-30-controlled-workspace-host-activation-transaction-opening-readiness-proof.json");
    ...
    const prepared = {
      ...
      activationTransactionOpeningReadinessId: "feed.discovery.adaptive-workspace.host-activation-transaction-opening-readiness.v1",
    ...
    writeFileSync(
      join(outDir, "phase3b3-30-controlled-workspace-host-activation-transaction-opening-readiness-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.30 Controlled Workspace Host Activation Transaction Opening Readiness Proof Summary",
      "",
      `- Verdict: **${artifact.overallVerdict}**`,
      `- Commit: \`${commit}\``,
      `- Branch: \`${branch}\``,
      `- Commit-boundary entry: result=${reg.transactionOpeningReadinessResult} entered=${reg.activationCommitBoundaryEntered} state=${reg.activationCommitBoundaryState}`,
      ...
      `- activationTransactionOpeningReadinessMetaOk: ${activationTransactionOpeningReadinessMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-30-controlled-workspace-host-activation-transaction-opening-readiness-summary.md"),
      summary,
    );
