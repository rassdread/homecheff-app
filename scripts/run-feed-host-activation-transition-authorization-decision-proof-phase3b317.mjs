#!/usr/bin/env node
/**
 * Phase 3B.3.17 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.17 host activation transition authorization decision Chromium proof (new)
 */
...
const port = process.env.PHASE3B317_PORT || "3038";
...
mkdirSync(out3b317, { recursive: true });
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b317] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b317] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    if (preflightProof.overallVerdict !== "READY_FOR_PHASE_3B_3_17") {
      throw new Error(
        `Phase 3B.3.16 preflight proof required: ${preflightProof.overallVerdict}`,
      );
    }
    ...
    console.log(
      "[phase3b317] Phase 3B.3.17 host activation transition authorization decision proof",
    );
    await run("node", [
    ...
    if (p317.overallVerdict !== "READY_FOR_PHASE_3B_3_18") {
      throw new Error(`Phase 3B.3.17 proof failed: ${p317.overallVerdict}`);
    }
    if (p317.commit !== commit) {
      throw new Error(
        `Proof commit mismatch: artifact=${p317.commit} HEAD=${commit}`,
      );
    }

    console.log("[phase3b317] READY_FOR_PHASE_3B_3_18");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b317, "phase3b3-17-server-log-tail.txt"),
