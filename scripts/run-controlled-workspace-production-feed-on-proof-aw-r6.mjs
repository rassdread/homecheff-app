...
const port = process.env.AW_R6_PORT || "3074";
...
try {
  ...
  await run("node", [
    "scripts/probe-controlled-workspace-production-feed-on-aw-r6.mjs",
    `--base-url=${baseUrl}`,
    `--commit=${commit}`,
    "--branch=workspace/aw-r6-production-freeze-feed-on",
    `--out-dir=${outDir}`,
  ]);
  console.log("[aw-r6] ADAPTIVE_WORKSPACE_PRODUCTION_LIVE_FROZEN");
} finally {
  server.kill("SIGTERM");
}
