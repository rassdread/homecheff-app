/**
 * Phase 3B.3.33 static validator — controlled workspace host activation transaction-preparation.
 */
...
const PREDECESSOR_PHASE = "3B.3.34";
const PREDECESSOR_HEAD =
  "a4ddf4e5f721ed221ac4b8a2bb96b7b93f55e6d0";
const PREDECESSOR_PROOF_TARGET =
  "8bf7ba819ad03af183d4e34f9326f985f3afc087";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-transaction-preparation-authorized-not-prepared";
const PREDECESSOR_LIFECYCLE = "TRANSACTION_PREPARATION_AUTHORIZED_NOT_PREPARED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_35";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-preparation.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-transaction-preparation-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-preparation-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-transaction-preparation-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-transaction-preparation-phase3b335.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-transaction-preparation-proof-phase3b335.mjs",
);
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b335/phase3b3-35-controlled-workspace-host-activation-transaction-preparation-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b335-controlled-workspace-host-activation-transaction-preparation.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B335_ARTIFACTS === "1") {
...
assert.equal(host.nextEligibleStep, "3B.3.36");
assert.ok(
...
const descriptor =
  createControlledWorkspaceHostActivationTransactionPreparationDescriptor();
assert.equal(descriptor.phase, "3B.3.35");
assert.equal(
  descriptor.transactionPreparationResult,
  "controlled-workspace-host-activation-transaction-prepared-not-committed",
);
assert.equal(
  descriptor.transactionPreparationState,
...
assert.equal(descriptor.transactionPreparationCount, 1);
assert.equal(descriptor.duplicateTransactionPreparationCount, 0);
...
assert.equal(
  descriptor.predecessorActivationTransactionPreparationAuthorizationState,
  PREDECESSOR_LIFECYCLE,
);
assert.equal(
  descriptor.predecessorActivationTransactionPreparationAuthorizationResult,
  PREDECESSOR_RESULT,
);
...
assert.equal(descriptor.nextEligibleStep, "3B.3.36");
assert.equal(
...
rejectUnresolved("descriptor.transactionPreparationResult", descriptor.transactionPreparationResult);
...
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.35");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.36");
assert.equal(
...
assert.equal(gate.currentStep, "3B.3.35");
assert.equal(gate.eligibleStep, "3B.3.36");
assert.ok(
...
assert.equal(FEED_DISCOVERY_HOST_CANDIDATE_METADATA.nextEligibleStep, "3B.3.36");
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_36");
  assert.equal(proof.transactionPreparedMetaOk, true);
  ...
  validateFeedWorkspaceHostActivationTransactionPreparationPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-activation-transaction-preparation-phase3b335: PASS",
);
