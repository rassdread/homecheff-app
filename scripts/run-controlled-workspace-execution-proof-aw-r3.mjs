...
const port = process.env.AW_R3_PORT || "3071";
const outDir = join(root, "docs/audits/artifacts/aw-r3");
...
try {
  ...
  await run("node", [
    "scripts/probe-controlled-workspace-execution-aw-r3.mjs",
    `--base-url=${baseUrl}`,
    `--commit=${commit}`,
    "--branch=workspace/aw-r3-controlled-execution",
    `--out-dir=${outDir}`,
  ]);
  console.log("[aw-r3] READY_FOR_AW_R4");
} finally {
  server.kill("SIGTERM");
}
