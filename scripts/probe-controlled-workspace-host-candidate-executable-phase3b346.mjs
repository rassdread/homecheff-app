#!/usr/bin/env node
/**
 * Phase 3B.3.46 — Chromium proof: Controlled Workspace Host Candidate Executable Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3067";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b346");
  for (const a of argv) {
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_45") {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const candidateActivationReadiness = await api.readControlledWorkspaceHostCandidateExecutable();
    ...
    const reg = probe.candidateActivationReadiness;
    ...
    const candidateExecutableMetaOk =
      probe.version === 47 &&
      reg.phase === "3B.3.46" &&
      ...
      reg.candidateActivationState === "CANDIDATE_EXECUTABLE_NOT_EXECUTED" &&
      reg.candidateActivationResult === "controlled-workspace-host-candidate-executable-not-executed" &&
      ...
      reg.nextEligibleStep === "3B.3.47" &&
      reg.activationBlocker === "PHASE_3B3_46_CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_ONLY" &&
    ...
    const artifact = {
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_47" : "READY_FOR_PHASE_3B_3_47",
    };

    const proofPath = join(outDir, "phase3b3-46-controlled-workspace-host-candidate-executable-proof.json");
    writeFileSync(proofPath, JSON.stringify(artifact, null, 2) + "\n");

    const prepared = {
      schemaVersion: 1,
      phase: "3B.3.46",
      status: "controlled-workspace-host-candidate-executable-prepared",
    ...
    writeFileSync(
      join(outDir, "phase3b3-46-controlled-workspace-host-candidate-executable-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.46 Controlled Workspace Host Candidate Executable Authorization Proof Summary",
      "",
      ...
      `- candidateExecutableMetaOk: ${candidateExecutableMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-46-controlled-workspace-host-candidate-executable-summary.md"),
      summary,
    );
