#!/usr/bin/env node
/**
 * WX Phase 1B.5.6 — Honesty Density & Compacted States browser proof.
 * Asserts honesty diagnostics present, zero DOM/layout/chrome delta, no remount.
 */
...
function parseArgs(argv) {
  ...
  let outDir = join(process.cwd(), "docs/audits/wx-phase1b5-6-honesty-density");
  let journey = false;
  for (const a of argv) {
...
async function snap(page) {
  return page.evaluate(() => {
    ...
    const newHonestyUi = document.querySelector(
      "[data-wx-density-panel],[data-wx-compact-ui],[data-wx-honesty-ui],[data-wx-density-overlay]",
    );
    ...
    return {
      ...
      noNewHonestyUi: newHonestyUi == null,
      ...
      focusableDiagnostics: [...document.querySelectorAll("[data-wx-honesty]")].some(
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    if (baselineHonestyUi == null) baselineHonestyUi = s.noNewHonestyUi;
    ...
    const checks = {
      phase: s.phase === "1b.5.6",
      honesty: s.honesty === "wx-honesty-density-v1",
      honestyVersion: s.honestyVersion === "1.0.0",
      honestyRendersOff: s.honestyRenders === "0",
      honestyDrivesChromeOff: s.honestyDrivesChrome === "0",
      honestyAppliesCompactionOff: s.honestyAppliesCompaction === "0",
      hasDensity: typeof s.density === "string" && s.density.includes("stage:"),
      hasCompact: typeof s.compact === "string" && s.compact.includes("stage:"),
      honestyStatusOk: s.honestyStatus === "ok",
      ...
      hasToken: Boolean(s.honestyToken),
      hasOrderedIds:
        typeof s.honestyIds === "string" &&
        s.honestyIds.includes("stage") &&
        s.honestyIds.includes("tool"),
      noNewHonestyUi: s.noNewHonestyUi === true,
      noNewDisclosureUi: s.noNewDisclosureUi === true,
      noNewAssistUi: s.noNewAssistUi === true,
      noNewToolUi: s.noNewToolUi === true,
      visibleHonestyDomDeltaZero: s.noNewHonestyUi === baselineHonestyUi,
  ...
  if (args.journey) {
    ...
    journey = {
      ...
      noConsoleErrors: filteredConsole.length === 0,
      noPageErrors: pageErrors.length === 0,
      zeroVisibleHonestyUi: steps.every((s) => s.snap.noNewHonestyUi),
      ...
      pass:
        ...
        steps.every(
          (s) =>
            s.snap.noNewHonestyUi &&
            s.snap.noNewDisclosureUi &&
            s.snap.noNewAssistUi &&
            s.snap.noNewToolUi &&
            s.snap.honestyDrivesChrome === "0" &&
            s.snap.honestyRenders === "0" &&
            s.snap.honestyAppliesCompaction === "0" &&
            s.snap.phase === "1b.5.6",
        ),
    };
    await page.close();
  ...
  const report = {
    ...
    verdict: allPass
      ? "WX_PHASE_1B5_6_BROWSER_PROOF_PASS"
      : "WX_PHASE_1B5_6_BROWSER_PROOF_FAIL",
    passCount: cases.filter((c) => c.pass).length,
    caseCount: cases.length,
    visibleHonestyDomDeltaZero: cases.every((c) => c.checks.visibleHonestyDomDeltaZero),
    ...
    JSON.stringify(
      {
        ...
        visibleHonestyDomDeltaZero: report.visibleHonestyDomDeltaZero,
