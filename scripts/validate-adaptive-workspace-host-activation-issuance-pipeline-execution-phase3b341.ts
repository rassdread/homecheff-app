/**
 * Phase 3B.3.41 static validator — controlled workspace host activation issuance pipeline execution.
 */
...
const PREDECESSOR_PHASE = "3B.3.40";
const PREDECESSOR_BRANCH =
  "workspace/phase3b340-controlled-workspace-host-activation-issuance-pipeline-execution-authorization";
const PREDECESSOR_HEAD =
  "0fcb6c11eabaea50f7c1138db9c170a033e75960";
const PREDECESSOR_PROOF_TARGET =
  "313dd01f0260254f3e46f9a6d6762a1bd5e2a361";
const PREDECESSOR_RESULT =
  "controlled-workspace-host-activation-issuance-pipeline-execution-authorized-not-executed";
const PREDECESSOR_LIFECYCLE = "PIPELINE_EXECUTION_AUTHORIZED_NOT_EXECUTED";
const PREDECESSOR_VERDICT = "READY_FOR_PHASE_3B_3_41";
...
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-issuance-pipeline-execution.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/controlled-workspace-host-activation-issuance-pipeline-execution-contract.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-issuance-pipeline-execution-identity.ts",
);
mustExist(
  "lib/adaptive-workspace/sealed/feed-workspace-host-activation-issuance-pipeline-execution-prepared.ts",
);
mustExist(
  "scripts/probe-controlled-workspace-host-activation-issuance-pipeline-execution-phase3b341.mjs",
);
mustExist(
  "scripts/run-controlled-workspace-host-activation-issuance-pipeline-execution-proof-phase3b341.mjs",
);
...
const preparedPath = join(
  root,
  "docs/audits/artifacts/phase3b341/phase3b3-41-controlled-workspace-host-activation-issuance-pipeline-execution-prepared.json",
);
const auditPath =
  "docs/audits/homecheff-adaptive-workspace-phase3b341-controlled-workspace-host-activation-issuance-pipeline-execution.md";
const artifactsPresent = existsSync(proofPath) && existsSync(preparedPath);
if (!artifactsPresent && process.env.REQUIRE_PHASE3B341_ARTIFACTS === "1") {
  assert.fail("Phase 3B.3.41 proof/prepared artifacts required but missing");
}
if (artifactsPresent || process.env.REQUIRE_PHASE3B341_ARTIFACTS === "1") {
  mustExist(auditPath);
}
...
const descriptor =
  createControlledWorkspaceHostActivationIssuancePipelineExecutionDescriptor();
assert.equal(descriptor.phase, "3B.3.41");
assert.equal(
  descriptor.pipelineExecutionResult,
  "controlled-workspace-host-activation-issuance-pipeline-executed-not-activated",
);
assert.equal(
  descriptor.pipelineExecutionState,
...
assert.equal(descriptor.nextEligibleStep, "3B.3.42");
assert.equal(
...
rejectUnresolved("descriptor.pipelineExecutionResult", descriptor.pipelineExecutionResult);
...
assert.equal(evaluation.diagnostics.currentPhase, "3B.3.41");
assert.equal(evaluation.diagnostics.nextEligibleStep, "3B.3.42");
assert.equal(
...
const failClosedCases: Array<{ label: string; input: Record<string, unknown> }> = [
  ...
  {
    label: "duplicate issuance pipeline execution",
    input: { issuancePipelineExecuted: true },
  },
  {
    label: "missing issuance pipeline execution authorization",
  ...
  },
  {
    ...
    input: { entryRecords: [{}, {}], issuancePipelineExecuted: true },
  },
  {
...
if (artifactsPresent) {
  ...
  assert.equal(proof.overallVerdict, "READY_FOR_PHASE_3B_3_42");
  assert.equal(proof.issuancePipelineExecutedMetaOk, true);
  ...
  validateFeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract(prepared);
...
console.log(
  "validate-adaptive-workspace-host-activation-issuance-pipeline-execution-phase3b341: PASS",
);
