#!/usr/bin/env node
/**
 * Phase 3B.3.45 — Chromium proof: Controlled Workspace Host Candidate Active Authorization.
 */
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3066";
  let commit = "unknown";
  let branch = "BR";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b345");
  for (const a of argv) {
...
async function main() {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      const candidateActivationReadiness = await api.readControlledWorkspaceHostCandidateActive();
    ...
    const forcedNegativeProofs = {
      ...
      candidateStillNonExecutable: reg.candidateExecutable === false,
    ...
    const candidateActiveMetaOk =
      ...
      reg.candidateActivationState === "CANDIDATE_ACTIVE_NOT_EXECUTABLE" &&
      reg.candidateActivationResult === "controlled-workspace-host-candidate-active-not-executable" &&
      ...
      reg.nextEligibleStep === "3B.3.46" &&
      reg.activationBlocker === "PHASE_3B3_45_CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVE_ONLY" &&
    ...
    const artifact = {
      ...
      invariants,
      overallVerdict: anyFail ? "NOT_READY_FOR_PHASE_3B_3_46" : "READY_FOR_PHASE_3B_3_46",
    };

    const proofPath = join(outDir, "phase3b3-45-controlled-workspace-host-candidate-active-proof.json");
    ...
    const prepared = {
      ...
      activationCandidateActivationId: "feed.discovery.adaptive-workspace.host-candidate-active.v1",
    ...
    writeFileSync(
      join(outDir, "phase3b3-45-controlled-workspace-host-candidate-active-prepared.json"),
      JSON.stringify(prepared, null, 2) + "\n",
    );

    const summary = [
      "# Phase 3B.3.45 Controlled Workspace Host Candidate Active Authorization Proof Summary",
      "",
      ...
      `- candidateActiveMetaOk: ${candidateActiveMetaOk}`,
      "",
    ].join("\n");
    writeFileSync(
      join(outDir, "phase3b3-45-controlled-workspace-host-candidate-active-summary.md"),
      summary,
    );
