#!/usr/bin/env node
/**
 * Phase 3B.3.7 — Chromium proof: Controlled Host Activation Decision.
...
function parseArgs(argv) {
  let baseUrl = "http://127.0.0.1:3028";
  let commit = "unknown";
  let branch = "workspace/phase3b37-controlled-host-eligibility";
  let outDir = join(process.cwd(), "docs/audits/artifacts/phase3b37");
  for (const a of argv) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return Boolean(probe) && c?.mountCount >= 1 && probe.version >= 8;
      },
      { timeout: 120000 },
    );
    ...
    pass(
      ...
      "legacy writer; host activation decision; activation blocked",
      JSON.stringify({
        activationAttempt,
    ...
    const decisionMetaOk =
      ...
      hostActivationDecision.confidence === "high" &&
      hostActivationDecision.canStartActivation === false &&
      hostActivationDecision.hostActivation === false &&
      hostActivationDecision.renderActivation === false &&
      hostActivationDecision.activationBlocker ===
        "PHASE_3B3_7_HOST_ACTIVATION_DECISION_ONLY" &&
      hostActivationDecision.nextEligibleStep === "3B.3.8" &&
      hostActivationDecision.runtimeId ===
        "feed.discovery.legacy-single-mount.v1" &&
      hostActivationDecision.diagnostics?.decisionCompleted === true &&
      hostActivationDecision.diagnostics?.decisionResult === "ALLOW" &&
      hostActivationDecision.diagnostics?.wouldActivate === true &&
      hostActivationDecision.diagnostics?.activationBlocked === true &&
      hostActivationDecision.diagnostics?.canStartActivation === false &&
      Array.isArray(hostActivationDecision.decisionReasons) &&
      hostActivationDecision.decisionReasons.length > 0 &&
      shadowPlacement.placementState === "shadow-registered" &&
    ...
    const artifact = {
      ...
      overallVerdict:
        anyFail || !decisionMetaOk
          ? "NOT_READY_FOR_PHASE_3B_3_8"
          : "READY_FOR_PHASE_3B_3_8",
    };

    const outPath = join(
      outDir,
      "phase3b3-7-feed-host-activation-decision-proof.json",
    );
    ...
    const readiness = {
      schemaVersion: 1,
      phase: "3B.3.7",
      status: "host-activation-decision-prepared",
      decisionContract: "valid",
      ...
      browserProof: anyFail || !decisionMetaOk ? "fail" : "pass",
    ...
    writeFileSync(
      join(outDir, "phase3b3-7-feed-host-activation-decision-prepared.json"),
      JSON.stringify(readiness, null, 2),
    );
    writeFileSync(
      join(outDir, "phase3b3-7-feed-host-activation-decision-summary.md"),
      [
        `# Phase 3B.3.7 Host Activation Decision Proof Summary`,
        ``,
        `- Verdict: **${artifact.overallVerdict}**`,
        `- Commit: \`${commit}\``,
        `- Branch: \`${branch}\``,
        `- Decision: result=${hostActivationDecision.decisionResult} wouldActivate=${hostActivationDecision.wouldActivate} confidence=${hostActivationDecision.confidence}`,
        `- Diagnostics: phase=${hostActivationDecision.diagnostics.currentPhase} inputs=${hostActivationDecision.diagnostics.decisionInputSources.length}`,
        `- Registry: hostCount=${hostRegistry.hostCount} runtimeId=${hostRegistry.runtimeId}`,
        `- Host activation: false (blocked by PHASE_3B3_7_HOST_ACTIVATION_DECISION_ONLY); canStartActivation=false`,
    ...
    await browser.close();
    process.exit(anyFail || !decisionMetaOk ? 1 : 0);
  } catch (err) {
    console.error(err);
    try {
      await browser.close();
