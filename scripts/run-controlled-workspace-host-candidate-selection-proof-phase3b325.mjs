#!/usr/bin/env node
/**
 * Phase 3B.3.25 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.25 candidate registration Chromium proof
 */
...
const port = process.env.PHASE3B325_PORT || "3046";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b325 = join(root, "docs/audits/artifacts/phase3b325");
mkdirSync(out3b2, { recursive: true });
mkdirSync(out3b325, { recursive: true });
...
async function main() {
  ...
  if (prior.overallVerdict !== "READY_FOR_PHASE_3B_3_25") {
    throw new Error(`Phase 3B.3.24 proof required: ${prior.overallVerdict}`);
  }

  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b325] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b325] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    const p324 = JSON.parse(
      readFileSync(
        join(
          out3b325,
          "phase3b3-25-controlled-workspace-host-candidate-selection-proof.json",
        ),
        "utf8",
      ),
    );
    if (p324.overallVerdict !== "READY_FOR_PHASE_3B_3_26") {
      throw new Error(`Phase 3B.3.25 proof failed: ${p324.overallVerdict}`);
    }
    if (p324.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p324.commit} HEAD=${commit}`);
    }
    console.log("[phase3b325] READY_FOR_PHASE_3B_3_26");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
      join(out3b325, "phase3b3-24-server-log-tail.txt"),
