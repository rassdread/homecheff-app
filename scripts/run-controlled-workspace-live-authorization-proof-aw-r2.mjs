...
try {
  ...
  await run("node", [
    "scripts/probe-controlled-workspace-live-authorization-aw-r2.mjs",
    `--base-url=${baseUrl}`,
    `--commit=${commit}`,
    "--branch=workspace/aw-r2-controlled-live-authorization",
    `--out-dir=${outDir}`,
  ]);
  console.log("[aw-r2] READY_FOR_AW_R3");
} finally {
  server.kill("SIGTERM");
}
