#!/usr/bin/env node
/**
 * Phase 3B.3.42 orchestrator:
 * 1) production build with sealed baseline
 * 2) Phase 3B.2 Chromium proof rerun (20/20)
 * 3) Phase 3B.3.42 candidate-activation-readiness Chromium proof
 */
...
const port = process.env.PHASE3B342_PORT || "3063";
const out3b2 = join(root, "docs/audits/artifacts/phase3b2");
const out3b333 = join(root, "docs/audits/artifacts/phase3b342");
...
async function main() {
  ...
  if (process.env.SKIP_BUILD !== "1") {
    console.log("[phase3b342] production build NEXT_PUBLIC_FEED_SEALED_BASELINE=1");
  ...
  try {
    ...
    console.log("[phase3b342] Phase 3B.2 proof rerun");
    await run("node", [
    ...
    console.log("[phase3b342] Phase 3B.3.42 candidate-activation-readiness proof");
    await run("node", [
    ...
    if (p333.overallVerdict !== "READY_FOR_PHASE_3B_3_43") {
      throw new Error(`Phase 3B.3.42 proof failed: ${p333.overallVerdict}`);
    }
    if (p333.commit !== commit) {
      throw new Error(`Proof commit mismatch: artifact=${p333.commit} HEAD=${commit}`);
    }
    console.log("[phase3b342] READY_FOR_PHASE_3B_3_43");
  } finally {
    server.kill("SIGTERM");
    writeFileSync(
