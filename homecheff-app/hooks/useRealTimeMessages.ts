...
export function useRealTimeMessages({ conversationId, currentUserId }: UseRealTimeMessagesProps) {
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[useRealTimeMessages] Loaded ${loadedMessages.length} messages`);
      } else {
      ...
      if (response.ok) {
        ...
        if (newMessages.length > 0) {
          ...
          if (latestMessageId !== lastMessageId) {
            ...
            setLastMessageId(latestMessageId);
            setConnectionStatus('connected');
            console.log('[useRealTimeMessages] New messages detected, updating UI');
  ...
  const startPolling = useCallback(() => {
    ...
    console.log('[useRealTimeMessages] Starting polling...');
  ...
  const stopPolling = useCallback(() => {
    if (pollIntervalRef.current) {
      ...
      console.log('[useRealTimeMessages] Polling stopped');
    }
  }, []);
  ...
  useEffect(() => {
    if (connectionStatus === 'disconnected') {
      reconnectTimeoutRef.current = setTimeout(() => {
        console.log('[useRealTimeMessages] Attempting to reconnect...');
        loadMessages();
        startPolling();
