...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("Garden photo upload - Grootte:", Math.round(file.size / 1024), "KB");
    ...
    if (token) {
      try {
        ...
        publicUrl = blob.url;
        console.log("Garden photo upload successful:", blob.url);
      } catch (error) {
    ...
    if (!publicUrl) {
      try {
        ...
        console.log("Using base64 fallback for garden photo");
      } catch (e) {
