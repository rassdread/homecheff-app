    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              console.log(`[ImageOptimization] Original: ${Math.round(file.size / 1024)}KB → Compressed: ${Math.round(blob.size / 1024)}KB (${Math.round((1 - blob.size / file.size) * 100)}% smaller)`);
              resolve(blob);
            } else {
