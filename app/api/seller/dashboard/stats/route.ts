...
export async function GET(request: Request) {
  try {
    ...
    // Get seller profile with subscription
    ...
    // Calculate platform fee percentage (12% default, or subscription fee if applicable)
  ...
  const [sellerProducts, orders, reviews, viewsCount] = await Promise.all([
    ...
    prisma.order.findMany({
      ...
      select: {
        ...
        userId: true,
        createdAt: true,
        stripeSessionId: true, // Need this for mode filtering
        items: {
          where: {
            Product: { sellerId: sellerId }
          },
    ...
    // Get average rating from reviews (only submitted reviews)
