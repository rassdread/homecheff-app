...
export async function POST(req: NextRequest) {
  try {
    ...
    const sellerPrivacy = await prisma.user.findUnique({
    ...
    if (sellerPrivacy) {
      ...
      if (sellerPrivacy.role !== 'ADMIN') {
        if (sellerPrivacy.messagePrivacy === 'NOBODY') {
          return NextResponse.json(
            { error: 'Deze gebruiker accepteert geen berichten' },
            { status: 403 }
          );
        } else if (sellerPrivacy.messagePrivacy === 'FANS_ONLY') {
          // Check if user is a fan of the seller
          const fanRelation = await prisma.follow.findFirst({
            where: {
              sellerId: sellerPrivacy.id,
              followerId: user.id
            }
          });
          if (!fanRelation) {
    ...
    if (initialMessage && conversation) {
      ...
      void notifyConversationMessageRecipients({
        ...
        senderId: user.id,
        text: initialMessage,
      }).catch((e) => console.error('[start-seller] notify', e));
    }
