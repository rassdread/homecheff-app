...
export async function GET(req: Request) {
  try {
    ...
    if (!centralUserId) {
      return NextResponse.json({ ok: false, code: "NO_IDENTITY" }, { status: 422, headers: NO_STORE });
    }
    ...
    return NextResponse.json(json, { status: 200, headers: NO_STORE });
  } catch (err) {
    console.warn("[affiliate/ecosystem-dashboard] failed", err);
    return NextResponse.json(
      {
        ok: false,
        degraded: true,
