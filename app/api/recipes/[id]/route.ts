...
export async function GET(
  ...
  try {
    ...
    if (!recipe) {
      ...
      if (dish) {
        ...
        recipe = {
          ...
          photos: allPhotos,
          videos: dish.videos || [], // Include videos
          sellerProfile: {
            User: dish.user
          },
    ...
    const transformedRecipe = {
      ...
      video: (recipe as any).videos && Array.isArray((recipe as any).videos) && (recipe as any).videos.length > 0 ? {
        url: (recipe as any).videos[0].url,
        thumbnail: (recipe as any).videos[0].thumbnail || null,
        duration: (recipe as any).videos[0].duration || null
      } : null,
