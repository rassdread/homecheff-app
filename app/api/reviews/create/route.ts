...
export async function POST(req: NextRequest) {
  try {
    ...
    if (updatedReview.product.seller?.User?.id) {
      await prisma.notification.create({
        data: {
          id: `review_${updatedReview.id}_${Date.now()}`,
          userId: updatedReview.product.seller.User.id,
          type: 'REVIEW_RECEIVED',
          payload: {
            title: '⭐ Nieuwe review ontvangen!',
            message: `${review.buyer.name || 'Een klant'} heeft een ${rating}-sterren review achtergelaten voor ${updatedReview.product.title}`,
            reviewId: updatedReview.id,
            productId: updatedReview.productId,
            rating: rating,
            link: `/verkoper/products/${updatedReview.productId}`
          }
        }
      });
    }
