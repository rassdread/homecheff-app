...
function LoginForm() {
  ...
  /** Tijdelijke shell-diagnose (geen secrets): adb logcat / Chrome remote — verwijderen na verificatie. */
  useEffect(() => {
    if (!googleAuthChecked || typeof window === "undefined") return;
    if (!isNativeApp()) return;
    const bridge = !!(window as unknown as { androidBridge?: unknown }).androidBridge;
    console.info(
      "[HomeCheff login shell]",
      JSON.stringify({
    ...
    if (provider === "google") {
      ...
      if (
        ...
        setState((prev) => ({
          ...prev,
          isLoading: false,
          error: "Native Google login gebruiken — tik op de knop hierboven.",
          success: false,
        }));
        return;
      }
