...
describe('how-it-works dedicated page', () => {
  ...
  it('NL and EN copy pass without raw keys or false 50% claims', () => {
    ...
    assert.match(earnHowItWorksNl.growth.basisBody, /HC.*ecosysteem|reserveren we eerst|deelbare marge/i);
    assert.match(earnHowItWorksEn.growth.basisBody, /HomeCheff ecosystem|reserve the full face|distributable margin/i);
    assert.match(earnHowItWorksNl.growth.basisBody, /Growth, HomeCheff Studio en op HomeCheff/i);
    assert.match(earnHowItWorksEn.growth.basisBody, /Growth, HomeCheff Studio and on HomeCheff/i);
    assert.match(earnHowItWorksNl.growth.basisBody, /niet uitbetaalbaar als koperssaldo/i);
    assert.match(earnHowItWorksEn.growth.basisBody, /not withdrawable as buyer cash/i);
    assert.equal(PUBLIC_GROWTH_HC_CROSS_PLATFORM_CLAIM, 'ECOSYSTEM');
    assert.doesNotMatch(earnHowItWorksNl.growth.basisBody, /uitbetalen|cash tegoed|withdrawable cash/i);
    assert.doesNotMatch(earnHowItWorksEn.growth.basisBody, /cash payout|withdraw to bank/i);
