/**
 * Phase 3B.3.38 — Workspace host activation transaction-commit identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionCommitIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.38";
...
export function createFeedWorkspaceHostActivationTransactionCommitIdentity(): FeedWorkspaceHostActivationTransactionCommitIdentity {
  return validateFeedWorkspaceHostActivationTransactionCommitIdentity({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_SCHEMA_VERSION,
    phase: "3B.3.38",
...
export function validateFeedWorkspaceHostActivationTransactionCommitIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionCommitIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_INVALID",
      "Commit-readiness identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionCommitIdentity;
  if (c.phase !== "3B.3.38") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_PHASE",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_CHAIN",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_IDENTITY_TRANSFER",
