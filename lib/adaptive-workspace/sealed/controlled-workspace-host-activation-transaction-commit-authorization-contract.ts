/**
 * Phase 3B.3.37 — Controlled Workspace Host Activation Transaction Commit Readiness Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationTransactionCommitAuthorizationContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.37";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_ID;
  ...
  metadataTransactionCommitAuthorizationAllowed: true;
  activationAllowed: false;
  transactionCommitAuthorizationAllowed: false;
  ...
  activationRestriction: typeof PHASE_3B3_37_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_ONLY;
  nextEligibleStep: "3B.3.38";
};

export function createControlledWorkspaceHostActivationTransactionCommitAuthorizationContract(): ControlledWorkspaceHostActivationTransactionCommitAuthorizationContract {
  return validateControlledWorkspaceHostActivationTransactionCommitAuthorizationContract({
    schemaVersion:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_SCHEMA_VERSION,
    phase: "3B.3.37",
    previousPhase: "3B.3.36",
    widgetId: "feed.discovery",
    ...
    activationTransactionCommitAuthorizationId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_ID,
...
export function validateControlledWorkspaceHostActivationTransactionCommitAuthorizationContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationTransactionCommitAuthorizationContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_INVALID",
      "Transaction commit authorization contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostActivationTransactionCommitAuthorizationContract;
  if (
    c.phase !== "3B.3.37" ||
    c.previousPhase !== "3B.3.36" ||
    c.nextEligibleStep !== "3B.3.38"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_PHASE",
      "phase must be 3B.3.37 with previousPhase 3B.3.36",
    );
  }
  if (
    ...
    c.transactionCommitAuthorizationAllowed !== false ||
    c.metadataTransactionCommitAuthorizationAllowed !== true
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_CONTRACT_FLAGS",
