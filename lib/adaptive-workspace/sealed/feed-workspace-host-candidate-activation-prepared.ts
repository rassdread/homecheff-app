/**
 * Phase 3B.3.44 prepared / freeze-for-next-step after candidate-activation.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateActivationPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.44";
...
export function createFeedWorkspaceHostCandidateActivationPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostCandidateActivationPreparedContract {
  return validateFeedWorkspaceHostCandidateActivationPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.44",
    status: "controlled-workspace-host-candidate-activation-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostCandidateActivationPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostCandidateActivationPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.44" ||
    c.status !== "controlled-workspace-host-candidate-activation-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_PHASE",
  ...
  if (
    ...
    c.candidateActivationState !== "CANDIDATE_ACTIVATED_NOT_ACTIVE" ||
    c.candidateActivationResult !==
      "controlled-workspace-host-candidate-activated-not-active"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_FLAGS",
  ...
  if (c.nextEligibleStep !== "3B.3.45") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.45",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostCandidateActivationPreparedContract;
}
