/**
 * Phase 3B.3.45 — Workspace host activation candidate-activation identity contract.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateActiveIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.45";
...
export function createFeedWorkspaceHostCandidateActiveIdentity(): FeedWorkspaceHostCandidateActiveIdentity {
  return validateFeedWorkspaceHostCandidateActiveIdentity({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_SCHEMA_VERSION,
    phase: "3B.3.45",
...
export function validateFeedWorkspaceHostCandidateActiveIdentity(
  candidate: unknown,
): FeedWorkspaceHostCandidateActiveIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_INVALID",
      "Commit-readiness identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostCandidateActiveIdentity;
  if (c.phase !== "3B.3.45") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_PHASE",
      "phase must be 3B.3.45",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_CHAIN",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_IDENTITY_TRANSFER",
