/**
 * Phase 3B.3.45 — Controlled Workspace Host Candidate Active Contract.
 */
...
export type ControlledWorkspaceHostCandidateActiveContract = {
  ...
  activationRestriction: typeof PHASE_3B3_45_CONTROLLED_WORKSPACE_HOST_CANDIDATE_ACTIVE_ONLY;
  nextEligibleStep: "3B.3.46";
};

export function createControlledWorkspaceHostCandidateActiveContract(): ControlledWorkspaceHostCandidateActiveContract {
  return validateControlledWorkspaceHostCandidateActiveContract({
    schemaVersion:
...
export function validateControlledWorkspaceHostCandidateActiveContract(
  candidate: unknown,
): ControlledWorkspaceHostCandidateActiveContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_CONTRACT_INVALID",
      "Transaction commit contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostCandidateActiveContract;
  if (
    c.phase !== "3B.3.45" ||
    c.previousPhase !== "3B.3.44" ||
    c.nextEligibleStep !== "3B.3.46"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_CONTRACT_PHASE",
      "phase must be 3B.3.45 with previousPhase 3B.3.44",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVE_CONTRACT_FLAGS",
