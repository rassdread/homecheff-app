/**
 * Phase 3B.3.42 prepared / freeze-for-next-step after candidate-activation-readiness-authorization.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateActivationReadinessPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.42";
  status: "controlled-workspace-host-candidate-activation-readiness-prepared";
...
export function createFeedWorkspaceHostCandidateActivationReadinessPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostCandidateActivationReadinessPreparedContract {
  return validateFeedWorkspaceHostCandidateActivationReadinessPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.42",
    status: "controlled-workspace-host-candidate-activation-readiness-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostCandidateActivationReadinessPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostCandidateActivationReadinessPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.42" ||
    c.status !== "controlled-workspace-host-candidate-activation-readiness-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_PHASE",
  ...
  if (
    ...
    c.candidateActivationReady !== true ||
    ...
    c.candidateActivationReadinessState !== "CANDIDATE_ACTIVATION_READY_NOT_ACTIVATED" ||
    c.candidateActivationReadinessResult !==
      "controlled-workspace-host-candidate-activation-ready-not-activated"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_FLAGS",
      "Prepared candidate activation readiness readiness must be ready-not-executed without activation or pipeline execution",
    );
  }
  if (c.nextEligibleStep !== "3B.3.43") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.43",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_READINESS_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostCandidateActivationReadinessPreparedContract;
}
