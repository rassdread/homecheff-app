/**
 * Phase 3B.3.32 — Workspace host activation transaction-opening identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionOpeningIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.32";
  ...
  transactionOpeningStartAllowed: false;
...
export function createFeedWorkspaceHostActivationTransactionOpeningIdentity(): FeedWorkspaceHostActivationTransactionOpeningIdentity {
  return validateFeedWorkspaceHostActivationTransactionOpeningIdentity({
    schemaVersion:
...
export function validateFeedWorkspaceHostActivationTransactionOpeningIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionOpeningIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_INVALID",
      "Commit-boundary entry identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionOpeningIdentity;
  if (c.phase !== "3B.3.32") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_PHASE",
      "phase must be 3B.3.32",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_CHAIN",
      "Opening identity must preserve exact opening/authorization chain and legacy authority",
    );
  }
  if (
    ...
    c.transactionOpeningStartAllowed === true ||
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_IDENTITY_TRANSFER",
      "Transaction-opening identity forbids remount, transfer, activation, and further boundary progression",
    );
  }
  return c;
}
