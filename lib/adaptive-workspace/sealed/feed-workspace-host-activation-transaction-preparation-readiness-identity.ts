/**
 * Phase 3B.3.33 — Workspace host activation transaction-preparation-readiness identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionPreparationReadinessIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.33";
  ...
  transactionPreparationStartAllowed: false;
...
export function createFeedWorkspaceHostActivationTransactionPreparationReadinessIdentity(): FeedWorkspaceHostActivationTransactionPreparationReadinessIdentity {
  return validateFeedWorkspaceHostActivationTransactionPreparationReadinessIdentity({
    schemaVersion:
...
export function validateFeedWorkspaceHostActivationTransactionPreparationReadinessIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionPreparationReadinessIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_INVALID",
      "Preparation-readiness identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionPreparationReadinessIdentity;
  if (c.phase !== "3B.3.33") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_PHASE",
      "phase must be 3B.3.33",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_CHAIN",
      "Preparation-readiness identity must preserve exact opening/preparation-readiness chain and legacy authority",
    );
  }
  if (
    ...
    c.transactionPreparationStartAllowed === true ||
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_IDENTITY_TRANSFER",
      "Preparation-readiness identity forbids remount, transfer, activation, and further boundary progression",
    );
  }
  return c;
}
