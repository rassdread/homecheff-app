/**
 * Phase 3B.3.39 prepared / freeze-for-next-step after transaction-commit.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.39";
...
export function createFeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract {
  return validateFeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.39",
    status: "controlled-workspace-host-activation-issuance-pipeline-execution-readiness-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.39" ||
    c.status !== "controlled-workspace-host-activation-issuance-pipeline-execution-readiness-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_PHASE",
  ...
  if (
    ...
    c.pipelineExecutionReadinessState !== "PIPELINE_EXECUTION_READY_NOT_EXECUTED" ||
    c.pipelineExecutionReadinessResult !==
      "controlled-workspace-host-activation-issuance-pipeline-execution-ready-not-executed"
  ) {
    throw new HardContractViolation(
  ...
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_READINESS_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationIssuancePipelineExecutionReadinessPreparedContract;
}
