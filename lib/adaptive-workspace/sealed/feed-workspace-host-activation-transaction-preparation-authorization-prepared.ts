/**
 * Phase 3B.3.34 prepared / freeze-for-next-step after transaction-preparation-authorization.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.34";
...
export function createFeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract {
  return validateFeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract({
    schemaVersion:
    ...
    futureActivationAuthorized: true,
    futureActivationStarted: false,
    predecessorActivationTransactionPreparationReadinessState:
...
export function validateFeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.34" ||
    c.status !== "controlled-workspace-host-activation-transaction-preparation-authorization-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_PHASE",
  ...
  if (
    ...
    c.transactionPreparationAuthorizationState !== "TRANSACTION_PREPARATION_AUTHORIZED_NOT_PREPARED" ||
    c.transactionPreparationAuthorizationResult !==
      "controlled-workspace-host-activation-transaction-preparation-authorized-not-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_FLAGS",
      "Prepared preparation-authorization must be ready-not-prepared without activation or pipeline execution",
    );
  }
  if (c.nextEligibleStep !== "3B.3.35") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.35",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationTransactionPreparationAuthorizationPreparedContract;
}
