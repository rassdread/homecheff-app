/**
 * Phase 3B.3.47 prepared / freeze-for-next-step after candidate-activation.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateExecutionStartedPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.47";
...
export function createFeedWorkspaceHostCandidateExecutionStartedPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostCandidateExecutionStartedPreparedContract {
  return validateFeedWorkspaceHostCandidateExecutionStartedPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.47",
    status: "controlled-workspace-host-candidate-execution-started-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostCandidateExecutionStartedPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostCandidateExecutionStartedPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.47" ||
    c.status !== "controlled-workspace-host-candidate-execution-started-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_PHASE",
  ...
  if (
    ...
    c.candidateActivationState !== "CANDIDATE_EXECUTION_STARTED_NOT_EXECUTED" ||
    c.candidateActivationResult !==
      "controlled-workspace-host-candidate-execution-started-not-executed"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_FLAGS",
  ...
  if (c.nextEligibleStep !== "3B.3.48") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.48",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTION_STARTED_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostCandidateExecutionStartedPreparedContract;
}
