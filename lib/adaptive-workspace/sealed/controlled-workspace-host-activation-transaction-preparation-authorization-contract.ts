/**
 * Phase 3B.3.34 — Controlled Workspace Host Activation Transaction Preparation Authorization Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.34";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_CONTRACT_ID;
  ...
  transactionPreparationAuthorizationAllowed: false;
  ...
  activationRestriction: typeof PHASE_3B3_34_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_ONLY;
  nextEligibleStep: "3B.3.35";
};

export function createControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract(): ControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract {
  return validateControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract({
    schemaVersion:
    ...
    activationTransactionPreparationAuthorizationId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_AUTHORIZATION_CONTRACT_ID,
...
export function validateControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract {
  ...
  const c = candidate as ControlledWorkspaceHostActivationTransactionPreparationAuthorizationContract;
  if (
    c.phase !== "3B.3.34" ||
    c.previousPhase !== "3B.3.33" ||
    c.nextEligibleStep !== "3B.3.35"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_CONTRACT_PHASE",
      "phase must be 3B.3.34 with previousPhase 3B.3.33",
    );
  }
  if (
    ...
    c.transactionPreparationAuthorizationAllowed !== false ||
    c.metadataTransactionPreparationAuthorizationAllowed !== true
  ) {
    throw new HardContractViolation(
