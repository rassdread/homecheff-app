/**
 * Phase 3B.3.46 — Controlled Workspace Host Candidate Executable Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostCandidateExecutableContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.46";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_ID;
  ...
  activationRestriction: typeof PHASE_3B3_46_CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_ONLY;
  nextEligibleStep: "3B.3.47";
};

export function createControlledWorkspaceHostCandidateExecutableContract(): ControlledWorkspaceHostCandidateExecutableContract {
  return validateControlledWorkspaceHostCandidateExecutableContract({
    schemaVersion:
      CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_SCHEMA_VERSION,
    phase: "3B.3.46",
    previousPhase: "3B.3.45",
    widgetId: "feed.discovery",
    ...
    activationTransactionCommitId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_ID,
...
export function validateControlledWorkspaceHostCandidateExecutableContract(
  candidate: unknown,
): ControlledWorkspaceHostCandidateExecutableContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_INVALID",
      "Transaction commit contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostCandidateExecutableContract;
  if (
    c.phase !== "3B.3.46" ||
    c.previousPhase !== "3B.3.45" ||
    c.nextEligibleStep !== "3B.3.47"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_PHASE",
      "phase must be 3B.3.46 with previousPhase 3B.3.45",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_CONTRACT_FLAGS",
