/**
 * Phase 3B.3.38 — Controlled Workspace Host Activation Transaction Commit Readiness Contract.
 */
...
import {
  PHASE_3B3_37_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_ONLY,
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationTransactionCommitContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.38";
  ...
  activationRestriction: typeof PHASE_3B3_37_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_ONLY;
  nextEligibleStep: "3B.3.39";
};

export function createControlledWorkspaceHostActivationTransactionCommitContract(): ControlledWorkspaceHostActivationTransactionCommitContract {
  return validateControlledWorkspaceHostActivationTransactionCommitContract({
    schemaVersion:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_SCHEMA_VERSION,
    phase: "3B.3.38",
    previousPhase: "3B.3.37",
    widgetId: "feed.discovery",
...
export function validateControlledWorkspaceHostActivationTransactionCommitContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationTransactionCommitContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_INVALID",
      "Transaction commit contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostActivationTransactionCommitContract;
  if (
    c.phase !== "3B.3.38" ||
    c.previousPhase !== "3B.3.37" ||
    c.nextEligibleStep !== "3B.3.39"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_PHASE",
      "phase must be 3B.3.39 with previousPhase 3B.3.37",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_CONTRACT_FLAGS",
