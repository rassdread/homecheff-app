/**
 * Phase 3B.3.31 — Workspace host activation transaction-opening authorization identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.31";
...
export function createFeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity(): FeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity {
  return validateFeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity({
    schemaVersion:
...
export function validateFeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_IDENTITY_INVALID",
      "Commit-boundary entry identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionOpeningAuthorizationIdentity;
  if (c.phase !== "3B.3.31") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_IDENTITY_PHASE",
      "phase must be 3B.3.31",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_IDENTITY_TRANSFER",
