/**
 * Phase 3B.3.35 — Workspace host activation transaction-preparation identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionPreparationIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.35";
...
export function createFeedWorkspaceHostActivationTransactionPreparationIdentity(): FeedWorkspaceHostActivationTransactionPreparationIdentity {
  return validateFeedWorkspaceHostActivationTransactionPreparationIdentity({
    schemaVersion:
...
export function validateFeedWorkspaceHostActivationTransactionPreparationIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionPreparationIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_INVALID",
      "Preparation-authorization identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionPreparationIdentity;
  if (c.phase !== "3B.3.35") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_PHASE",
      "phase must be 3B.3.35",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_CHAIN",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_IDENTITY_TRANSFER",
