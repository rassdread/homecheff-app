/**
 * Phase 3B.3.17 readiness / freeze-for-next-step after authorization decision.
 */

import { HardContractViolation } from "../schema/validation-error";

export const FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedHostActivationTransitionAuthorizationDecisionPreparedContract = {
  schemaVersion: typeof FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.17";
...
export function createFeedHostActivationTransitionAuthorizationDecisionPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedHostActivationTransitionAuthorizationDecisionPreparedContract {
  return validateFeedHostActivationTransitionAuthorizationDecisionPreparedContract({
    schemaVersion:
...
export function validateFeedHostActivationTransitionAuthorizationDecisionPreparedContract(
  candidate: unknown,
): FeedHostActivationTransitionAuthorizationDecisionPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.17" ||
    c.status !== "host-activation-transition-authorization-decision-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_PHASE",
      "phase/status mismatch for 3B.3.17",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_ACTIVATION",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_RESULT",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_COUNTS",
      "condition/guard counts must be valid",
    );
  }
  if (c.writer !== "legacy" || c.owner !== "legacy" || c.renderer !== "legacy") {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_OWNER",
      "writer/owner/renderer must be legacy",
    );
  }
  if (c.rollbackFoundation !== "prepared-not-active") {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_ROLLBACK",
      "rollbackFoundation must be prepared-not-active",
    );
  }
  if (c.nextEligibleStep !== "3B.3.18") {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_NEXT",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_MIGRATION",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_PROOF",
  ...
  if (typeof c.evidenceCommit !== "string" || c.evidenceCommit.length < 7) {
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_COMMIT",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_HOST_ACTIVATION_TRANSITION_AUTHORIZATION_DECISION_PREPARED_PATH",
      "evidenceArtifactPath required",
    );
  }
  return c as FeedHostActivationTransitionAuthorizationDecisionPreparedContract;
}
