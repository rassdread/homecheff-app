/**
 * Phase 3B.3.41 — Controlled Workspace Host Activation Issuance Pipeline Execution Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationIssuancePipelineExecutionContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.41";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_ID;
  ...
  activationRestriction: typeof PHASE_3B3_41_CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_ONLY;
  nextEligibleStep: "3B.3.42";
};

export function createControlledWorkspaceHostActivationIssuancePipelineExecutionContract(): ControlledWorkspaceHostActivationIssuancePipelineExecutionContract {
  return validateControlledWorkspaceHostActivationIssuancePipelineExecutionContract({
    schemaVersion:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_SCHEMA_VERSION,
    phase: "3B.3.41",
    previousPhase: "3B.3.40",
    widgetId: "feed.discovery",
    ...
    activationTransactionCommitId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_ID,
...
export function validateControlledWorkspaceHostActivationIssuancePipelineExecutionContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationIssuancePipelineExecutionContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_INVALID",
      "Transaction commit contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostActivationIssuancePipelineExecutionContract;
  if (
    c.phase !== "3B.3.41" ||
    c.previousPhase !== "3B.3.40" ||
    c.nextEligibleStep !== "3B.3.42"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_PHASE",
      "phase must be 3B.3.41 with previousPhase 3B.3.40",
    );
  }
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_CONTRACT_FLAGS",
