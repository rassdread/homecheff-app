/**
 * Phase 3B.3.31 — Controlled Workspace Host Activation Transaction Opening Authorization Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.31";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_ID;
  ...
  metadataTransactionOpeningAuthorizationAllowed: true;
  activationAllowed: false;
  transactionOpeningAuthorizationAllowed: false;
  ...
  activationRestriction: typeof PHASE_3B3_31_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_ONLY;
  nextEligibleStep: "3B.3.32";
};

export function createControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract(): ControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract {
  return validateControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract({
    schemaVersion:
    ...
    activationTransactionOpeningAuthorizationId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_ID,
...
export function validateControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_INVALID",
      "Commit-boundary entry contract must be a plain object",
    );
  }
  const c = candidate as ControlledWorkspaceHostActivationTransactionOpeningAuthorizationContract;
  if (
    c.phase !== "3B.3.31" ||
    c.previousPhase !== "3B.3.30" ||
    c.nextEligibleStep !== "3B.3.32"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_PHASE",
      "phase must be 3B.3.31 with previousPhase 3B.3.30",
    );
  }
  if (
    ...
    c.transactionOpeningAuthorizationAllowed !== false ||
    c.metadataTransactionOpeningAuthorizationAllowed !== true
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_AUTHORIZATION_CONTRACT_FLAGS",
