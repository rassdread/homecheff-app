/**
 * Phase 3B.3.35 — Controlled Workspace Host Activation Transaction Preparation Contract.
 */
...
export const CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_CONTRACT_SCHEMA_VERSION =
  1 as const;

export type ControlledWorkspaceHostActivationTransactionPreparationContract = {
  schemaVersion: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_CONTRACT_SCHEMA_VERSION;
  phase: "3B.3.35";
  ...
  contractId: typeof CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_CONTRACT_ID;
  ...
  metadataTransactionPreparationAllowed: true;
  activationAllowed: false;
  transactionPreparationAllowed: false;
  ...
  activationRestriction: typeof PHASE_3B3_35_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_ONLY;
  nextEligibleStep: "3B.3.36";
};

export function createControlledWorkspaceHostActivationTransactionPreparationContract(): ControlledWorkspaceHostActivationTransactionPreparationContract {
  return validateControlledWorkspaceHostActivationTransactionPreparationContract({
    schemaVersion:
    ...
    activationTransactionPreparationId:
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_ID,
    contractId: CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_CONTRACT_ID,
...
export function validateControlledWorkspaceHostActivationTransactionPreparationContract(
  candidate: unknown,
): ControlledWorkspaceHostActivationTransactionPreparationContract {
  ...
  const c = candidate as ControlledWorkspaceHostActivationTransactionPreparationContract;
  if (
    c.phase !== "3B.3.35" ||
    c.previousPhase !== "3B.3.34" ||
    c.nextEligibleStep !== "3B.3.36"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_CONTRACT_PHASE",
      "phase must be 3B.3.35 with previousPhase 3B.3.34",
    );
  }
  if (
    ...
    c.transactionPreparationAllowed !== false ||
    c.metadataTransactionPreparationAllowed !== true
  ) {
    throw new HardContractViolation(
