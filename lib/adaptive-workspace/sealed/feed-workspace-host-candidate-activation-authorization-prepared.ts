/**
 * Phase 3B.3.43 prepared / freeze-for-next-step after candidate-activation-readiness-authorization.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateActivationAuthorizationPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.43";
...
export function createFeedWorkspaceHostCandidateActivationAuthorizationPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostCandidateActivationAuthorizationPreparedContract {
  return validateFeedWorkspaceHostCandidateActivationAuthorizationPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.43",
    status: "controlled-workspace-host-candidate-activation-authorization-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostCandidateActivationAuthorizationPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostCandidateActivationAuthorizationPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.43" ||
    c.status !== "controlled-workspace-host-candidate-activation-authorization-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_PHASE",
  ...
  if (
    ...
    c.candidateActivationAuthorizationState !== "CANDIDATE_ACTIVATION_AUTHORIZED_NOT_ACTIVATED" ||
    c.candidateActivationAuthorizationResult !==
      "controlled-workspace-host-candidate-activation-authorized-not-activated"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_FLAGS",
  ...
  if (c.nextEligibleStep !== "3B.3.44") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.44",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_ACTIVATION_AUTHORIZATION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostCandidateActivationAuthorizationPreparedContract;
}
