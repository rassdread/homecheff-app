/**
 * Phase 3B.3.37 prepared / freeze-for-next-step after transaction-commit-authorization.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.37";
...
export function createFeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract {
  return validateFeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.37",
    status: "controlled-workspace-host-activation-transaction-commit-authorization-prepared",
    transactionCommitAuthorizationContract: "valid",
    identityContract: "valid",
    ...
    predecessorActivationTransactionCommitReadinessState:
...
export function validateFeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.37" ||
    c.status !== "controlled-workspace-host-activation-transaction-commit-authorization-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_PHASE",
  ...
  if (
    ...
    c.transactionCommitAuthorizationState !== "TRANSACTION_COMMIT_AUTHORIZED_NOT_COMMITTED" ||
    c.transactionCommitAuthorizationResult !==
      "controlled-workspace-host-activation-transaction-commit-authorized-not-committed"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_FLAGS",
      "Prepared commit-authorization must be commit-authorized-not-committed without activation or pipeline execution",
    );
  }
  if (c.nextEligibleStep !== "3B.3.38") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.38",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationTransactionCommitAuthorizationPreparedContract;
}
