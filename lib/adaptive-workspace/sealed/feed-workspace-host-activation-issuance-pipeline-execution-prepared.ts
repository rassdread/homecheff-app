/**
 * Phase 3B.3.41 prepared / freeze-for-next-step after issuance-pipeline-execution-authorization.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.41";
...
export function createFeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract {
  return validateFeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.41",
    status: "controlled-workspace-host-activation-issuance-pipeline-execution-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.41" ||
    c.status !== "controlled-workspace-host-activation-issuance-pipeline-execution-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_PHASE",
  ...
  if (
    ...
    c.pipelineExecutionState !== "PIPELINE_EXECUTED_NOT_ACTIVATED" ||
    c.pipelineExecutionResult !==
      "controlled-workspace-host-activation-issuance-pipeline-executed-not-activated"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_FLAGS",
      "Prepared issuance pipeline execution readiness must be ready-not-executed without activation or pipeline execution",
    );
  }
  if (c.nextEligibleStep !== "3B.3.42") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.42",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationIssuancePipelineExecutionPreparedContract;
}
