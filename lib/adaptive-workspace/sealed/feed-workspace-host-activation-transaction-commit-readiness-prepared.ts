/**
 * Phase 3B.3.36 prepared / freeze-for-next-step after transaction-commit-readiness.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.36";
...
export function createFeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract {
  return validateFeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.36",
    status: "controlled-workspace-host-activation-transaction-commit-readiness-prepared",
    transactionCommitReadinessContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.36" ||
    c.status !== "controlled-workspace-host-activation-transaction-commit-readiness-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_PHASE",
  ...
  if (
    ...
    c.transactionCommitReadinessState !== "TRANSACTION_COMMIT_READY_NOT_COMMITTED" ||
    c.transactionCommitReadinessResult !==
      "controlled-workspace-host-activation-transaction-commit-ready-not-committed"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_FLAGS",
      "Prepared commit-readiness must be commit-ready-not-committed without activation or pipeline execution",
    );
  }
  if (c.nextEligibleStep !== "3B.3.37") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.37",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationTransactionCommitReadinessPreparedContract;
}
