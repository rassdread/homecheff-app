/**
 * Phase 3B.3.40 prepared / freeze-for-next-step after transaction-commit.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.40";
...
export function createFeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract {
  return validateFeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.40",
    status: "controlled-workspace-host-activation-issuance-pipeline-execution-authorization-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract(
  candidate: unknown,
): FeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.40" ||
    c.status !== "controlled-workspace-host-activation-issuance-pipeline-execution-authorization-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_PHASE",
  ...
  if (
    ...
    c.pipelineExecutionAuthorizationState !== "PIPELINE_EXECUTION_AUTHORIZED_NOT_EXECUTED" ||
    c.pipelineExecutionAuthorizationResult !==
      "controlled-workspace-host-activation-issuance-pipeline-execution-authorized-not-executed"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_FLAGS",
  ...
  if (c.nextEligibleStep !== "3B.3.41") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.41",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_ISSUANCE_PIPELINE_EXECUTION_AUTHORIZATION_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostActivationIssuancePipelineExecutionAuthorizationPreparedContract;
}
