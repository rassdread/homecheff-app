/**
 * Phase 3B.3.37 — Workspace host activation transaction-commit-authorization identity contract.
 */
...
export const FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_SCHEMA_VERSION;
  phase: "3B.3.37";
  ...
  transactionCommitAuthorizeAllowed: false;
...
export function createFeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity(): FeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity {
  return validateFeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity({
    schemaVersion:
      FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_SCHEMA_VERSION,
    phase: "3B.3.37",
...
export function validateFeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity(
  candidate: unknown,
): FeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_INVALID",
      "Commit-readiness identity must be a plain object",
    );
  }
  const c = candidate as FeedWorkspaceHostActivationTransactionCommitAuthorizationIdentity;
  if (c.phase !== "3B.3.37") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_PHASE",
      "phase must be 3B.3.37",
    );
  }
  if (
    ...
    c.activationTransactionPreparationId !==
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_ID ||
    c.activationTransactionCommitAuthorizationId !==
      CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_ID ||
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_CHAIN",
  ...
  if (
    ...
    c.transactionCommitAuthorizeAllowed === true ||
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_AUTHORIZATION_IDENTITY_TRANSFER",
