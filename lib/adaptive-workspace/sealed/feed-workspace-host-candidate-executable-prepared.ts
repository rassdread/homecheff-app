/**
 * Phase 3B.3.46 prepared / freeze-for-next-step after candidate-activation.
 */
...
export const FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_SCHEMA_VERSION =
  1 as const;

export type FeedWorkspaceHostCandidateExecutablePreparedContract = {
  schemaVersion: typeof FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_SCHEMA_VERSION;
  phase: "3B.3.46";
...
export function createFeedWorkspaceHostCandidateExecutablePreparedContract(args: {
  evidenceCommit: string;
  evidenceArtifactPath: string;
  conditionCount: number;
  satisfiedConditionCount: number;
  guardCount: number;
  satisfiedGuardCount: number;
}): FeedWorkspaceHostCandidateExecutablePreparedContract {
  return validateFeedWorkspaceHostCandidateExecutablePreparedContract({
    schemaVersion:
      FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_SCHEMA_VERSION,
    phase: "3B.3.46",
    status: "controlled-workspace-host-candidate-executable-prepared",
    transactionCommitContract: "valid",
    identityContract: "valid",
...
export function validateFeedWorkspaceHostCandidateExecutablePreparedContract(
  candidate: unknown,
): FeedWorkspaceHostCandidateExecutablePreparedContract {
  if (candidate === null || typeof candidate !== "object") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_INVALID",
  ...
  if (
    ...
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_SCHEMA",
      "Unsupported prepared schemaVersion",
    );
  }
  if (
    c.phase !== "3B.3.46" ||
    c.status !== "controlled-workspace-host-candidate-executable-prepared"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_PHASE",
  ...
  if (
    ...
    c.candidateActivationState !== "CANDIDATE_EXECUTABLE_NOT_EXECUTED" ||
    c.candidateActivationResult !==
      "controlled-workspace-host-candidate-executable-not-executed"
  ) {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_FLAGS",
  ...
  if (c.nextEligibleStep !== "3B.3.47") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_NEXT",
      "nextEligibleStep must be 3B.3.47",
    );
  }
  if (c.issuanceCommitBoundaryState !== "NOT_ENTERED") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_BOUNDARY",
      "Issuance commit boundary must remain frozen at NOT_ENTERED",
    );
  }
  if (c.browserProof !== "pass" || c.existing20Invariants !== "pass") {
    throw new HardContractViolation(
      "FEED_WORKSPACE_HOST_CANDIDATE_EXECUTABLE_PREPARED_PROOF",
      "Browser proof and invariants must pass",
    );
  }
  return c as unknown as FeedWorkspaceHostCandidateExecutablePreparedContract;
}
