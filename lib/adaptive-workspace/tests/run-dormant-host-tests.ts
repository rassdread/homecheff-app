...

{
  ...
  assert.equal(a.nextEligibleStep, "3B.3.2");
...

{
  ...
  assert.equal(gate.currentStep, "3B.3.1");
  assert.equal(gate.eligibleStep, "3B.3.2");
...

{
  ...
  assert.ok(bad.blockers.includes(PHASE_3B3_1_DORMANT_HOST_ONLY));
...

{
  ...
  assert.equal(
    ...
    "3B.3.2-controlled-host-shadow-placement",
  );
