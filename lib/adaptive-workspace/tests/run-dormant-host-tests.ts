...

{
  ...
  assert.ok(gate.blockers.includes(PHASE_3B3_33_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_ONLY));
...

{
  ...
  assert.ok(bad.blockers.includes(PHASE_3B3_33_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_PREPARATION_READINESS_ONLY));
...

{
  ...
  assert.equal(
    ...
    "3B.3.34-controlled-workspace-host-activation",
  );
