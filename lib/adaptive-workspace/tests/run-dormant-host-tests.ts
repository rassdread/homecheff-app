...

{
  ...
  assert.ok(gate.blockers.includes(PHASE_3B3_36_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_ONLY));
...

{
  ...
  assert.ok(bad.blockers.includes(PHASE_3B3_36_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_COMMIT_READINESS_ONLY));
...

{
  ...
  assert.equal(
    ...
    "3B.3.37-controlled-workspace-host-activation",
  );
