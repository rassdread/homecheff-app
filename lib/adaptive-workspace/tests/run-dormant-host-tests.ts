...

{
  ...
  assert.ok(gate.blockers.includes(PHASE_3B3_32_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_ONLY));
...

{
  ...
  assert.ok(bad.blockers.includes(PHASE_3B3_32_CONTROLLED_WORKSPACE_HOST_ACTIVATION_TRANSACTION_OPENING_ONLY));
...

{
  ...
  assert.equal(
    ...
    "3B.3.33-controlled-workspace-host-activation",
  );
