...
import { CAPGO_GOOGLE_SERVER_CLIENT_ID } from '@/lib/native/google-sign-in-config';
...
export async function ensureGoogleSocialLoginInitialized(): Promise<boolean> {
  if (typeof window === 'undefined' || !CAPGO_GOOGLE_SERVER_CLIENT_ID) return false;
  ...
  initPromise = (async () => {
    try {
      ...
      await SocialLogin.initialize({
        google: {
          webClientId: CAPGO_GOOGLE_SERVER_CLIENT_ID,
          mode: 'online',
        },
      });
