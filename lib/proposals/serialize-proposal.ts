...
export function serializeCommunityOrder(row: CommunityOrder): CommunityOrderDTO {
  return {
    ...
    confirmedScheduleDate: row.confirmedScheduleDate?.toISOString() ?? null,
    confirmedScheduleTimeWindow: row.confirmedScheduleTimeWindow ?? null,
    locationCompletedAt: row.locationCompletedAt?.toISOString() ?? null,
    locationCompletedById: row.locationCompletedById ?? null,
