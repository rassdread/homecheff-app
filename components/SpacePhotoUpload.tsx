    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mb-3">
          {t('spacePhotos.description', { maxPhotos })}
        </p>
      </div>
            ...
            >
              {uploading ? (
                <>
                  ...
                  {t('spacePhotos.uploading')}
                </>
              ) : (
                <>
                  <Upload className="w-4 h-4 mr-2" />
                  {t('spacePhotos.addPhotos')}
                </>
              )}
            </label>
            <span className="text-sm text-gray-600">
              {t('spacePhotos.photoCount', { current: photos.length, max: maxPhotos })}
            </span>
          </div>
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="flex items-center mb-3">
            ...
            <span className="text-sm font-medium text-gray-700">
              {t('spacePhotos.uploadedPhotos', { count: photos.length })}
            </span>
          </div>
              ...
              >
                ...
                {!photo.uploading && (
                  <button
                    ...
                    aria-label={t('spacePhotos.removePhoto')}
                  >
                    <X className="w-3 h-3" />
                  </button>
                )}
      ...
      {photos.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">{t('spacePhotos.noPhotos')}</p>
          <p className="text-xs">{t('spacePhotos.noPhotosDescription')}</p>
        </div>
      )}
    </div>
  );
