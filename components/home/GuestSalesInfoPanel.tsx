...
export default function GuestSalesInfoPanel({ panel, onClose }: Props) {
  ...
  const base = `guestSalesPanels.${panel}`;
  const bullets =
    panel === 'share'
      ? ([1, 2, 3] as const).map((i) => t(`${base}.bullet${i}`)).filter(Boolean)
      : [];
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="guest-sales-panel-title"
      onClick={(e) => {
    ...
    >
      <div className="w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl bg-white shadow-xl border border-gray-200 overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 flex items-start justify-between gap-3 border-b border-gray-100 bg-white px-5 py-4">
          <h2 id="guest-sales-panel-title" className="text-lg font-bold text-gray-900 pr-2">
            {t(`${base}.title`)}
          </h2>
          <button
            type="button"
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700 leading-relaxed">{t(`${base}.body`)}</p>
          {bullets.length > 0 ? (
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-2">
            ...
            >
              {t('guestSalesPanels.ctaRegister')}
            </Link>
            <Link
              href="/login"
              onClick={onClose}
