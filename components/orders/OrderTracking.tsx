...
export default function OrderTracking({ orderId }: OrderTrackingProps) {
  ...
  const getStatusInfo = (status: string): OrderStatus => {
    const statusMap: Record<string, OrderStatus> = {
      PENDING: {
        id: 'pending',
        status: t('order.statusPending'),
        timestamp: order?.createdAt || '',
        description: t('order.descriptionPending'),
        icon: <Clock className="w-5 h-5" />
      },
      CONFIRMED: {
        id: 'confirmed',
        status: t('order.statusConfirmed'),
        timestamp: order?.confirmedAt || '',
        description: t('order.descriptionConfirmed'),
        icon: <CheckCircle className="w-5 h-5" />
      },
      PROCESSING: {
        id: 'processing',
        status: t('order.statusProcessing'),
        timestamp: order?.processingAt || '',
        description: t('order.descriptionProcessing'),
        icon: <Package className="w-5 h-5" />
      },
      SHIPPED: {
        id: 'shipped',
        status: t('order.statusShipped'),
        timestamp: order?.shippedAt || '',
        description: t('order.descriptionShipped'),
        icon: <Truck className="w-5 h-5" />
      },
      DELIVERED: {
        id: 'delivered',
        status: t('order.statusDelivered'),
        timestamp: order?.deliveredAt || '',
        description: t('order.descriptionDelivered'),
        icon: <CheckCircle className="w-5 h-5" />
      },
      CANCELLED: {
        id: 'cancelled',
        status: t('order.statusCancelled'),
        timestamp: order?.cancelledAt || '',
        description: t('order.descriptionCancelled'),
        icon: <Clock className="w-5 h-5" />
      }
    };
