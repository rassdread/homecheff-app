...
export default function VerdienCheckResultSummary(props: {
  ...
  const { route } = props;
  const showCanStart =
    route.proceedSemantics === 'CHECK_FIRST' ||
    route.proceedSemantics === 'INSUFFICIENT_CONTEXT';
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
      {props.omitHeadline ? null : (
        <h2 className="text-2xl font-semibold tracking-tight text-stone-900">{route.headline}</h2>
      )}
      <p className={`${props.omitHeadline ? '' : 'mt-2 '}text-base leading-relaxed text-stone-600`}>
        {route.summary}
      </p>
      {showCanStart ? (
