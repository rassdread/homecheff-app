...
export default function CountrySelector({
  ...
  const getCountryName = (country: { name: string; code: string }) => {
    // If name contains " / ", split and return appropriate language
    if (country.name.includes(' / ')) {
      const [nlName, enName] = country.name.split(' / ');
      return language === 'nl' ? nlName : enName;
    }
    // Otherwise return as is (for countries with same name in both languages)
    return country.name;
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-medium text-gray-900">
              {currentCountry ? getCountryName(currentCountry) : (language === 'nl' ? placeholder : 'Select country')}
            </div>
              ...
              <div className="py-1">
                {filteredCountries.map((country) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="font-medium">{getCountryName(country)}</div>
                        <div className="text-sm text-gray-500">
                ...
                {filteredCountries.length === 0 && (
                  <div className="px-4 py-3 text-gray-500 text-center">
                    {language === 'nl' ? 'Geen landen gevonden' : 'No countries found'}
                  </div>
                )}
              </div>
            ) : (
