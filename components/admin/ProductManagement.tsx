...
export default function ProductManagement() {
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (!confirm(t('errors.confirmDeleteProduct'))) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(t('errors.productDeletedSuccess'));
      } else {
      ...
      alert(t('errors.productDeleteError'));
    } finally {
      setDeleting(null);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(t('errors.confirmDeleteProducts', { count: selectedProducts.length }))) return;
    ...
    try {
      ...
      alert(t('admin.productsDeletedSuccess', { count: selectedProducts.length }));
    } catch (error) {
      console.error('Error bulk deleting products:', error);
      alert(`${t('admin.errorBulkDeleting')}: ${error instanceof Error ? error.message : t('admin.unknownError')}`);
    }
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">{t('admin.productManagement')}</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('admin.noProductsFound')}</h3>
