  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-gray-600">{t('admin.chatManagement.loading')}</span>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                {selectedConversation.product && `${t('admin.chatManagement.about', { title: selectedConversation.product.title })} • `}
                {t('admin.chatManagement.messagesCount', { count: selectedConversation.messageCount, plural: selectedConversation.messageCount !== 1 ? 'en' : '' })}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {selectedConversation.isEncrypted && (
              <span className="px-3 py-1 bg-green-100 text-green-800 text-sm rounded-full flex items-center gap-1">
                ...
                {t('admin.chatManagement.encrypted')}
              </span>
            )}
            <span className={`px-3 py-1 text-sm rounded-full ${
              selectedConversation.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'
            }`}>
              {selectedConversation.isActive ? t('admin.chatManagement.active') : t('admin.chatManagement.inactive')}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-lg font-medium text-gray-900">
              {t('admin.chatManagement.messages', { count: messages.length })}
            </h3>
          </div>
          ...
          {selectedConversation.isEncrypted ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">{t('admin.encryptedChat')}</h3>
              <p className="text-gray-600">
                {t('admin.messagesEncrypted')}<br />
                {t('admin.messagesEncryptedDesc')}
              </p>
              <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4 max-w-md mx-auto">
                <p className="text-sm text-green-800">
                  <strong>{t('admin.metadataAvailable')}</strong><br />
                  • {t('admin.messageCount')} {selectedConversation.messageCount}<br />
                  • {t('admin.lastActive')} {selectedConversation.lastMessageAt ? new Date(selectedConversation.lastMessageAt).toLocaleString(language === 'en' ? 'en-US' : 'nl-NL') : t('common.unknown') || 'Onbekend'}<br />
                  • {t('common.status')}: {selectedConversation.isActive ? t('admin.statusActive') : t('admin.statusInactive')}
                </p>
              </div>
            </div>
          ) : (
            <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto">
              {messages.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">{t('admin.noMessages')}</h3>
                  <p className="text-gray-500">{t('admin.noMessagesDesc')}</p>
                </div>
              ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">{t('admin.chatManagementDesc')}</p>
        </div>
        <button
