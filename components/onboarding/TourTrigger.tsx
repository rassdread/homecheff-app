...
/**
 * Component die een knop toont om de rondleiding te starten
 * De rondleiding start alleen wanneer de gebruiker op de knop klikt - geen automatische start
 */
    ...
    return (
      <>
        ...
        >
          ...
          <span className="text-xs sm:text-sm font-semibold hidden sm:inline whitespace-nowrap">{t('common.startTour')}</span>
        </button>
        {showTour && (
          <OnboardingTour pageId={pageId} autoStart={true} forceStart={true} onComplete={() => setShowTour(false)} />
        )}
      </>
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        <span className="sm:hidden font-semibold">✨ {t('common.startTour')}</span>
      </button>
      {showTour && (
        <OnboardingTour pageId={pageId} autoStart={true} forceStart={true} onComplete={() => setShowTour(false)} />
      )}
