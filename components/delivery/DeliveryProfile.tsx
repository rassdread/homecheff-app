  ...
  const handlePhotoUpload = async (files: FileList) => {
    ...
    const validFiles = fileArray.filter(file => {
      ...
      if (!file.type.startsWith('image/')) {
        alert(t('upload.fileNoImage', { fileName: file.name }));
        return false;
      }
      
      if (file.size > 10 * 1024 * 1024) { // 10MB
        alert(t('upload.fileTooBig', { fileName: file.name }));
        return false;
      }
                  ...
                  <div className="flex items-center gap-2">
                    {deliveryProfile.isVerified ? (
                      ...
                      <>
                        ...
                        <span className="text-orange-600 font-medium">{t('order.statusProcessing')}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div className="p-6">
                {activeTab === 'overview' && (
                  <div className="space-y-6">
                    ...
                    {/* Availability */}
                    <div>
                      <h3 className="font-semibold text-gray-900 mb-3">{t('delivery.availability')}</h3>
