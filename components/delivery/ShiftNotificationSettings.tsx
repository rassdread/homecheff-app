...
export default function ShiftNotificationSettings({ 
  ...
  const addReminder = (minutes: number) => {
    if (reminders.length >= 5) {
      alert(t('errors.maxRemindersAllowed'));
      return;
    }
    if (reminders.includes(minutes)) {
      alert(t('errors.reminderAlreadyExists'));
      return;
    }
  ...
  const removeReminder = (minutes: number) => {
    if (reminders.length <= 2) {
      alert(t('errors.minRemindersRequired'));
      return;
    }
  ...
  const handleAddCustom = () => {
    ...
    if (isNaN(minutes) || minutes < 1 || minutes > 180) {
      alert(t('errors.enterValidMinutes'));
      return;
    }
    addReminder(minutes);
    setCustomMinutes('');
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-blue-50 transition-colors cursor-pointer">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-600">{t('delivery.onlyLastReminder')}</div>
              </div>
            </label>
          </div>
        </div>
