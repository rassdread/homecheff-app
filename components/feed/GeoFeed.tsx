...
export default function GeoFeed() {
  ...
  const { coords, loading: locationLoading, error: locationError, supported: locationSupported, getCurrentPosition } = useGeolocation({
    ...
    timeout: 15000,
    maximumAge: 300000,
    fallbackToManual: false, // No automatic fallback - use profile location instead
    onFallback: (reason) => {
      // Will be handled by fetching user profile location
    }
  });
  
  // Load user's profile location automatically on mount
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (userData.lat && userData.lng) {
            const { lat, lng, place, postcode, address } = userData;
            
            // Save profile location
            setProfileLocation({ place, postcode, lat, lng });
            
            // Automatically use profile location as default start location
            setUserLocation({ lat, lng });
            setLocationSource('profile');
            
            // Set place from profile
            if (place || postcode || address) {
              setPlace(place || postcode || address || '');
            }
          }
        }
      } catch (error) {
  ...
  // Handle GPS failure - fallback to user profile location (only if profile location not already loaded)
  useEffect(() => {
      ...
      if (locationError && !userLocation && !profileLocation && session?.user) {
        try {
          ...
          if (response.ok) {
            const userData = await response.json();
            if (userData.lat && userData.lng) {
              const { lat, lng, place: userPlace, postcode, address } = userData;
              
              setProfileLocation({ place: userPlace, postcode, lat, lng });
              setUserLocation({ lat, lng });
              setLocationSource('profile');
  ...
  <div className="flex flex-wrap gap-3 items-end bg-white/60 rounded-xl p-4 border border-gray-200">
        <div className="flex-1 min-w-[180px]">
          <label className="block text-base font-semibold mb-1">{t('common.place')}</label>
          <input 
            value={place} 
        ...
        <div className="min-w-[120px]">
          <label className="block text-base font-semibold mb-1">{t('common.radius')} (km)</label>
        ...
        <div className="flex-1 min-w-[180px]">
          <label className="block text-base font-semibold mb-1">{t('common.search')}</label>
          <input value={q} onChange={e => setQ(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-primary/40 text-lg placeholder-gray-400" placeholder={t('common.searchPlaceholder')} />
        </div>
        <div className="min-w-[140px]">
          <label className="block text-base font-semibold mb-1">{t('common.category')}</label>
          <select 
            value={category} 
        ...
        <div className="min-w-[120px]">
          <label className="block text-base font-semibold mb-1">{t('common.location')}</label>
          <button
          ...
          >
            ...
            {locationLoading ? t('common.loading') : coords ? t('common.gps') : t('common.location')}
          </button>
        </div>
        <div className="w-full">
          ...
          {!userLocation && !place && (
            <p className="text-xs text-gray-500">
              {locationSupported ? t('common.getLocation') : t('common.typePlaceOrPostcode')}
            </p>
          )}
          {place && (
        ...
        {showFilters && (
          <div className="border-t pt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('common.priceEuro')}</label>
              ...
              <div className="p-3 space-y-1">
                <div className="flex justify-between items-start">
                  <a href={`/listings/${it.id}`} className="flex-1">
                    <p className="font-medium truncate">{it.title ?? t('common.dish')}</p>
                  </a>
                  <ShareButton
                ...
                <div className="flex items-center justify-between text-xs">
                  <p>{it.deliveryMode === "PICKUP" ? t('common.pickup') : it.deliveryMode === "DELIVERY" ? t('common.delivery') : it.deliveryMode === "BOTH" ? t('common.pickupOrDelivery') : ""}</p>
