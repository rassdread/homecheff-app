...
function mapRawFeedApiItems(
  rawItems: Record<string, unknown>[],
  viewerForDistance: ViewerCoords | null,
): FeedItem[] {
...
export default function GeoFeed({
  ...
  useEffect(() => {
    if (sessionStatus === "loading") {
      feedPerfMark("session:loading");
      return;
    }
    feedPerfMark("session:resolved");
  }, [sessionStatus]);

  useEffect(() => {
    if (!session?.user) return;
    if (bootstrapStatus === "loading") {
      feedPerfMark("bootstrap:loading");
      return;
    }
    if (bootstrapStatus === "ready") {
      feedPerfMark("bootstrap:resolved");
    }
  }, [session?.user, bootstrapStatus]);
  ...
  const [nearbyLocationStatus, setNearbyLocationStatus] =
    useState<NearbyLocationStatus>(
      NEARBY_LOCATION_STATUS.NO_LOCATION_SELECTED,
    );
  ...
  useEffect(() => {
    ...
    setNearbyLocationStatus((prev) => {
      if (
        prev === NEARBY_LOCATION_STATUS.GPS_GRANTED ||
        prev === NEARBY_LOCATION_STATUS.LOCATION_READY ||
        prev === NEARBY_LOCATION_STATUS.NEARBY_RESULTS
      ) {
        return prev === NEARBY_LOCATION_STATUS.NEARBY_RESULTS
          ? prev
          : NEARBY_LOCATION_STATUS.LOCATION_READY;
      }
      return NEARBY_LOCATION_STATUS.LOCATION_READY;
    });
  ...
  useEffect(() => {
    if (coords || profileLocation || userLocation) {
      feedPerfMark("location:available");
    }
  }, [coords, profileLocation, userLocation]);

  useEffect(() => {
    if (!feedHydrated || loading || items.length === 0) return;
    feedPerfMarkFirstTileOnce();
    ...
    const t = window.setTimeout(markStable, 800);
    return () => window.clearTimeout(t);
  }, [feedHydrated, loading, items.length]);
          ...
          if (isGeoFeedDiagnosticsEnabled()) {
            ...
            pushGeoFeedDiag({
              ...
              nearbyLocationStatus:
                appliedScope === FEED_SCOPE_NEARBY
                  ? NEARBY_LOCATION_STATUS.NEARBY_RESULTS
                  : null,
            });
          }
          if (process.env.NODE_ENV === "development") {
  ...
  useEffect(() => {
    ...
    if (
      ...
      lastInspiratieFetchKeyRef.current === inspKey
    ) {
      return;
    }
    lastInspiratieFetchKeyRef.current = inspKey;
    ...
    const run = async () => {
      ...
      try {
        const inspRes = await fetch(
          `/api/inspiratie?${inspParams.toString()}`,
        ...
        if ((error as Error)?.name === "AbortError") return;
        console.error("[GeoFeed] deferred inspiratie fetch failed", error);
      }
    };
    void run();
    return () => ac.abort();
      ...
      >
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <button
            ...
            onClick={() => setWorkspaceFiltersExpanded((v) => !v)}
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50 touch-manipulation whitespace-nowrap"
          >
          ...
          <div className="mt-3 space-y-3 border-t border-gray-100 pt-3">
            {feedQuickCreateIntent ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-900 hover:bg-emerald-100 touch-manipulation"
              >
