...
export default function SocialLoginOnboarding() {
  ...
  const handleContactSubmit = () => {
    ...
    // Validate address fields based on country format
    const addressFormat = addressData.country === 'NL' 
      ? 'postcode_house' 
      : addressData.country 
        ? 'street_city' 
        : 'full_address';
    ...
    if (addressFormat === 'postcode_house') {
      if (!addressData.postalCode || !addressData.houseNumber) {
        setContactError('Vul postcode en huisnummer in');
        return;
      }
    } else {
      if (!addressData.address || !addressData.city) {
        setContactError('Vul straat en plaats in');
        return;
      }
    }
