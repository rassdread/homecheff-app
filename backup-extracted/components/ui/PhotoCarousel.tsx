...
interface Photo {
  ...
  fileUrl: string;
  sortOrder?: number;
  type?: 'image' | 'video'; // Future support for videos
  videoUrl?: string; // Future support for video URLs
}
  ...
  return (
    <>
      ...
      <div className={`relative group ${className}`}>
        ...
        <div className="relative aspect-[4/3] sm:aspect-[16/10] lg:aspect-[5/3] bg-gray-100 overflow-hidden">
          {currentPhoto.type === 'video' && currentPhoto.videoUrl ? (
            // Future video support
            <video
              src={currentPhoto.videoUrl}
              controls
              className="w-full h-full object-cover"
              poster={currentPhoto.fileUrl} // Use image as poster
            />
          ) : (
            <SafeImage
      ...
      {isFullscreen && (
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          <div className="relative max-w-7xl max-h-full w-full h-full flex items-center justify-center">
            {currentPhoto.type === 'video' && currentPhoto.videoUrl ? (
              // Future video support in fullscreen
              <video
                src={currentPhoto.videoUrl}
                controls
                autoPlay
                className="max-w-full max-h-full object-contain rounded-lg"
                poster={currentPhoto.fileUrl}
              />
            ) : (
              <SafeImage
                src={currentPhoto.fileUrl}
