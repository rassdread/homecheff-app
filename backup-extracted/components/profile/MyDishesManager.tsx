...
import InspiratieManager from "./InspiratieManager";
...
export default function MyDishesManager({ onStatsUpdate, activeRole = 'generic', userId, isPublic = false, role, showOnlyActive = false, contentSubTab = 'dorpsplein' }: MyDishesManagerProps) {
  ...
  const getInitialTab = (): 'dishes' | 'products' | 'inspiratie' => {
    return 'products'; // Always start with Dorpsplein
  };
  
  const [activeTab, setActiveTab] = useState<'dishes' | 'products' | 'inspiratie'>(getInitialTab());
  ...
  // Filter items based on public mode and sub-tab (dorpsplein/inspiratie)
  const filteredItems = useMemo(() => {
    let filtered = isPublic ? items.filter(item => item.status === 'PUBLISHED') : items;
    
    // Filter op basis van sub-tab (voor public mode)
    if (isPublic && contentSubTab) {
      if (contentSubTab === 'dorpsplein') {
        // Dorpsplein: ALLEEN items met prijs > 0 (artikelen te koop)
        filtered = filtered.filter(item => item.priceCents && item.priceCents > 0);
      } else if (contentSubTab === 'inspiratie') {
        // Inspiratie: ALLEEN items zonder prijs (priceCents === 0 of null) EN status PUBLISHED
        filtered = filtered.filter(item => 
          (!item.priceCents || item.priceCents === 0) && item.status === 'PUBLISHED'
        );
      }
    }
    
    return filtered;
  }, [items, isPublic, contentSubTab]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs - only show in private mode AND when contentSubTab is NOT provided */}
        ...
        <div className="bg-white rounded-2xl border">
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8 px-6">
              ...
              <button
                onClick={() => setActiveTab('inspiratie')}
                className={`flex items-center space-x-2 py-4 px-1 border-b-2 font-medium text-sm ${
          ...
          <div className="p-6">
            {/* Dorpsplein products tab - shows products filtered by current role */}
            {activeTab === 'products' && (
              <ProductManagement 
                onUpdate={() => {
                  onStatsUpdate?.();
          ...
          {/* Inspiratie tab - shows inspiration items filtered by category */}
          {activeTab === 'inspiratie' && (
            <InspiratieManager 
              isActive={activeTab === 'inspiratie'} 
              userId={userId}
              isPublic={isPublic}
              categoryFilter={currentRole === 'chef' ? 'CHEFF' : currentRole === 'garden' ? 'GROWN' : currentRole === 'designer' ? 'DESIGNER' : undefined}
            />
          )}
          </div>
        </div>
      ...
      {/* Direct content when contentSubTab is provided (no tabs) */}
      {!isPublic && contentSubTab === 'dorpsplein' && (
        <ProductManagement 
          onUpdate={() => {
            onStatsUpdate?.();
          }} 
          categoryFilter={currentRole === 'chef' ? 'CHEFF' : currentRole === 'garden' ? 'GROWN' : currentRole === 'designer' ? 'DESIGNER' : null}
          key={`products-${currentRole}`}
        />
      )}
