...
export default function OrdersPage() {
  ...
  const getStatusDescription = (status: string) => {
    switch (status) {
      case 'PENDING':
        return 'Wachten op bevestiging van de verkoper';
      case 'CONFIRMED':
        return 'Bestelling bevestigd door verkoper';
      case 'PROCESSING':
        return 'Bestelling wordt voorbereid';
      case 'SHIPPED':
        return 'Bestelling is onderweg';
      case 'DELIVERED':
        return 'Bestelling is bezorgd';
      case 'CANCELLED':
        return 'Bestelling is geannuleerd';
      default:
        return '';
    }
  };
  ...
  return (
    <main className="min-h-screen bg-neutral-50">
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Status Filter - Tabs gebaseerd op verkoper status opties */}
        <div className="mb-6" data-tour="order-filter">
          <div className="flex gap-2 flex-wrap">
            {[
              { value: 'all', label: 'Alle', icon: '📦' },
              { value: 'PENDING', label: 'Wachtend', icon: '⏳' },
              { value: 'CONFIRMED', label: 'Bevestigd', icon: '✅' },
              { value: 'PROCESSING', label: 'In behandeling', icon: '🔄' },
              { value: 'SHIPPED', label: 'Onderweg', icon: '🚚' },
