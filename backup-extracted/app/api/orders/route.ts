...
export async function GET(req: NextRequest) {
  try {
    ...
    // Show ALL orders for the user (including those with and without stripeSessionId)
    // This ensures users see all their orders regardless of payment status
    // Orders without stripeSessionId might be test orders or orders created before payment

    if (status && status !== 'all') {
      where.status = status.toUpperCase();
    }
    
    // Ensure we get orders regardless of stripeSessionId presence
    // This is important because orders should be visible even if webhook hasn't processed yet
