  ...
  try {
    ...
    return NextResponse.json({
      ...
      review: {
        ...
        productTitle: review.product?.title || review.dish?.title || 'Item',
        productImage: review.product?.Image[0]?.fileUrl || null,
        sellerName: review.product?.seller?.User?.name || review.product?.seller?.User?.username || review.dish?.user?.name || 'Verkoper',
