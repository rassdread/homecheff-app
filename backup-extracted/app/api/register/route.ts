...
export async function POST(req: Request) {
  ...
  if (isBusiness) {
      ...
      // Map subscription value from form ("basic", "pro", "premium") to database name ("Basic", "Pro", "Premium")
      const subscriptionName = normalizeSubscriptionName(subscription);
      ...
      if (!sub) {
        sub = await prisma.subscription.findUnique({ 
          where: { 
            id: subscription.toLowerCase()
          } 
        });
      }
      
      // Also check isActive after finding by id
      if (sub && !sub.isActive) {
        sub = null;
      }
      
      if (!sub) {
        console.error(`Subscription not found: ${subscription} (mapped to name: ${subscriptionName})`);
        // List available subscriptions for debugging
        const availableSubs = await prisma.subscription.findMany({ where: { isActive: true } });
        console.error('Available subscriptions:', availableSubs.map(s => ({ id: s.id, name: s.name })));
        return NextResponse.json({ 
          error: `Ongeldig abonnement: ${subscription}. Beschikbare abonnementen: ${availableSubs.map(s => s.name).join(', ')}` 
        }, { status: 400 });
      }
