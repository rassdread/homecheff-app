...
export class NotificationService {
      ...
      const htmlEmail = `
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>${message.body}</p>
              ${message.data?.link ? `<a href="${message.data.link}" class="button">Bekijk Details</a>` : ''}
            </div>
            <div class="footer">
  ...
  static async sendOrderReadyForPickupNotification(buyerId: string, sellerId: string, orderId: string, orderNumber: string, pickupAddress: string): Promise<void> {
    const buyerMessage: NotificationMessage = {
      ...
      body: `Je bestelling #${orderNumber} is klaar! Je kunt het ophalen bij de verkoper.`,
      data: {
      ...
      actions: [
        { label: 'Bekijk adres', action: 'VIEW_ADDRESS' },
